...
#include "GameWiring/GameWiring.h"
...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Consegna Finale");
    ...
    bool prevDiscardActive = false; // track previous active state to detect finish transition
    ...
    auto closeResponsePrompt = [&](){
        // Interpreta la chiusura come "passa nella chain" per il giocatore locale, se una chain è attiva
        if(game && game->isChainActive()){
            // il giocatore locale è P1 (indice 0)
            game->playerPassOnChain(0);
            // Esegui auto-pass dell'AI/avversario per permettere la risoluzione (priorità chain semplificata)
            game->playerPassOnChain(1);
        }
    ...
    if(inputBlocked()) return;
        ...
        int cur = game->getTurn().getCurrentPlayerIndex();
        // Mostra SOLO le carte del giocatore locale (P1, indice 0) nel prompt di risposta.
        // Il giocatore locale può attivare le sue Magie Rapide/Trappole in risposta; non
        // esporre le carte dell'avversario in questo prompt.
        const int localPlayerIdx = 0; // P1 (bottom) è il giocatore umano locale
        responseOwner = localPlayerIdx;
        ...
        if (!responseActivables.empty()) {
            ...
            // Se non ci sono attivazioni disponibili, assicura che il prompt sia chiuso
            if(responsePromptActive) closeResponsePrompt();
            // Se una chain è attiva ma non ci sono risposte disponibili, esegui auto-pass per risolvere
            if(game && game->isChainActive()){
                game->playerPassOnChain(0);
                game->playerPassOnChain(1);
            }
    ...
    // Contesto per le sottoscrizioni di AppHandlers (alzato così il reset può scollegare in sicurezza)
    ...
    auto resetMatch = [&](){
        ...
        scrollOffset = 0.f;
        // Forza chiusura di eventuali overlay aperti
        extraOverlay.handleEscape();
        graveyardOverlay.handleEscape();
        // Svuota le cache interne degli overlay per evitare sprite/texture residue tra le partite
        extraOverlay.reset();
        graveyardOverlay.reset();
        ...
        // Pulisci tutti i flag/modali di input che possono bloccare click/tasti
        ...
        ssChoiceActive = false;
        responsePromptActive = false;
        responseActivables.clear();
        responseSelected = 0;
        responseOwner = -1;
        deckSendChoiceActive = false;
        deckSendCandidates.clear();
        deckSendIndexMap.clear();
        deckSendOwner = -1;
        deckSendSelected = 0;
        std::cout << "[ResetMatch] Modal flags cleared; returning to Home." << std::endl;
        
    // Scollega gli handler prima di distruggere Game per evitare callback pendenti
        if(game){
            try {
        ...
        deck.resetDeckCardPositions(
            ...
            AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack));
        deck.resetAnimation();
    // Assicura che il controller di scarto sia resettato per evitare residui visivi
    ...
    // Assicura che gli overlay vengano resettati dopo la ricreazione dei deck
    extraOverlay.reset();
    graveyardOverlay.reset();
    ...
    // Annulla il drag attivo: riporta la carta in mano e resetta lo stato di drag
    auto cancelActiveDrag = [&](){
        if(isDragging && draggingCardIndex.has_value() && handPtr && draggingCardIndex.value() < handPtr->size()){
            ...
            // Evita forward-reference: richiamiamo direttamente updateHandPositions
            updateHandPositions(*handPtr, windowSize, cardSize, spacing, y, HAND_MAXSIZE);
        }
        isDragging = false;
        isPotentialDrag = false;
    ...
    Input::Context state{
        ...
        /*graveyardSlotPosTop*/field.getSlotPosition(Type::Graveyard, P2),
        ...
        /*confirmDeckSend*/[&](size_t idx){
            if(game){
                bool ok = false;
                // Se abbiamo una richiesta di Add (Melodia) attiva, risolviamo quella; altrimenti Send (Reliquiario)
                if(game->hasPendingAddFromDeck()){
                    ok = game->resolvePendingAddFromDeck(idx);
                    if(ok){ battleFeedbackMsg = "Carta aggiunta alla mano"; battleFeedbackClock.restart(); }
                } else if(game->hasPendingAddLevel1Dragon()) {
                    ok = game->resolvePendingAddLevel1Dragon(idx);
                    if(ok){ battleFeedbackMsg = "Aggiungi Drago Livello 1 alla mano"; battleFeedbackClock.restart(); }
                } else if(game->hasPendingSendFromDeck()) {
                    ok = game->resolvePendingSendFromDeck(idx);
                    if(ok){ battleFeedbackMsg = "Carta inviata al Cimitero"; battleFeedbackClock.restart(); }
                } else if(game->hasPendingBlueEyesSSChoice()) {
                    ok = game->resolvePendingBlueEyesOrSpiritChoice(idx);
                    if(ok){ battleFeedbackMsg = "Selezionato Drago Bianco/Spirito per SS"; battleFeedbackClock.restart(); }
                } else if(game->hasPendingGraveyardChoice()) {
                    size_t mapped = idx;
                    if(idx < deckSendIndexMap.size()) mapped = deckSendIndexMap[idx];
                    ok = game->resolvePendingSelectFromGraveyard(mapped);
                    if(ok){ battleFeedbackMsg = "Bersaglio selezionato dal Cimitero"; battleFeedbackClock.restart(); }
                } else if(game->hasPendingOpponentSpellTrapChoice()) {
                    // L'index map qui mappa ST index o SIZE_MAX per Field Spell; passiamo l'indice nella lista virtuale così com'è
                    ok = game->resolvePendingSelectOpponentSpellTrap(idx);
                    if(ok){ battleFeedbackMsg = "M/T avversaria bandita"; battleFeedbackClock.restart(); }
                }
                if(ok){
                    // Se dopo la risoluzione resta una Add pendente, stiamo riaprendo per la seconda scelta: non chiudere l'overlay
                    if(game->hasPendingAddFromDeck()){
                        deckSendSelected = 0; // reset selezione per la nuova lista
                    } else {
                        deckSendChoiceActive = false; deckSendCandidates.clear(); deckSendIndexMap.clear(); deckSendSelected = 0; deckSendOwner = -1;
                    }
                }
                if(!ok){ battleFeedbackMsg = "Selezione non valida"; battleFeedbackClock.restart(); }
            }
        },
    ...
    /*confirmResponsePrompt*/[&](size_t sel){
            if(game && sel < responseActivables.size()){
                ...
                if(ok){
                    ...
                    syncSpellTrapZones();
                    // NON chiudere il prompt qui: resta aperto per consentire ulteriori risposte nella chain
                    // Verrà chiuso quando i giocatori passeranno o non ci saranno più attivazioni disponibili.
                } else {
    ...
    while (window.isOpen()) {
                ...
                if(but == M_MOUSE_LEFT) {
                    ...
                    if(!inputBlocked() && gamestate == GameState::HomeScreen && homeScreen.getGiocaBounds().contains(initialMousePos)) {
                        if(selectedDeckName.empty()) {
                            ...
                            GameWiring::createGameAndAttachResources(selectedDeckName, deck, opponentDeck, resourceManager, game, drawController, aiP2, handPtr, p1STCards, p2STCards, p1FieldSpellCard, p2FieldSpellCard, textureManager);
                ...
                if (!cardClicked) {
                    for (size_t i = 0; i < fieldCards.size(); ++i) {
                        if (fieldCards[i].isClicked(mousePos)) {
                            ...
                            scrollOffset = 0.f;
                            // Se la logica di gioco ha richiesto la selezione di un proprio mostro (es. Saggio), risolvi al click
                            if(game && game->hasPendingOwnMonsterChoice()){
                                auto ownerOpt = game->getPendingOwnMonsterOwner();
                                int owner = ownerOpt.has_value() ? *ownerOpt : game->getTurn().getCurrentPlayerIndex();
                                // Consenti solo a P1 di scegliere dal proprio lato
                                if(owner == 0){
                                    if(game->resolvePendingSelectOwnMonster(i)){
                                        // Sync immediato del campo dopo l'invio al Cimitero
                                        syncMonsterZoneToField();
                                        // Non continuare con selezioni battaglia o tributi in questo click
                                        break;
                                    }
                                }
                            }
                            ...
                            if(game && !inputBlocked() && !game->hasPendingOwnMonsterChoice() && game->getTurn().getPhase() == GamePhase::Battle){
                    ...
                    if (!inputBlocked() && isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (hasFreeSlotUnderMouse || canOpenSummonChoiceForTribute) {
                            ...
                            if(game && !inputBlocked()){
                                // Cattura l'indice della carta in mano PRIMA di annullare il drag,
                                // così non accediamo a draggingCardIndex dopo cancelActiveDrag()
                                size_t handIdxForPlay = draggingCardIndex.value();
                                pendingHandIndexForPlay = handIdxForPlay;
                                Type t = (*handPtr)[handIdxForPlay].getType();
                                if(t == Type::Monster){
                                    pendingIsSpellTrap = false;
                                    chooseSummonOrSet = true;
                                    // Preleva il nome prima di cancellare il drag per evitare invalidazioni
                                    ...
                                    // Riporta la carta in mano come per i mostri: evita che resti flottante
                                    cancelActiveDrag();
        ...
        if(deckSelected){
            ...
            fieldCards.clear();
            drawController.reset();
            // Resetta eventuali animazioni/pending di scarto residue
            discardController.reset();
            selectedCardIndex.reset();
            selectedCardIsOnField = false;
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            syncZones();
                            // popola il contesto hoisted e collega gli handler tramite l'helper
                            GameWiring::setupAppHandlers(
                                ...
                                /*setSelectingTributes*/[&](bool v){ 
                                    selectingTributes = v; 
                                    if(v){ cancelActiveDrag(); }
                                },
                                ...
                                /*setDeckSendIndexMap*/[&](const std::vector<size_t>& m){ deckSendIndexMap = m; },
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                // Se siamo in Draw Phase e la pescata è terminata, passa subito alla prossima fase (Standby/Main1)
                // Salta solo nelle pescate di apertura alternata
                if(drawController.idle()
                   && game->getTurn().getPhase() == GamePhase::Draw
                   && !openingAltDrawActive
                   && !showDrawnCardDetails
                   && !inputBlocked()){
                    game->advancePhase();
                }
                ...
                // Rileva transizione active -> inactive per chiamare onDiscardAnimationFinished
                bool curDiscardActive = discardController.active();
                if(game->getTurn().getPhase() == GamePhase::End){
                    if(!curDiscardActive && prevDiscardActive){
                        // l'animazione è appena terminata
                        updateHandLayout();
                        game->onDiscardAnimationFinished();
                        std::cout << "[AutoTurn] Fine turno dopo animazione scarto." << std::endl;
                    } else if(!curDiscardActive && !prevDiscardActive){
                        // Non ci sono animazioni in corso: se serve auto end turn, chiudilo
                ...
                prevDiscardActive = curDiscardActive;
        ...
        // Se la logica di gioco ha segnato la partita come terminata, assicuriamoci
        // che l'overlay Game Over venga mostrato impostando il flag UI corrispondente.
        // Questa variabile e' posseduta dal main e osservata dal controller di input.
        if (game && game->isGameOver() && !gameOverActive) {
            gameOverActive = true;
        }
        ...
        if(gamestate == GameState::HomeScreen){
            // Rete di sicurezza: la Home non dovrebbe mai essere bloccata; se lo è, pulisci i modali
            if(inputBlocked()){
                selectingTributes = false;
                selectedTributes.clear();
                tributesNeeded = 0;
                ...
                extraOverlay.handleEscape();
                graveyardOverlay.handleEscape();
                std::cout << "[Home] Cleared unexpected modal state." << std::endl;
            }
            ...
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                bool isHighlighted = std::find(selectedTributes.begin(), selectedTributes.end(), i) != selectedTributes.end();
                RenderUtils::drawMonsterCard(window, fieldCards[i], /*isOpponentSide*/false, slotPos, slotSize, isDef, isFD, isSelected, showDot, isHighlighted, tex);
            }
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                RenderUtils::drawMonsterCard(window, oppFieldCards[i], /*isOpponentSide*/true, slotPos, slotSize, isDef, isFD, /*isSelectedAttacker*/false, showDot, /*isHighlighted*/false, tex);
            }
            ...
            // Disegna la pila Cimitero (semplice: ultima carta)
            {
                ...
                if (game && !game->getGraveyardOf(0).empty()) top = game->getGraveyardOf(0).back();
            ...
            {
                std::optional<Card> top;
                if (game && !game->getGraveyardOf(1).empty()) top = game->getGraveyardOf(1).back();
                RenderUtils::drawTopCardAt(
                    window,
                    top,
            ...
            // Disegna le carte in mano (P1 in basso) con texture frontali
            if(handPtr){ RenderUtils::drawPlayerHand(window, *handPtr, ssChoiceActive, &textureManager); }
            ...
            // NOTA: se un overlay (Extra/Graveyard) è visibile, non mostrare contemporaneamente i dettagli della selezione di campo
            if (selectedCardIndex.has_value() && !extraOverlay.isOverlayVisible() && !graveyardOverlay.isOverlayVisible()) {
            ...
            if(game){
                // Disegna il cimitero relativo all'overlay: se l'overlay è visibile usa l'owner corrente, altrimenti mostra P1 in basso
                if(graveyardOverlay.isOverlayVisible()){
                    int owner = graveyardOverlay.getCurrentOwner();
                    graveyardOverlay.draw(window, detailFont, game->getGraveyardOf(owner), scrollOffset);
                } else {
                    graveyardOverlay.draw(window, detailFont, game->getGraveyardOf(0), scrollOffset);
                }
            }
            ...
            if(game){
                ...
                if(deckSendChoiceActive){
                    // Se la Game ha una Add-from-Deck pendente (Melodia), adatta testo titolo e hint
                    std::string titleOverride;
                    std::string hintOverride;
                    if(game && game->hasPendingAddFromDeck()){
                        int rem = game->getMelodiaAddsRemaining();
                        titleOverride = "Scegli un Mostro Drago dal Deck da aggiungere alla mano";
                        hintOverride = rem > 1 ?
                            "Invio = Conferma, ESC = Fine (puoi aggiungere fino a 2), Frecce o Click per selezionare" :
                            "Invio = Conferma, ESC = Annulla, Frecce o Click per selezionare";
                    } else if(game && game->hasPendingBlueEyesSSChoice()){
                        // Fanciulla/Saggio: scelta del Drago da Evocare Specialmente da mano/Deck/Cimitero
                        titleOverride = "Seleziona mostro da evocare specialmente";
                    } else if(game && game->hasPendingOpponentSpellTrapChoice()){
                        // Spirito del Bianco: scelta M/T avversaria da bandire
                        titleOverride = "Scegli la carta da bandire";
                    } else if(game && game->hasPendingAddLevel1Dragon()){
                        // Saggio on-summon: aggiungi un Drago di Livello 1 alla mano
                        // Richiesta: usare il testo specifico seguente al posto del default
                        titleOverride = "scegli il mostro da aggiungere alla mano";
                    } else if(game && game->hasPendingGraveyardChoice()){
                        // Richiamo del Posseduto: scelta dal Cimitero
                        titleOverride = "Scegli il mostro da evocare";
                    }
                    RenderUtils::drawDeckSendOverlay(
                        ...
                        hintOverride,
                        &tex,
                        (game && game->hasPendingOpponentSpellTrapChoice())
                    );
                }
                ...
                // Prompt persistente durante la scelta di scarto dalla mano
                if(game && game->hasPendingHandDiscard()){
                    RenderUtils::drawCenterFeedback(window, detailFont, windowSize, "Seleziona una carta in mano da scartare", AppConfig::Ui::HudSize, AppConfig::Ui::CenterFeedbackY);
                }
                if(!battleFeedbackMsg.empty() && !(game && game->hasPendingHandDiscard())){
