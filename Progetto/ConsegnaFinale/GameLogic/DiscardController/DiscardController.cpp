...
void DiscardController::start(std::vector<Card>&& cards, sf::Vector2f graveyardCenter, int ownerIdx, float duration) {
    animations.clear();
    pending.clear();
    pendingOwner = ownerIdx;
    for (auto& c : cards) {
...
void DiscardController::update(float dt, Game& game) {
    ...
    if (endCount == animations.size()) {
        if(pendingOwner >= 0){
            auto& gy = game.getGraveyardOf(pendingOwner);
