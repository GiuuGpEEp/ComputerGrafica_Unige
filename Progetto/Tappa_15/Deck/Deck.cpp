                        ...
                        sf::Texture& textureFlipped)
{
    ...
    return Deck(deckCards, extra, deckSlotPos, extraDeckSlotPos, deckCardSize, slotSize, textureFlipped, deckName);
}



Deck::Deck(const std::vector<Card>& cards, const std::vector<Card>& extraCards,
        sf::Vector2f deckSlotPos, sf::Vector2f extraDeckSlotPos, sf::Vector2f deckCardSize, sf::Vector2f slotSize, sf::Texture& textureFlipped, std::string deckName)
    : cards(cards), extraCards(extraCards), cardsSize(static_cast<int>(cards.size())), animationFinished(false), deckName(deckName)
{
    ...
    for (int i = 0; i < numberExtraDeckCards; ++i) {
        sf::Vector2f slotCenter = extraDeckSlotPos + sf::Vector2f(slotSize.x / 2.f, slotSize.y / 2.f + 5.f);
        sf::Vector2f offset(0.f, -1.f * (numberExtraDeckCards - 1 - i));
        sf::Vector2f centeredPos = slotCenter - sf::Vector2f(deckCardSize.x / 2.f, deckCardSize.y / 2.f) + offset;
        this->extraCards[i].setPosition(centeredPos);
        this->extraCards[i].setTexture(textureFlipped);
        sf::Vector2u texSize = textureFlipped.getSize();
        this->extraCards[i].setTextureRect(sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(texSize.x, texSize.y)));
        this->extraCards[i].setSize(deckCardSize);
        lastPos = centeredPos;
    }   
    
    if (numberExtraDeckCards > 0) {
        this->extraCards[numberExtraDeckCards - 1].setPosition(lastPos);
        this->extraCards[numberExtraDeckCards - 1].setTexture(textureFlipped);
        sf::Vector2u texSize = textureFlipped.getSize();
        this->extraCards[numberExtraDeckCards - 1].setTextureRect(sf::IntRect(sf::Vector2i(0, 0), sf::Vector2i(texSize.x, texSize.y)));
        this->extraCards[numberExtraDeckCards - 1].setSize(deckCardSize);
    }
...
void Deck::resetAnimation(){
    deckAlpha = 0.f;
    animationFinished = false;
}

std::optional<Card> Deck::removeFirstByName(const std::string& name){
    for(size_t i = 0; i < cards.size(); ++i){
        if(cards[i].getName() == name){
            Card found = cards[i];
            cards.erase(cards.begin() + static_cast<long long>(i));
            cardsSize = static_cast<int>(cards.size());
            return found;
        }
    }
    return std::nullopt;
...
std::optional<Card> Deck::findFirst(std::function<bool(const Card&)> pred) const{
    for(const auto& c : cards){
        if(pred(c)) return c; // copia per ispezione
    }
    return std::nullopt;
}

std::vector<Card> Deck::collectWhere(std::function<bool(const Card&)> pred) const{
    std::vector<Card> out;
    out.reserve(cards.size());
    for(const auto& c : cards){ if(pred(c)) out.push_back(c); }
    return out;
}
