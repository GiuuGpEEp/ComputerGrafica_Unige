...
std::optional<size_t> AIController::findFirstFreeMonsterSlot() const {
    ...
    // Calcola slot occupati leggendo la mappatura sui mostri del giocatore di turno (che è l'AI)
    std::set<int> used;
    const auto& mz = game_->getMonsterZone();
    for(size_t i=0;i<mz.size();++i){
        int slot = game_->getMonsterSlotIndexAt(aiPlayerIdx_, i);
        if(slot >= 0) used.insert(slot);
    }
    for(size_t s=0; s<3; ++s){
        if(used.find((int)s) == used.end()) return s;
    }
    return std::nullopt;
}
...
void AIController::update(float){
    ...
    switch(game_->getTurn().getPhase()){
        case GamePhase::Draw:
            // Avanza subito oltre la Draw (la pescata è gestita dal DrawController nel layer esterno)
            game_->advancePhase();
            break;
        case GamePhase::Standby:
            // La Standby viene saltata automaticamente in Game::processEnterStandby quando si entra in questa fase
            // Se ci trovassimo qui, non facciamo nulla: l'avanzamento è già stato fatto nel passaggio da Draw
            break;
