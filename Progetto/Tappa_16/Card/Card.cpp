...
Card Card::cardFromJson(const nlohmann::json& jsonData, sf::Vector2f pos, sf::Vector2f size, TextureManager& textureManager) {
    ...
    const std::vector<std::string> deckFolders = {"textureDeck1/", "textureDeck2/", "textureDeck3/"};
...
Attribute Card::stringToAttribute(const std::string& str) {
    ...
    if (str == "Oscurita" || str == "Oscurità") return Attribute::Oscurita;
...
Feature Card::stringToFeature(const std::string& str) {
    ...
    if (str == "Gemello") return Feature::Gemello;
...
void Card::setRotation(float degrees){
    if(sprite.has_value()) sprite->setRotation(sf::degrees(degrees));
}

void Card::setOrigin(sf::Vector2f origin){
    if(sprite.has_value()) sprite->setOrigin(origin);
}

void Card::centerOrigin(){
    if(sprite.has_value()){
        auto gb = sprite->getLocalBounds();
        sprite->setOrigin(sf::Vector2f(gb.size.x/2.f, gb.size.y/2.f));
    }
}

void Card::resetOrigin(){
    if(sprite.has_value()) sprite->setOrigin(sf::Vector2f(0.f,0.f));
}
