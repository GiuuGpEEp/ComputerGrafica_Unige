...
void FieldLoadingAnimation::draw(sf::RenderWindow& window){
    ...
    homeSprite.setScale(sf::Vector2f(
        float(window.getSize().x) / sizeH.x * homeScaleFactor,
        float(window.getSize().y) / sizeH.y * homeScaleFactor
    ));
    fieldSprite.setScale(sf::Vector2f(
        float(window.getSize().x) / sizeF.x * fieldScaleFactor,
    ...
    homeSprite.setPosition(sf::Vector2f(windowCenter.x - hBounds.size.x / 2.f, windowCenter.y - hBounds.size.y / 2.f));
    fieldSprite.setPosition(sf::Vector2f(windowCenter.x - fBounds.size.x / 2.f, windowCenter.y - fBounds.size.y / 2.f));
