...
bool ResourceManager::initialize(const sf::Vector2f& deckSlotPos, const sf::Vector2f& extraDeckSlotPos, const sf::Vector2f& deckCardSize, const sf::Vector2f& slotSize, sf::Texture& textureFlipped) {
    try {
        ...
        loadDecksFromJson(decksJsonPath, deckSlotPos, extraDeckSlotPos, deckCardSize, slotSize, textureFlipped);
...
void ResourceManager::loadDecksFromJson(const std::string& decksJsonPath, const sf::Vector2f& deckSlotPos, const sf::Vector2f& extraDeckSlotPos, const sf::Vector2f& deckCardSize, const sf::Vector2f& slotSize, sf::Texture& textureFlipped) {
    ...
    if (decksJson.is_array()) {
        for (const auto& deckJson : decksJson) {
            try {
                Deck deck = Deck::deckFromJson(deckJson, allCards, deckSlotPos, extraDeckSlotPos, deckCardSize, slotSize, textureFlipped);
        ...
        try {
            Deck deck = Deck::deckFromJson(decksJson, allCards, deckSlotPos, extraDeckSlotPos, deckCardSize, slotSize, textureFlipped);
            std::string deckName = deck.getDeckName();
...
Deck ResourceManager::getDeckByName(const std::string& deckName) const {
    ...
    if (it != availableDecks.end()) {
        return it->second; // Restituisce una copia
    }
    // Restituisci un Deck vuoto se non trovato
    return Deck();
}
...
Card ResourceManager::getCardByName(const std::string& cardName) const {
    ...
    // Restituisci una Card vuota se non trovata
    return Card();
}
...
const std::unordered_map<std::string, Card>& ResourceManager::getAllCards() const {
    return allCards;
}

bool ResourceManager::hasDeck(const std::string& deckName) const { 
    return availableDecks.find(deckName) != availableDecks.end(); 
}

size_t ResourceManager::getDeckCount() const { 
    return availableDecks.size(); 
}

bool ResourceManager::hasCard(const std::string& cardName) const { 
    return allCards.find(cardName) != allCards.end(); 
}