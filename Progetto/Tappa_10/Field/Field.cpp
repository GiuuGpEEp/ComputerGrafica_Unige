...
void Field::draw(sf::RenderWindow& window, const sf::Vector2i& mousePos, GameState gamestate, bool enableHover) const {
    ...
    if(gamestate == GameState::FieldVisible || gamestate == GameState::Playing) {
        for (const auto& slot : slots) {
            bool hovered = enableHover ? slot.isHovered(mousePos) : false;
...
void Field::resetAnimation(){
    animationFinished = false;
    fieldAlpha = 0.f;
    fieldOffset = 200.f; // stesso valore iniziale usato nel costruttore
}
