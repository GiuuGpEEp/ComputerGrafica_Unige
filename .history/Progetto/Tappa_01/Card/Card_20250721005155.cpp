...
Card::Card(const std::string& name,const std::string& description, int atk, int def, sf::Vector2f pos)
    : name(name), description(description), attack(atk), defense(def)
{
    ...
    //Creo il testo della carta usando emplace per costruirlo in-place
...
//Distruttore
Card::~Card() {
    std::cout << "Distruggendo carta: " << name << std::endl;
}
