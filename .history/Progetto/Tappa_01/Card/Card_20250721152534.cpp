    ...
    : name(name), description(description), attack(atk), defense(def) //Notazione alternativa per l'inizializzazione dei membri = a this -> name = name
{
    ...
    // Prova a caricare una texture per la carta
    if (!cardTexture.openFromFile("card_texture.png")) {
        // Se la texture non esiste, crea una texture di fallback colorata
        std::cerr << "Texture carta non trovata, uso colore di fallback" << std::endl;
        
        // Creiamo una texture semplice gialla come fallback
        sf::Image fallbackImage;
        fallbackImage.create(100, 150, sf::Color::Yellow);
        cardTexture.loadFromImage(fallbackImage);
    }
...
bool Card::isClicked(const sf::Vector2i& mousePos) {
    return cardSprite.getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePos));
}
