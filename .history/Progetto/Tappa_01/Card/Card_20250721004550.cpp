...
Card::Card(const std::string& name,const std::string& description, int atk, int def, sf::Vector2f pos)
    : name(name), description(description), attack(atk), defense(def), label(font, "", 12) //Notazione alternativa per l'inizializzazione dei membri = a this -> name = name
{
    ...
    //Aggiorno il testo della carta con il font caricato
    label = sf::Text{font, name + "\n" + description + "\nATK: " + std::to_string(attack) + "\tDEF: " + std::to_string(defense), 12};
