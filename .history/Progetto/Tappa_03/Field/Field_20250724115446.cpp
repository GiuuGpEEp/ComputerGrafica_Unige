...
{
    //Ruoto le texture per il player 2
    // SFML 3.0: sf::Transform and setRotation are deprecated for sprites, use setScale for flipping
    // Per ruotare la texture dei mostri per il player 2, si può usare setScale(-1, -1) e setOrigin al centro
    // Esempio per un singolo sprite:
    // sf::Sprite rotatedMonsterSprite(monsterTexture);
    // rotatedMonsterSprite.setOrigin(monsterTexture.getSize().x / 2.f, monsterTexture.getSize().y / 2.f);
    // rotatedMonsterSprite.setScale(-1.f, -1.f);

    // NOTA: Se vuoi ruotare le texture per i player 2, dovrai gestirlo quando crei gli Slot
    // Ad esempio, per i mostri di player 2:
    // slots.emplace_back(..., monsterTexture, ..., slotSize, true /* flipped */);
    // E poi nello Slot::draw, applica setScale(-1, -1) se flipped == true

    // Qui non serve creare sprite temporanei, la rotazione va gestita nel rendering degli Slot
