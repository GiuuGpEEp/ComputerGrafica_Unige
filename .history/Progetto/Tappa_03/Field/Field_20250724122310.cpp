        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    sf::Texture rotatedResultTexture = createRotatedTexture(spellTrapTexture);

    // 3 slot magie/trappole giocatore 2
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), rotatedResultTexture, Slot::Type::SpellTrap, slotSize);
    }

    // Crea la texture ruotata per i mostri del giocatore 2
    //createRotatedTexture(monsterTexture, rotatedResultTexture);
...
sf::Texture Field::createRotatedTexture(const sf::Texture& original) {
    ...
    sf::Texture rotated = renderTexture.getTexture();
    return rotated;
}
