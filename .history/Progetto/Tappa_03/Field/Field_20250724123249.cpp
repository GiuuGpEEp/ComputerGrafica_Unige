...
{
    // Crea le texture ruotate una sola volta all'inizio
    createRotatedTextures(monsterTexture, spellTrapTexture);
    ...
    // 3 slot magie/trappole giocatore 2 (ruotate)
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), rotatedSpellTrapTexture, Slot::Type::SpellTrap, slotSize);
    }

    // 3 slot mostri giocatore 2 (con texture ruotata di 180 gradi)
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), rotatedMonsterTexture, Slot::Type::Monster, slotSize);
    }
    ...
    slots.emplace_back(sf::Vector2f(rightX, p1MonsterY), deckTexture, Slot::Type::Deck, slotSize);
...
void Field::createRotatedTextures(const sf::Texture& monsterTexture, const sf::Texture& spellTrapTexture) {
    // Crea le texture ruotate e le memorizza come membri della classe
    rotatedMonsterTexture = createRotatedTexture(monsterTexture, 180.0f);
    rotatedSpellTrapTexture = createRotatedTexture(spellTrapTexture, 180.0f);
}

sf::Texture Field::createRotatedTexture(const sf::Texture& original, float degrees) const {
    // Funzione generica per ruotare qualsiasi texture di un numero specificato di gradi
    
    // Converte la texture in immagine
    sf::Image originalImage = original.copyToImage();
    ...
    // Crea una nuova immagine ruotata
    sf::Image rotatedImage;
    rotatedImage.resize(size);
    
    // Per ora supportiamo solo rotazioni di 180 gradi (pixel per pixel)
    // In futuro si può estendere per altre rotazioni
    if (degrees == 180.0f || degrees == -180.0f) {
        // Ruota di 180 gradi: pixel per pixel
        for (unsigned int x = 0; x < width; ++x) {
            for (unsigned int y = 0; y < height; ++y) {
                sf::Color pixel = originalImage.getPixel(sf::Vector2u(x, y));
                // Ruota di 180 gradi: nuovo_x = width - 1 - x, nuovo_y = height - 1 - y
                rotatedImage.setPixel(sf::Vector2u(width - 1 - x, height - 1 - y), pixel);
            }
        }
    } else if (degrees == 90.0f || degrees == -270.0f) {
        // Ruota di 90 gradi in senso orario
        rotatedImage.resize(sf::Vector2u(height, width)); // Dimensioni scambiate
        for (unsigned int x = 0; x < width; ++x) {
            for (unsigned int y = 0; y < height; ++y) {
                sf::Color pixel = originalImage.getPixel(sf::Vector2u(x, y));
                // Ruota di 90 gradi: nuovo_x = height - 1 - y, nuovo_y = x
                rotatedImage.setPixel(sf::Vector2u(height - 1 - y, x), pixel);
            }
        }
    } else if (degrees == 270.0f || degrees == -90.0f) {
        // Ruota di 270 gradi in senso orario (90 gradi in senso antiorario)
        rotatedImage.resize(sf::Vector2u(height, width)); // Dimensioni scambiate
        for (unsigned int x = 0; x < width; ++x) {
            for (unsigned int y = 0; y < height; ++y) {
                sf::Color pixel = originalImage.getPixel(sf::Vector2u(x, y));
                // Ruota di 270 gradi: nuovo_x = y, nuovo_y = width - 1 - x
                rotatedImage.setPixel(sf::Vector2u(y, width - 1 - x), pixel);
            }
        }
    } else {
        // Angolo non supportato, restituisce l'originale
        rotatedImage = originalImage;
    }
    
    // Crea la nuova texture dall'immagine ruotata
    sf::Texture rotatedTexture;
    rotatedTexture.loadFromImage(rotatedImage);
