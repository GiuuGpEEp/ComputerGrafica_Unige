...
{
    // Crea la texture ruotata per il mostro del giocatore 2
...
void Field::createRotatedTexture(const sf::Texture& original, sf::Texture& rotated) {
    ...
    /*Le texture sono solo immagini, non possono essere ruotate, quindi creo uno sprite a cui applico la texture */
    
    sf::RenderTexture renderTexture;  //Mentre le texture normali sono solo delle immagini da caricare, le renderTexture sono delle texture speciali su cui si puù disegnare, ciò permette di applicare trasformazioni ad un immagine per poi riusarla come texture
