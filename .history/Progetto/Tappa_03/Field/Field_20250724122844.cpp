...
{
    // Crea la texture ruotata per i mostri del giocatore 2
    createRotatedMonsterTexture(monsterTexture);
...
void Field::createRotatedMonsterTexture(const sf::Texture& original) {
    // Proviamo un approccio più semplice: carichiamo l'immagine, la ruotiamo e ricreiamo la texture
    ...
    // Crea una nuova immagine ruotata di 180 gradi
    sf::Image rotatedImage;
    rotatedImage.create(width, height, sf::Color::Transparent);
    
    // Ruota pixel per pixel (più lento ma più affidabile)
    for (unsigned int x = 0; x < width; ++x) {
        for (unsigned int y = 0; y < height; ++y) {
            sf::Color pixel = originalImage.getPixel(x, y);
            // Ruota di 180 gradi: nuovo_x = width - 1 - x, nuovo_y = height - 1 - y
            rotatedImage.setPixel(width - 1 - x, height - 1 - y, pixel);
        }
    }
    
    // Carica l'immagine ruotata nella texture
    rotatedMonsterTexture.loadFromImage(rotatedImage);
}
