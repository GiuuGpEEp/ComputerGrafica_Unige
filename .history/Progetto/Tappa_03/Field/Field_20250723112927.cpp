        ...
        ) : background(fieldTexture) 
{
   ...
   for (int i = 0; i < 3; ++i) {
    ...
    // ------- Giocatore 2 -------
    for (int i = 0; i < 3; ++i) {
        slots.emplace_back(sf::Vector2f(xStart + i * (slotWidth + spacing), topY), monsterTex, Slot::Type::Monster);
        slots.emplace_back(sf::Vector2f(xStart + i * (slotWidth + spacing), topY + slotHeight + spacing), spellTex, Slot::Type::SpellTrap);
    }

    slots.emplace_back(sf::Vector2f(600.f, topY), deckTex, Slot::Type::Deck);
    slots.emplace_back(sf::Vector2f(600.f, topY + slotHeight + spacing), graveTex, Slot::Type::Graveyard);
    slots.emplace_back(sf::Vector2f(680.f, topY), extraTex, Slot::Type::Extra);
    slots.emplace_back(sf::Vector2f(680.f, topY + slotHeight + spacing), fieldSlotTex, Slot::Type::Field);
