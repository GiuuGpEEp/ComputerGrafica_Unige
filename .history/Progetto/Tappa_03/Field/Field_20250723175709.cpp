        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        slots.emplace_back(sf::Vector2f(x, p1MonsterY), monsterTexture, slotSize);
    }
    
    // 3 slot magie/trappole giocatore 1
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p1SpellY), spellTrapTexture, slotSize);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), spellTrapTexture, slotSize);
    }
    
    // 3 slot mostri giocatore 2
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), monsterTexture, slotSize);
    }
    ...
    slots.emplace_back(sf::Vector2f(rightX, p1MonsterY), deckTexture, slotSize);
    slots.emplace_back(sf::Vector2f(rightX, p1SpellY), graveyardTexture, slotSize);

    // Zone speciali giocatore 2 (destra del campo)
    slots.emplace_back(sf::Vector2f(rightX, p2MonsterY), deckTexture, slotSize);
    slots.emplace_back(sf::Vector2f(rightX, p2SpellY), graveyardTexture, slotSize);

    // Extra Deck e Field Spell a sinistra
    slots.emplace_back(sf::Vector2f(leftX, p1MonsterY), extraDeckTexture, slotSize);
    slots.emplace_back(sf::Vector2f(leftX, p1SpellY), fieldSpellTexture, slotSize);

    slots.emplace_back(sf::Vector2f(leftX, p2MonsterY), extraDeckTexture, slotSize);
    slots.emplace_back(sf::Vector2f(leftX, p2SpellY), fieldSpellTexture, slotSize);
