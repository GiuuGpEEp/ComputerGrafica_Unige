...
void Field::createSlots(const sf::Texture& slotTexture) {
   ...
   for (int i = 0; i < 3; ++i) {
    ...
    // ------- Giocatore 2 -------
    for (int i = 0; i < 3; ++i) {
        slots.emplace_back(sf::Vector2f(xStart + i * (spacing), topY), monsterTexture, Slot::Type::Monster);
        slots.emplace_back(sf::Vector2f(xStart + i * (spacing), topY + slotHeight + spacing), spellTexture, Slot::Type::SpellTrap);
    }
