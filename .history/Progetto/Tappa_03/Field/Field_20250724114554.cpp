        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    // 3 slot magie/trappole giocatore 2 (ruotati)
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), spellTrapTexture, Slot::Type::SpellTrap, slotSize, true);
    }
    
    // 3 slot mostri giocatore 2 (ruotati)
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), monsterTexture, Slot::Type::Monster, slotSize, true);
    }
    ...
    // Zone speciali giocatore 2 (destra del campo) - ruotate
    slots.emplace_back(sf::Vector2f(rightX, p2MonsterY), extraDeckTexture, Slot::Type::Extra, slotSize, true);
    slots.emplace_back(sf::Vector2f(rightX, p2SpellY), fieldSpellTexture, Slot::Type::FieldSpell, slotSize, true);
    ...
    slots.emplace_back(sf::Vector2f(leftX, p2MonsterY), deckTexture, Slot::Type::Deck, slotSize, true);
    slots.emplace_back(sf::Vector2f(leftX, p2SpellY), graveyardTexture, Slot::Type::Graveyard, slotSize, true);
}
