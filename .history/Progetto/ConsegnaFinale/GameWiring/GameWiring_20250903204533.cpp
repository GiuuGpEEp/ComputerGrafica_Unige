...
#include "../GameLogic/Game/Effects/cards/BluEyes/SpiritOfWhite/SpiritOfWhiteEffect.h"
...
void registerDefaultEffects(Game &game, TextureManager& tm){
    ...
    // Effetto on-summon del Saggio con gli Occhi Blu (usa il nome esatto presente nei dati JSON)
    game.registerEffectForCardName("Saggio con gli Occhi Blu", std::make_unique<SageWithEyesOfBlueEffect>());
    game.registerEffectForCardName("Fanciulla con gli Occhi di Blu", std::make_unique<MaidenWithEyesOfBlueEffect>());
    game.registerEffectForCardName("Drago Spirito del Bianco", std::make_unique<SpiritOfWhiteEffect>());
}
...
void setupAppHandlers(
    ...
    if(game){
        ...
        game->setDiscardCallback([&](std::vector<Card>&& excess){
            ...
            // Determina il proprietario (indice giocatore assoluto) la cui mano ha prodotto l'eccesso (0 = P1, 1 = P2)
            int ownerIdx = game->getTurn().getCurrentPlayerIndex();
            // Field::getSlotPosition si aspetta il parametro player = 1 per il basso (P1) e 2 per l'alto (P2).
            int fieldPlayer = ownerIdx + 1; // convert 0->1, 1->2
            // Calcola la posizione dello slot Cimitero per quel proprietario
            ...
            // Avvia l'animazione di scarto verso il Cimitero del proprietario specifico
