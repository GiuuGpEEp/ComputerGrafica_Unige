...
bool DeckSelectionScreen::isFadeOutFinished() const {
    return fadeMode == FadeMode::None && fadeOutCompleted;
}

bool DeckSelectionScreen::isFading() const {
    return fadeMode != FadeMode::None;
}

float DeckSelectionScreen::getFadeAlpha() const {
    return fadeAlpha;
}