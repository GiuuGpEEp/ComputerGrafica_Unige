...
// RNG a livello di file per le decisioni dell'AI (dichiarato prima che gli handler lo usino)
...
void AppHandlers::attachGameHandlers(Game &g, const Context &ctx){
    ...
    // Preferisci leggere l'owner della pendingSS dal Game per evitare di affidarti all'indice del turno corrente
    ...
    if(owner == 1){
        // AI: sceglie autonomamente tra Attacco (false) e Difesa (true).
    ...
    // (handler precedente per OpponentSpellTrapChoiceRequested rimosso; si usa quello consolidato qui sotto)
