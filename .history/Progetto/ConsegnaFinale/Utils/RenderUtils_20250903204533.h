...
// =====================
// Helper di sincronizzazione delle zone (spostati da main.cpp)
// =====================
...
inline void drawDeckSendOverlay(
    ...
    float scale = 0.7f,
    float pad = 12.f,
    const std::string& titleOverride = std::string(),
    const std::string& hintOverride = std::string(),
    const std::unordered_map<AppConfig::TextureKey, sf::Texture*>* texMapOpt = nullptr,
    bool forceBackTextures = false
) {
    ...
    panel.setOutlineThickness(3.f);
    window.draw(panel);
    // Titolo: usa override se presente (per es. Melodia: "... da aggiungere alla mano"), altrimenti default (Reliquiario: "... da mandare al Cimitero")
    const std::string defaultTitle = "Scegli un Mostro Drago dal Deck da mandare al Cimitero";
    sf::Text title(font, titleOverride.empty() ? defaultTitle : titleOverride, 30);
    title.setFillColor(sf::Color(255,230,140));
    title.setOutlineColor(sf::Color(0,0,0,200));
    title.setOutlineThickness(2.f);
    sf::FloatRect tB = title.getLocalBounds();
    ...
    for(size_t i=0;i<candidates.size();++i){
        ...
        if(forceBackTextures && texMapOpt){
            RenderUtils::applyBackTexture(copy, *texMapOpt);
        } else {
            RenderUtils::applyFrontTextureIfAny(copy, textureManager);
        }
    ...
    // Suggerimento tasti
    const std::string defaultHint = "Invio = Conferma, ESC = Annulla, Frecce o Click per selezionare";
    sf::Text hint(font, hintOverride.empty() ? defaultHint : hintOverride, 22);
...
inline void drawPlayerHand(
    ...
    for (size_t i = 0; i < hand.size(); ++i) {
        ...
        // Le carte in mano del giocatore vanno mostrate scoperte: applica texture frontale se disponibile
        // Dopo lo swap di texture, riapplica la size originale per evitare discrepanze dovute a dimensioni texture diverse
        sf::FloatRect srcBounds = src.getGlobalBounds();
        if(textureManager){
            RenderUtils::applyFrontTextureIfAny(c, *textureManager);
            c.setSize(sf::Vector2f(srcBounds.size));
        }
