...
- Policy generale AI (P2) no‑prompt sulle scelte
	- Cosa cambia: P2 auto‑risolve senza aprire overlay in tutti i flussi di selezione supportati: sacrificio mostro proprio (Saggio), scelta BEWD/Spirito, scelta posizione per SS, scarto mano/aggiunta dal Deck, scelta dal Cimitero, e selezione M/T avversaria (Spirito del Bianco).
	- Dove: `Progetto/Tappa_16/AppHandlers/AppHandlers.cpp` negli handler dei rispettivi eventi; ramificazione owner==1 (AI) → auto‑pick.
	- Perché: UX coerente con "P2 è AI" e flusso scorrevole senza finestre modali.

- Gating di fase per l’attivazione dalla mano del Saggio
	- Cosa cambia: l’effetto dalla mano del Saggio è attivabile solo in Main1/Main2; il prompt “A per Attivare” compare solo in tali fasi.
	- Dove: `Game::canActivateMonsterFromHand(...)` in `Game.cpp` (controllo fase); `main.cpp` per la logica di hint in HUD.
	- Perché: corregge un bug che permetteva l’attivazione in Draw Phase e rende l’UX aderente alle regole temporali.

- Drago Spirito del Bianco — effetto on‑summon: bandisci 1 Magia/Trappola avversaria (incl. Magia Terreno)
	- Cosa cambia: quando “Drago Spirito del Bianco” viene Evocato Normalmente o Specialmente, si apre la scelta di 1 M/T controllata dall’avversario da bandire; se esiste una Magia Terreno avversaria ed è abilitata l’inclusione, può essere bersagliata.
	- Titolo overlay: “Scegli la carta da bandire”.
	- UX texture: durante questo specifico prompt le carte mostrate usano la texture coperta (retro). Gli altri prompt mantengono il comportamento attuale (mostra fronte dove previsto).
	- Fix trigger: l’effetto si attiva solo quando è evocato lo Spirito stesso, non ad ogni evocazione di un altro mostro presente mentre lo Spirito è sul campo. Implementato con un nuovo tracciamento `Game::LastSummonedMonster` usato dall’effetto.
	- Dove:
		- `GameLogic/Game/Effects/cards/BluEyes/SpiritOfWhite/SpiritOfWhiteEffect.{h,cpp}`: trigger su `NormalSummon`/`MonsterSpecialSummoned` filtrato con `getLastSummonedMonster()`; richiesta selezione M/T avversaria includendo Magia Terreno.
		- `Progetto/Tappa_16/main.cpp`: titolo overlay e policy di texture “retro” per questo prompt.
		- `Progetto/Tappa_16/AppHandlers/AppHandlers.cpp`: gestione evento di selezione M/T avversaria; P2 auto‑pick, P1 overlay.
	- Stato/Evento/API nuove in `Game` (vedi sezioni dedicate sotto): `OpponentSpellTrapChoiceRequested` e API unificate per la scelta M/T avversaria.

- Migliorie visuali mano avversaria
	- Cosa cambia: la mano dell’avversario è stata spostata più in alto e scalata più piccola per migliorare leggibilità del campo.
	- Dove: `Progetto/Tappa_16/Utils/Config.h` (es. `AppConfig::Ui::OpponentHandTopMarginPx`, `OpponentHandScaleFactor`) e `Utils/RenderUtils.h`.
	- Valori: TopMargin ≈ 14 px; scala ≈ 0.85.
