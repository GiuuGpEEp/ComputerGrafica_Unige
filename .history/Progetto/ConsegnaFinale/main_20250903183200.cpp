...
int main(){
    ...
    auto closeResponsePrompt = [&](){
        // Interpret closing as passing on chain for the local player, if a chain is active
        if(game && game->isChainActive()){
            // local player is P1 (index 0)
            game->playerPassOnChain(0);
            // Auto-pass the AI/opponent to allow resolution (simplified chain priority)
    ...
    // Cancel active drag: restore dragged card to hand and reset drag state
    ...
    while (window.isOpen()) {
        ...
        if(gamestate == GameState::HomeScreen){
            // Safety net: Home should never be blocked; if it is, clear modals
            if(inputBlocked()){
                selectingTributes = false;
                selectedTributes.clear();
                tributesNeeded = 0;
            ...
            // NOTE: se un overlay (Extra/Graveyard) è visibile, non mostrare contemporaneamente i dettagli della selezione di campo
