...
bool Game::canActivateMonsterFromHand(size_t handIndex){
    ...
    // Consentito solo durante le Main Phases (effetto a Velocità 1)
    if(turn.getPhase() != GamePhase::Main1 && turn.getPhase() != GamePhase::Main2) return false;
...
// (Vecchia versione della selezione M/T avversaria rimossa in favore dell'API unificata)
...
// --- Helper per l'interazione con la chain ---
void Game::startChainWithActivation(const std::string& cardName, int ownerIdx){
    // Avvia una finestra di risposta: inserisci l'attivazione iniziale e marca la chain come attiva
    effects.pushActivationRequest(cardName, ownerIdx);
    chainActive = true;
    lastResponder = -1;
    // Notifica alla UI tramite evento che è iniziata una chain/finestra di risposta
...
bool Game::playerRespondWithActivation(int playerIdx, const std::string& cardName){
    ...
    // Verifica che il giocatore possa attivare ora la carta (mano o Set Quick/Trap). Il chiamante deve garantire il mapping corretto del nome carta.
    IActivationEffect* act = effects.getActivationFor(cardName);
    if(!act) return false;
    if(!act->canActivate(*this, playerIdx)) return false;
    // Inserisci la risposta in cima alla chain
    effects.pushActivationRequest(cardName, playerIdx);
    lastResponder = playerIdx;
    // Segnala alla UI tramite evento
    dispatcher.emit(GameEventType::SpellActivated);
    return true;
}
