...
public:
    ...
    // Restituisce l'indice del proprietario della SS in sospeso, se presente
    ...
    // Restituisce l'indice del proprietario per una richiesta di invio dal Deck, se presente
    ...
    // Selezione M/T dell'avversario (inclusa Magia Terreno) per effetti tipo Drago Spirito del Bianco
    // Flusso: request -> UI mostra la lista M/T avversarie (ST zone + Field Spell) -> resolve con indice flatten
    // (vedi API unificata più in basso)
    ...
    // Ultimo mostro Evocato (Normal o Special): nome e owner assoluto (0 = P1 basso, 1 = P2 alto)
    struct LastSummonedMonster { std::string name; int ownerIdx; };
    std::optional<LastSummonedMonster> getLastSummonedMonster() const { return lastSummonedMonster; }
    void clearLastSummonedMonster() { lastSummonedMonster.reset(); }
    ...
    // --- Selezione M/T avversaria (inclusa eventuale Magia Terreno) per effetti come Drago Spirito del Bianco ---
    // Richiede al giocatore ownerIdx di scegliere una carta tra le M/T controllate dall'avversario; opzionalmente include la Magia Terreno
    bool requestSelectOpponentSpellTrap(int ownerIdx, bool includeFieldSpell = true);
    bool hasPendingOpponentSpellTrapChoice() const { return pendingOppST.has_value(); }
    std::optional<int> getPendingOpponentSpellTrapOwner() const;
    // indexInList: 0..(N-1) enumerando prima la zona M/T avversaria da sinistra a destra, poi (se presente e includeFieldSpell) la Magia Terreno come ultimo elemento
    bool resolvePendingSelectOpponentSpellTrap(size_t indexInList);
    void cancelPendingSelectOpponentSpellTrap();
    ...
    // Tracciamento minimale per trigger "mostro Evocato" (Normal/Special)
    std::optional<LastSummonedMonster> lastSummonedMonster;
    ...
    // Stato pending per scelta M/T dell'avversario (inclusa Magia Terreno) – definito più in basso
    ...
    // Stato pending per selezione M/T avversaria
    struct PendingOpponentST { int ownerIdx; bool includeField; };
    std::optional<PendingOpponentST> pendingOppST;
