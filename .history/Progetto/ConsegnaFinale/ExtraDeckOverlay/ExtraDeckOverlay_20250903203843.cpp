    ...
    if(state == GameState::FieldVisible || state == GameState::Playing) {
        if(extraSlotBounds.contains(mousePos)) {
            ...
            if(!showOverlay) {
                ...
                // Overlay già aperto: interpreta il nuovo click come semplice interazione,
...
void ExtraDeckOverlay::handleMousePress(const sf::Vector2f& mousePos, const std::vector<Card>& extraCards){
    ...
    // Dentro l'overlay: verifica la selezione della carta
...
bool ExtraDeckOverlay::isOverlayVisible() const { 
    return showOverlay; 
}

bool ExtraDeckOverlay::detailsActive() const { 
    return detailsShown; 
}

float ExtraDeckOverlay::getScrollOffset() const { 
    return scrollOffset; 
}

void ExtraDeckOverlay::setScrollOffset(float v) { 
    scrollOffset = v; 
}

const Card* ExtraDeckOverlay::getSelectedCard() const { 
