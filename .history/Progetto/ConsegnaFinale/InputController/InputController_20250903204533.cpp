    ...
    if(kp3->code == M_KEY_A && state.gamestate == GameState::Playing && !state.inputBlocked()){
        ...
        if(const auto* kp4 = e.getIf<sf::Event::KeyPressed>()){
            ...
            if(state.chooseSummonOrSet && game && state.pendingHandIndexForPlay.has_value() && !state.inputBlocked()){
                // NEW: Attivazione effetto mostro dalla mano (A) per Saggio con Occhi di Blu / Saggio con gli Occhi Blu
                if(kp4->code == M_KEY_A){
                    size_t handIdx = state.pendingHandIndexForPlay.value();
                    bool ok = false;
                    // Delego alla logica di gioco la validazione dell'attivazione dalla mano
                    if(game->canActivateMonsterFromHand(handIdx)){
                        ok = game->activateMonsterFromHand(handIdx);
                    }
                    if(ok){
                        // Rimuove Saggio dalla mano e apre la richiesta di selezione del proprio mostro (evento gestito da AppHandlers)
                        if(state.updateHandLayout) state.updateHandLayout();
                        state.battleFeedbackMsg = "Effetto attivato: seleziona un tuo mostro"; state.battleFeedbackClock.restart();
                    } else {
                        state.battleFeedbackMsg = "Effetto non attivabile"; state.battleFeedbackClock.restart();
        ...
        if(kpEsc->code == M_KEY_ESCAPE){
            ...
            if(state.gamestate != GameState::DeckSelection){
                ...
                if(state.selectingTributes){
                    ...
                    state.selectingTributes = false; state.selectedTributes.clear(); state.tributesNeeded = 0;
                    // ripristina eventuale drag attivo
                    if(state.cancelActiveDrag) state.cancelActiveDrag();
    ...
    if(!state.inputBlocked() && (e.is<sf::Event::MouseButtonPressed>() || e.is<sf::Event::MouseButtonReleased>())){
        ...
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
            if(mbp->button == M_MOUSE_LEFT){
                ...
                // Avvia l'hold con i bounds corretti dello slot (passando top/bottom di conseguenza)
                ...
                // Per il click sull'overlay del Cimitero dobbiamo passare le carte del Cimitero del proprietario corretto
                std::vector<Card> gyCards;
                // Se l'overlay è già visibile, usa il currentOwner dell'overlay (impostato a startHold/open)
