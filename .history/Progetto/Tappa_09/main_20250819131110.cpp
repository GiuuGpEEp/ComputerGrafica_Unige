            ...
            if(returnPopupActive){
                ...
                if(const auto* mb = event->getIf<sf::Event::MouseButtonPressed>()){
                    if(mb->button == sf::Mouse::Button::Left){
                        ...
                        float btnY = popup.top + popup.height - btnSize.y - 40.f;
                        float gap = 80.f;
                        float totalBtnWidth = btnSize.x*2 + gap;
                        float startX = center.x - totalBtnWidth/2.f;
                        sf::FloatRect yesRect(startX, btnY, btnSize.x, btnSize.y);
                        sf::FloatRect noRect(startX + btnSize.x + gap, btnY, btnSize.x, btnSize.y);
