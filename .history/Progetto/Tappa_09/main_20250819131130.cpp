            ...
            if(returnPopupActive){
                ...
                if(const auto* mb = event->getIf<sf::Event::MouseButtonPressed>()){
                    if(mb->button == sf::Mouse::Button::Left){
                        ...
                        sf::FloatRect popup(center.x - popupSize.x/2.f, center.y - popupSize.y/2.f, popupSize.x, popupSize.y);
                        sf::Vector2f btnSize(160.f,60.f);
                        float btnY = popup.position.y + popup.size.y - btnSize.y - 40.f;
                        float gap = 80.f;
                        float totalBtnWidth = btnSize.x*2 + gap;
                        float startX = center.x - totalBtnWidth/2.f;
                        sf::FloatRect yesRect(sf::Vector2f(startX, btnY), sf::Vector2f(btnSize.x, btnSize.y));
                        sf::FloatRect noRect(sf::Vector2f(startX + btnSize.x + gap, btnY), sf::Vector2f(btnSize.x, btnSize.y));
    ...
    // (Ritorno alla Home ora gestito automaticamente al termine del fade out della DeckSelection)
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(gamestate == GameState::FieldVisible || gamestate == GameState::Playing ) {
                bool enableHover = !extraOverlay.isOverlayVisible();
        ...
        if(returnPopupActive){
            ...
            sf::Text hint(detailFont, "Y = Si   N / Esc = No", 28);
            hint.setFillColor(sf::Color(220,220,220));
            sf::FloatRect hb = hint.getLocalBounds();
            hint.setOrigin(sf::Vector2f(hb.size.x/2.f, hb.size.y/2.f));
            hint.setPosition(sf::Vector2f(center.x, popupPos.y + popupSize.y - 80.f));
            window.draw(hint);
            ...
            float totalBtnWidth = btnSize.x*2 + gap;
            float startX = center.x - totalBtnWidth/2.f;
            float btnY = popupPos.y + popupSize.y - btnSize.y - 40.f;
            sf::RectangleShape yesBtn(btnSize); yesBtn.setPosition(sf::Vector2f(startX, btnY));
            sf::RectangleShape noBtn(btnSize); noBtn.setPosition(sf::Vector2f(startX + btnSize.x + gap, btnY));
            yesBtn.setFillColor(sf::Color(60,160,60)); noBtn.setFillColor(sf::Color(170,60,60));
            window.draw(yesBtn); window.draw(noBtn);
            sf::Text yesT(detailFont, "SI", 30); sf::FloatRect yb = yesT.getLocalBounds(); yesT.setOrigin(sf::Vector2f(yb.size.x/2.f, yb.size.y/2.f)); yesT.setPosition(yesBtn.getPosition()+sf::Vector2f(btnSize.x/2.f, btnSize.y/2.f)); window.draw(yesT);
            sf::Text noT(detailFont, "NO", 30); sf::FloatRect nb = noT.getLocalBounds(); noT.setOrigin(sf::Vector2f(nb.size.x/2.f, nb.size.y/2.f)); noT.setPosition(noBtn.getPosition()+sf::Vector2f(btnSize.x/2.f, btnSize.y/2.f)); window.draw(noT);
        }

        window.display();

    // Disegna popup (sopra tutto dopo display precedente? Spostiamo prima di display finale) -- già sopra
    }
    return 0;
}