...
int main(){
    ...
    // Preview Extra Deck (hold)
    bool holdingExtraDeck = false;            // true mentre tengo premuto sullo stack extra (fase di attivazione)
    bool showExtraPreview = false;            // overlay attualmente visibile
    sf::Clock extraHoldClock;                 // misura durata hold
    const float EXTRA_HOLD_THRESHOLD = 0.45f; // secondi da tenere premuto
    sf::Vector2f extraDeckPreviewOrigin;      // posizione calcolata dell'overlay
    sf::FloatRect extraPreviewBounds;         // bounds attuali dell'overlay per gestione click fuori
    std::vector<sf::FloatRect> extraPreviewCardBounds; // bounds di ciascuna carta in overlay
    bool extraPreviewCardDetailsActive = false;        // mostra dettagli carta extra deck
    Card extraPreviewCardSelected;                     // copia della carta selezionata nell'overlay
    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    // Inizia possibile hold su extra deck (solo in Field/Playing)
                    if((gamestate == GameState::FieldVisible || gamestate == GameState::Playing)){
                        sf::FloatRect extraRect(extraDeckSlotPos, slotSize);
                        if(extraRect.contains(static_cast<sf::Vector2f>(mousePos))){
                            holdingExtraDeck = true;
                            showExtraPreview = false; // attendo soglia
                            extraHoldClock.restart();
                        }
                    }
            ...
            if(mousePressed && !isDragging) {
                ...
                // Se overlay extra deck aperto e clicco (press) fuori dai suoi bounds, pianifica chiusura al release
                if(showExtraPreview){
                    // Definito extraPreviewBounds quando lo disegniamo (aggiornato ogni frame)
                    if(!extraPreviewBounds.contains(static_cast<sf::Vector2f>(mousePos))){
                        // Click fuori: chiuderemo all'immediato rilascio se non inizia un drag
                        // Per UX reattiva chiudiamo subito
                        showExtraPreview = false;
                        extraPreviewCardDetailsActive = false;
                    } else {
                        // Click dentro overlay: controlla se cliccata una carta
                        for(size_t i=0; i<extraPreviewCardBounds.size(); ++i){
                            if(extraPreviewCardBounds[i].contains(static_cast<sf::Vector2f>(mousePos))){
                                const auto& extraVec = deck.getExtraCards();
                                if(i < extraVec.size()){
                                    extraPreviewCardSelected = extraVec[i];
                                    // Assicura texture fronte
                                    if(!extraPreviewCardSelected.originalTexturePath.empty()){
                                        sf::Texture& frontTex = textureManager.getTexture(extraPreviewCardSelected.originalTexturePath);
                                        extraPreviewCardSelected.setTexture(frontTex);
                                        sf::Vector2u ts = frontTex.getSize();
                                        extraPreviewCardSelected.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i(ts.x, ts.y)));
                                    }
                                    extraPreviewCardDetailsActive = true;
                                    scrollOffset = 0.f; // reset scroll
                                    cardClicked = true; // evita altre selezioni
                                }
                                break;
                            }
                        }
        ...
        if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonReleased>()) {
            ...
            if(holdingExtraDeck){ holdingExtraDeck = false; }
        ...
        if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
            ...
            if (const auto* mouseScroll = event->getIf<sf::Event::MouseWheelScrolled>()) {
                ...
                if (selectedCardIndex.has_value() || showDrawnCardDetails || extraPreviewCardDetailsActive) {
    ...
    if(gamestate == GameState::Playing){
        ...
        if(holdingExtraDeck && !isDragging && (gamestate == GameState::FieldVisible || gamestate == GameState::Playing)){
            if(extraHoldClock.getElapsedTime().asSeconds() >= EXTRA_HOLD_THRESHOLD){
                showExtraPreview = true;
            }
        }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(showExtraPreview && deck.getExtraSize() > 0){
                const auto& extraVec = deck.getExtraCards();
                // Layout: fila/e centrata/e sopra la mano del giocatore
                int total = static_cast<int>(extraVec.size());
                int cols = std::min(8, total); // massimo 8 per riga
                int rows = (total + cols - 1)/cols;
                float previewScale = 0.95f; // leggermente più grande
                sf::Vector2f previewCardSize(slotSize.x * previewScale, slotSize.y * previewScale);
                float hSpacing = 18.f;      // maggiore spaziatura orizzontale
                float vSpacing = 28.f;      // maggiore spaziatura verticale e verso il basso
                float gridWidth = cols * previewCardSize.x + (cols -1)*hSpacing;
                float gridHeight = rows * previewCardSize.y + (rows -1)*vSpacing;
                sf::Vector2f origin((windowSize.x - gridWidth)/2.f, windowSize.y * 0.60f - gridHeight); // leggermente più alto per più spazio basso

                // Backdrop semitrasparente (aumento padding inferiore per più spazio sotto le carte)
                float extraBottomPadding = 90.f; // spazio aggiuntivo sotto le carte
                sf::RectangleShape backdrop;
                backdrop.setPosition(origin - sf::Vector2f(20.f, 30.f));
                backdrop.setSize(sf::Vector2f(gridWidth + 40.f, gridHeight + 60.f + extraBottomPadding));
                ...
                // Aggiorna bounds overlay per gestione click fuori
                extraPreviewBounds = backdrop.getGlobalBounds();

                // Titolo
                sf::Text title(detailFont, "EXTRA DECK", 24);
                title.setFillColor(sf::Color::White);
                sf::FloatRect tB = title.getLocalBounds();
                title.setPosition(sf::Vector2f(origin.x + (gridWidth - tB.size.x)/2.f, backdrop.getPosition().y + 6.f));
                window.draw(title);

                // Disegno temporaneo delle carte (uso sprite corrente della Card)
                extraPreviewCardBounds.clear();
                extraPreviewCardBounds.reserve(total);
                for(int i=0; i<total; ++i){
                    int r = i / cols;
                    int c = i % cols;
                    sf::Vector2f pos = origin + sf::Vector2f(c*(previewCardSize.x + hSpacing), r*(previewCardSize.y + vSpacing) + 30.f);
                    // Copia leggera per non modificare card originale (solo posizione/size)
                    Card previewCard = extraVec[i];
                    // Se la carta al momento ha la texture flipped sul deck, ricarica texture originale dal path salvato
                    if(!previewCard.originalTexturePath.empty()){
                        // Carica/ottieni la texture corretta (usa textureManager esistente)
                    ...
                    extraPreviewCardBounds.emplace_back(pos, previewCardSize);
                }

                // Istruzione per chiudere
                sf::Text hint(detailFont, extraPreviewCardDetailsActive ? "ESC per chiudere i dettagli, click fuori per chiudere" : "Click carta per dettagli, fuori per chiudere", 16);
            ...
            // Dettagli carta selezionata dall'extra overlay
            if(extraPreviewCardDetailsActive){
                sf::Vector2f panelPos{ windowSize.x * 0.07f, windowSize.y * 0.12f };
                sf::Vector2f panelSize{ 340.f, 260.f };
                showCardDetails(window, extraPreviewCardSelected, detailFont, panelPos, panelSize, scrollOffset);
            }
        }
