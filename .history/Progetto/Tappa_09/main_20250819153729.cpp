...
int main(){
    ...
    // (Ritorno alla Home ora gestito automaticamente al termine del fade out della DeckSelection)
        ...
        if(returnPopupActive){
            ...
            sf::Text yesT(detailFont, "SI", 30); 
            sf::FloatRect yb = yesT.getLocalBounds(); yesT.setOrigin(sf::Vector2f(yb.size.x/2.f, yb.size.y/2.f)); yesT.setPosition(yesBtn.getPosition()+sf::Vector2f(btnSize.x/2.f, btnSize.y/2.f)); window.draw(yesT);
