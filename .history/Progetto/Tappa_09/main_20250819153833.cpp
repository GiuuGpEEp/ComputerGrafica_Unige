...
int main(){
    ...
    // (Ritorno alla Home ora gestito automaticamente al termine del fade out della DeckSelection)
        ...
        if(returnPopupActive){
            ...
            float totalBtnWidth = btnSize.x*2 + gap;
            float startX = center.x - totalBtnWidth/2.f;
            float buttonY = popupPos.y + popupSize.y - btnSize.y - 40.f;
            sf::RectangleShape yesBtn(btnSize); 
            yesBtn.setPosition(sf::Vector2f(startX, buttonY));
            sf::RectangleShape noBtn(btnSize); 
            noBtn.setPosition(sf::Vector2f(startX + btnSize.x + gap, buttonY));
