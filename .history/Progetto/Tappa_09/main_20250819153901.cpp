...
int main(){
    ...
    // (Ritorno alla Home ora gestito automaticamente al termine del fade out della DeckSelection)
        ...
        if(returnPopupActive){
            ...
            yesButton.setFillColor(sf::Color(60,160,60)); 
            noBtn.setFillColor(sf::Color(170,60,60));
            window.draw(yesButton); window.draw(noBtn);
            sf::Text yesT(detailFont, "SI", 30); 
            sf::FloatRect yb = yesT.getLocalBounds(); 
            yesT.setOrigin(sf::Vector2f(yb.size.x/2.f, yb.size.y/2.f)); 
            yesT.setPosition(yesButton.getPosition()+sf::Vector2f(btnSize.x/2.f, btnSize.y/2.f)); 
            window.draw(yesT);
