...
int main(){
    ...
    // (Ritorno alla Home ora gestito automaticamente al termine del fade out della DeckSelection)
        ...
        if(returnPopupActive){
            ...
            window.draw(yesT);
            sf::Text noT(detailFont, "NO", 30); 
            sf::FloatRect nb = noT.getLocalBounds(); noT.setOrigin(sf::Vector2f(nb.size.x/2.f, nb.size.y/2.f)); noT.setPosition(noBtn.getPosition()+sf::Vector2f(btnSize.x/2.f, btnSize.y/2.f)); window.draw(noT);
        }
