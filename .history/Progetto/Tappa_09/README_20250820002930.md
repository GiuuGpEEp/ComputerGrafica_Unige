...
**Altra aggiunta** --> alla pressione di un certo tasto (probabilmente space), se stiamo giocando si apre una sorta di menù / pop up che ci chiede se si vuole tornare alla schermata di home con risposta si / no.
Questa implementazione è svolta interamente nel main
Quindi l'unica variabile aggiunta è il booleano, che viene messo a true. Se è a true e si verifica qualche evento ci si concentra unicamente sui pop up e non sul campo da gioco. Inoltre nella parte di rendering ho aggiunto l'effettivo disegno del pop up, sempre se il booleano è impostato a true. Inoltre dopo essere tornati alla home bisogna reimpostare a 0 stato del deck e delle animazioni, resettando il tutto.
Ciò viene fatto dalla lambda expression ---> resetMatch
Anche le animazioni di fadeIn del deck e del field vengono ripristinate grazie a dei nuovi metodi appositi creati all'interno delle classi --> resetAnimation

Il reset era necessario perchè prima restavano i vecchi valori e appena si cliccava su gioca le animazioni e tot risultavano già terminate di conseguenza si passava diretti allo stato di playing.
Inoltre quando il pop up è aperto tutte le animazioni vengono messe in pausa (azzerando il deltaTime), e anche il mouseHover viene disabilitato mettendo a false la variabile enebleHover
