    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                ...
                // ESC dalla schermata di selezione deck torna alla Home
                if(keyPressed->code == sf::Keyboard::Key::Escape && gamestate == GameState::DeckSelection) {
                    std::cout << "ESC -> Ritorno alla Home dalla DeckSelection" << std::endl;
                    gamestate = GameState::HomeScreen;
                    pendingHomeReturn = false; // annulla eventuale ritorno ritardato
                }
            }
            
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                sf::Mouse::Button but = mouseButton->button;
                // Tasto destro: se siamo nella schermata di selezione deck, torna alla Home
                if(but == sf::Mouse::Button::Right && gamestate == GameState::DeckSelection) {
                    std::cout << "Right click -> Ritorno alla Home dalla DeckSelection" << std::endl;
                    gamestate = GameState::HomeScreen;
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                    if (keyPressed->code == sf::Keyboard::Key::Escape) {
                        // Se non siamo in DeckSelection (già gestito sopra) gestiamo la logica di deselezione carte
                        if(gamestate != GameState::DeckSelection) {
                            selectedCardIndex.reset();
