...
- Il progetto usa CMake e SFML. È disponibile un target per questa tappa: `mainTappa14`.
- Costruzione consigliata: generare/aggiornare la build directory e compilare il target `mainTappa14` in configurazione desiderata (ad esempio Release).
- Le risorse sono risolte relativamente a `../../Progetto/resources/` rispetto al binario di tappa.

## Prossimi passi suggeriti

- Migliorare l’handling del caso "chiave texture mancante": oggi c’è un fallback a una texture “dummy”. Valutare assert/log più espliciti.
- Introdurre un piccolo helper locale nel `main` (es. `getTex(TextureKey)`) per ridurre la verbosità delle chiamate.
- In una fase successiva, far accettare a costruttori/metodi pubblici direttamente `TextureKey` o un piccolo “view”, risolvendo la texture internamente.
