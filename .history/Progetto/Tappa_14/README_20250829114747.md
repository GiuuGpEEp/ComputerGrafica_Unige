...
Nota: queste modifiche sono mirate alla manutenzione e alla pulizia del codice; non cambiano la logica di gioco o il comportamento UX previsto.

### Come testare rapidamente (Windows PowerShell)

1) Ricompilare il target (Release):

```powershell
cmake --build c:\Users\GiuuG\Documents\Università\TerzoAnno\SecondoSemestre\FCG\Progetto\build --target mainTappa14 --config Release
```

2) Avviare l'eseguibile prodotto per un rapido smoke-test (apre la finestra del gioco):

```powershell
.\build\bin\mainTappa14.exe
```

3) Smoke test consigliato:
	- Vai su Home → Gioca.
	- Apri Deck Selection e scegli un deck normale, conferma.
	- Esegui una pescata, osserva che la carta finale abbia la texture corretta (non rimanga il retro in modo casuale).
	- Premi `E` durante il gioco per verificare che non ci siano crash improvvisi.
	- Prova ad aprire/chiudere overlay (ExtraDeck, Graveyard) e conferma che il comportamento non è cambiato.

### Se trovi problemi

- Se il progetto non compila, controlla gli include e i file sorgente aggiunti recentemente: `Progetto/Tappa_14/GameWiring.cpp` e `Progetto/Tappa_14/GameWiring.h`.
- Se riscontri crash runtime durante input (es. `E`), esegui il gioco dal terminale per vedere possibili messaggi di log su stdout/stderr.

Per qualsiasi correzione ulteriore o per standardizzare ulteriormente il wiring (es. spostare `AppHandlers` in una libreria comune), posso proseguire con altri refactor mirati.
