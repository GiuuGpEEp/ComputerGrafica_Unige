                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            openingCurrentPlayer = -1;
                                ...
                                ctx.syncZones = [&](){ syncZones(); };
                                ctx.syncMonsterZoneToField = [&](){ syncMonsterZoneToField(); };
                                ctx.syncSpellTrapZones = [&](){ syncSpellTrapZones(); };
                                ctx.updateHandLayout = [&](){ if(handPtr) updateHandLayout(); };
                                ctx.openResponsePromptIfAny = [&](){ openResponsePromptIfAny(); };
                                ctx.rebuildResponseActivables = [&](){ rebuildResponseActivables(); };
                                ctx.closeResponsePrompt = [&](){ closeResponsePrompt(); };
                                ctx.stopAllInputActions = [&](){ isDragging = false; isPotentialDrag = false; draggingCardIndex.reset(); potentialDragCardIndex.reset(); };
                                ctx.resetDeckPositions = [&](){ deck.resetDeckCardPositions(deckSlotPos, deckCardSize, slotSize, AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack)); opponentDeck.resetDeckCardPositions(deckSlotPosTop, deckCardSize, slotSize, AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack)); };
                                ctx.setSelectingTributes = [&](bool v){ selectingTributes = v; };
                                ctx.setTributesNeeded = [&](int v){ tributesNeeded = v; };
                                ctx.setSsChoiceActive = [&](bool v){ ssChoiceActive = v; };
                                ctx.setDeckSendChoiceActive = [&](bool v){ deckSendChoiceActive = v; };
                                ctx.setDeckSendCandidates = [&](const std::vector<Card>& c){ deckSendCandidates = c; };
                                ctx.setDeckSendOwner = [&](int o){ deckSendOwner = o; };
                                ctx.pushBattleFeedbackMsg = [&](const std::string &s){ battleFeedbackMsg = s; battleFeedbackClock.restart(); };
                                ctx.drawStartSubscriptionId = &drawStartSubscriptionId;
                                ctx.drawEndSubscriptionId = &drawEndSubscriptionId;
                                ctx.attackDeclaredSubId = &attackDeclaredSubId;
                                ctx.attackResolvedSubId = &attackResolvedSubId;
                                ctx.monsterDestroyedSubId = &monsterDestroyedSubId;
                                ctx.lifeChangedSubId = &lifeChangedSubId;
                                ctx.directAttackSubId = &directAttackSubId;
                                AppHandlers::attachGameHandlers(*game, ctx);
