...
Per ridurre ulteriormente la duplicazione e mantenere il refactor “mini”, sono stati introdotti e adottati tre helper locali direttamente in `main.cpp` (non esposti in header):

- `drawSpellTrapRow(side)`
- `drawFieldSpell(side)`
- `drawGraveyardTop(side)`

Questi sostituiscono i blocchi duplicati di disegno per P1 e P2. Ora il codice chiama:

- `drawSpellTrapRow(0)` e `drawSpellTrapRow(1)` per le righe Magie/Trappole,
- `drawFieldSpell(0)` e `drawFieldSpell(1)` per le carte Terreno,
- `drawGraveyardTop(0)` e `drawGraveyardTop(1)` per mostrare la top del Cimitero.
...
Uniformate le spaziature di `if/for/while` nel `main.cpp` per migliorare leggibilità e coerenza del codice.
