...
namespace AppConfig {
    ...
    inline sf::Texture& findTextureByName(TextureManager& tm, const std::string& name){
        static const std::unordered_map<std::string, TextureKey> lut = {
            {"FieldBackground", TextureKey::FieldBackground},
            {"MonsterText", TextureKey::MonsterText},
            {"SpellTrap", TextureKey::SpellTrap},
            {"Deck", TextureKey::Deck},
            {"Graveyard", TextureKey::Graveyard},
            {"ExtraDeck", TextureKey::ExtraDeck},
            {"FieldSpell", TextureKey::FieldSpell},
            {"CardBack", TextureKey::CardBack},
            {"CardFrontNotSet", TextureKey::CardFrontNotSet},
            {"StartScreen", TextureKey::StartScreen},
            {"HomeScreen", TextureKey::HomeScreen},
            {"DeckSelectionScreen", TextureKey::DeckSelectionScreen}
        };
        auto it = lut.find(name);
        if(it != lut.end()){
            return getTexture(tm, it->second);
        }
        // fallback: assume caller passed a full path
        std::cerr << "[Config] Nome texture sconosciuto ('" << name << "'), tento come path assoluto/relativo..." << std::endl;
