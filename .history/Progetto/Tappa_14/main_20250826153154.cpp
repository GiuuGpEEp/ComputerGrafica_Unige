...
// Helpers locali a questa TU per snellire il main senza cambiare logica
namespace {
    sf::Texture& loadTexture(TextureManager& tm, const std::string& path){
        sf::Texture& tex = tm.getTexture(path);
        std::cout << "Caricata: " << path << std::endl;
        return tex;
    }
...
// Path base per tutte le risorse (centralizzati in Config.hpp)
const std::string  deck1Path    = AppConfig::texturePath + "textureDeck1/";


// Piccolo contenitore per lo stato UI/input (non logico) per ridurre variabili top-level in main
struct UiState {
    bool mousePressed = false;
    bool selectingTributes = false;
    bool chooseSummonOrSet = false;
    bool chooseActivateOrSetST = false;
    std::optional<size_t> pendingHandIndexForPlay;
    bool pendingIsSpellTrap = false;
    std::vector<size_t> selectedTributes;
    int tributesNeeded = 0;
    bool showDrawnCardDetails = false;
    bool openingAltDrawActive = false;
    int openingNextPlayer = 0;
    bool openingDrawInFlight = false;
    int openingCurrentPlayer = -1;
    ...
    int responseOwner = -1;
    std::vector<size_t> responseActivables;
...
int main(){
    ...
    sf::Texture& fieldTexture = loadTexture(textureManager, AppConfig::texturePath + "backgroundTexture.png");
    sf::Texture& monsterTexture = loadTexture(textureManager, AppConfig::texturePath + "monsterText.png");
    sf::Texture& spellTrapTexture = loadTexture(textureManager, AppConfig::texturePath + "SpellTrapTexture.png");
    sf::Texture& deckTexture = loadTexture(textureManager, AppConfig::texturePath + "deckTexture.png");
    sf::Texture& graveyardTexture = loadTexture(textureManager, AppConfig::texturePath + "graveTexture.png");
    sf::Texture& extraDeckTexture = loadTexture(textureManager, AppConfig::texturePath + "ExtraDeckTexture.png");
    sf::Texture& fieldSpellTexture = loadTexture(textureManager, AppConfig::texturePath + "fieldSpell.png");
    sf::Texture& textureFlipped = loadTexture(textureManager, AppConfig::texturePath + "Texture1.png");
    sf::Texture& textureNonFlipped = loadTexture(textureManager, AppConfig::texturePath + "CardNotSet.jpg");
    sf::Texture& startScreenTexture = loadTexture(textureManager, AppConfig::texturePath + "startScreenTexture.png");
    sf::Texture& homeScreenTexture = loadTexture(textureManager, AppConfig::texturePath + "homeScreenTexture.png");
    sf::Texture& deckSelectionScreenTexture = loadTexture(textureManager, AppConfig::texturePath + "deckSelectionScreenTexture.png");

    // Carico il font per le etichette delle carte
    sf::Font detailFont;
    loadDetailFont(detailFont, AppConfig::fontPath);
    ...
    auto resetMatch = [&](){
        ...
        deck.resetAnimation();
        opponentDeck.resetDeckCardPositions(deckSlotPosTop, deckCardSize, slotSize, textureFlipped);
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(deckSendChoiceActive && but == sf::Mouse::Button::Left){
                    // Calcola griglia delle opzioni e aggiorna deckSendSelected se cliccato
                    size_t cols = 5; float pad = 12.f; float scale = 0.7f;
                    ...
                    sf::Vector2f mouseF(static_cast<float>(sf::Mouse::getPosition(window).x), static_cast<float>(sf::Mouse::getPosition(window).y));
                    for(size_t i=0;i<deckSendCandidates.size();++i){
                        size_t r = i / cols; size_t c = i % cols;
                        sf::Vector2f pos(gridPos.x + c*(cellSize.x+pad), gridPos.y + r*(cellSize.y+pad));
                        sf::FloatRect rect(pos, cellSize);
                        if(rect.contains(mouseF)){ deckSendSelected = i; break; }
                    }
                }
    ...
    if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                Deck &activeDeck = forceOpening ? (openingCurrentPlayer == 0 ? deck : opponentDeck)
                                                : ((cur == 0) ? deck : opponentDeck);
                sf::Vector2f activeDeckPos = forceOpening ? (openingCurrentPlayer == 0 ? deckSlotPos : deckSlotPosTop)
                                                          : ((cur == 0) ? deckSlotPos : deckSlotPosTop);
                std::vector<Card> &drawTargetHand = forceOpening ? game->getHandOf(openingCurrentPlayer)
                                                                  : game->current().getHand();
                float handYForAnim = (forceOpening ? (openingCurrentPlayer == 0 ? y : 30.f)
                                                    : ((cur == 0) ? y : 30.f));
                // Skip reveal when the drawing player is the opponent (P2)
                bool forceSkipReveal = forceOpening ? (openingCurrentPlayer == 1) : (cur == 1);
                drawController.update(deltaTime, mousePressed, forceSkipReveal, activeDeck, drawTargetHand, windowSize, cardSize, spacing, handYForAnim, HAND_MAXSIZE, activeDeckPos, textureFlipped, textureManager, &game->events());
        ...
        // Disegna le carte sul campo (P1)
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                if(isFD){
                    drawCard.setTexture(textureFlipped);
                    sf::Vector2u ts = textureFlipped.getSize();
                    drawCard.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
                }
                // Ruota orizzontalmente se in DIFESA o coperta
                if(isDef || isFD){
                ...
                if(!isDef && !isFD){
                    // P2 in piedi: carta capovolta (180°) e centrata nello slot
                    int slotIdx2 = game ? game->getMonsterSlotIndexAt(1, i) : static_cast<int>(i);
                    if(slotIdx2 < 0) slotIdx2 = static_cast<int>(i);
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P2, slotIdx2);
                    sf::Vector2f slotCenter(slotPos.x + slotSize.x/2.f, slotPos.y + slotSize.y/2.f);
                    drawCard.setSize(sf::Vector2f(slotSize.x * 0.75f, slotSize.y * 0.75f)); // allinea alla sync
                    drawCard.centerOrigin();
                    drawCard.setRotation(180.f);
                    drawCard.setPosition(slotCenter);
                }
                if(isDef){
                    auto mix = [](sf::Color a, sf::Color b){
            ...
            if(game && !game->getGraveyard().empty()){
                ...
                // Assicura la texture frontale
                if(!copy.originalTexturePath.empty()){
                    sf::Texture &front = textureManager.getTexture(copy.originalTexturePath);
                    copy.setTexture(front);
                    sf::Vector2u ts = front.getSize();
                    copy.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
                }
                sf::Vector2f gyPos = field.getSlotPosition(Type::Graveyard, P1);
                float scale = 0.75f;
            ...
            if(game){
                ...
                if(n > 0){
                    ...
                    for(size_t i=0;i<n;++i){
                        ...
                        copy.setTexture(textureFlipped);
                        sf::Vector2u ts = textureFlipped.getSize();
                        copy.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
                        copy.setSize(cardSize);
                        // calcola centro della carta, ruota 180° e posiziona
                        sf::Vector2f pos(startXTop + i * (cardSize.x + spacingTop), yTop);
                        // posizioniamo al centro come per le altre rotazioni
                        copy.centerOrigin();
                        copy.setRotation(180.f);
                        // converti da top-left a centro
                        sf::Vector2f center(pos.x + cardSize.x/2.f, pos.y + cardSize.y/2.f);
                        copy.setPosition(center);
                        copy.draw(window);
                    }
                }
            }
            ...
            if(game){
                ...
                if(responsePromptActive){
                    // Piccolo menu per attivare una carta settata nel turno avversario
                    ...
                    window.draw(panel);
                    sf::Text title(detailFont, "Attivare una carta? (Invio conferma, ESC annulla)", 22);
                    ...
                    window.draw(title);
                    // Lista delle carte attivabili con evidenziazione
                    ...
                    for(size_t i=0;i<responseActivables.size();++i){
                        ...
                        sf::Text item(detailFont, name, 22);
                        item.setFillColor(i==responseSelected ? sf::Color(255,240,160) : sf::Color(230,230,230));
                        item.setOutlineColor(sf::Color(0,0,0,200));
                        item.setOutlineThickness(2.f);
                        item.setPosition(sf::Vector2f(panelPos.x + 20.f, y0 + i*28.f));
                        window.draw(item);
                    }
                }
                if(deckSendChoiceActive){
                    ...
                    panel.setOutlineThickness(3.f);
                    window.draw(panel);
                    sf::Text title(detailFont, "Scegli un Mostro Drago dal Deck da mandare al Cimitero", 30);
                    title.setFillColor(sf::Color(255,230,140));
                    title.setOutlineColor(sf::Color(0,0,0,200));
                    title.setOutlineThickness(2.f);
                    ...
                    for(size_t i=0;i<deckSendCandidates.size();++i){
                        ...
                        sf::Vector2f pos(gridPos.x + c*(cellSize.x+pad), gridPos.y + r*(cellSize.y+pad));
                        Card copy = deckSendCandidates[i];
                        if(!copy.originalTexturePath.empty()){
                            sf::Texture &front = textureManager.getTexture(copy.originalTexturePath);
                            copy.setTexture(front);
                            sf::Vector2u ts = front.getSize();
                        ...
                        sf::Color tint = (i==deckSendSelected) ? sf::Color(255,240,160) : sf::Color::White;
                        copy.draw(window, tint);
                    }
                    // Hint tasti
                    sf::Text hint(detailFont, "Invio = Conferma, ESC = Annulla, Frecce o Click per selezionare", 22);
