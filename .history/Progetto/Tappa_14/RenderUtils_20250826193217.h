...
// Restituisce slotIdx sicuro e slotPos per Mostri (P1/P2) dato l'indice logico i
inline std::pair<int, sf::Vector2f> getMonsterSlotIdxAndPos(int side, size_t i,
    const Field& field, const sf::Vector2f& slotSize, const std::shared_ptr<Game>& game)
{
    int idx = static_cast<int>(i);
    if(game){
        int got = game->getMonsterSlotIndexAt(side, i);
        if(got >= 0) idx = got;
    }
    sf::Vector2f pos = field.getSlotPosition(Type::Monster, (side==0?P1:P2), idx);
    return {idx, pos};
}
