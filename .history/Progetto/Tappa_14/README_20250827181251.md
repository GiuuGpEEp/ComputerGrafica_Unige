...
Per ridurre ulteriormente la complessità di `main.cpp`, una prima tranche della gestione eventi è stata estratta in `InputController`:
...
1) Creare il contesto con i riferimenti necessari (finestra, stato, flag overlay, callback `resetMatch`, predicato `inputBlocked`, schermate/overlay interessati, getter di `Game*`).
2) Istanziate `Input::Controller controller(ctx);`
3) Nel poll degli eventi, delegare: `if (controller.handleEvent(event)) continue;`

Nota SFML v3: gli enum dei tasti e mouse sono namespaced come `sf::Keyboard::Key::Enter`, `sf::Mouse::Button::Left`.
