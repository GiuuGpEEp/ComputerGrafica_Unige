...
int main(){
    ...
    // Helpers locali per ridurre duplicazione nel rendering
    auto drawSpellTrapRow = [&](int side){
        if (!game) return;
        const bool isP1 = (side == 0);
        auto &vec = isP1 ? p1STCards : p2STCards;
        for (size_t i = 0; i < vec.size(); ++i) {
            Card c = vec[i];
            bool isFD = game->isSpellTrapFaceDownAt(side, i);
            if (isFD) { RenderUtils::applyBackTexture(c, tex); }
            c.draw(window);
        }
    };

    auto drawFieldSpell = [&] (int side) {
        const bool isP1 = (side == 0);
        auto &opt = isP1 ? p1FieldSpellCard : p2FieldSpellCard;
        if (opt.has_value()) {
            opt.value().draw(window);
        }
    };

    auto drawGraveyardTop = [&] (int side) {
        if (!game) return;
        const bool isP1 = (side == 0);
        const auto &gy = isP1 ? game->getGraveyard() : game->getOpponentGraveyard();
        if (gy.empty()) return;
        const Card &top = gy.back();
        Card copy = top;
        RenderUtils::applyFrontTextureIfAny(copy, textureManager);
        sf::Vector2f gyPos = field.getSlotPosition(Type::Graveyard, isP1 ? P1 : P2);
        float scale = RenderUtils::FieldScale;
        RenderUtils::placeCentered(copy, gyPos, slotSize, scale, scale);
        copy.draw(window);
    };
    ...
    // Se siamo nello stato StartScreen, disegna il testo sopra lo sfondo con animazione di dissolvenza
        ...
        // Disegna le carte sul campo (P1)
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(isFD){ RenderUtils::applyBackTexture(drawCard, tex); }
                // Ruota orizzontalmente se in DIFESA o coperta
                if(isDef || isFD){
                    int slotIdx = game ? game->getMonsterSlotIndexAt(0, i) : static_cast<int>(i);
                    if(slotIdx < 0) slotIdx = static_cast<int>(i);
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P1, slotIdx);
                    RenderUtils::placeRotatedDefense(drawCard, slotPos, slotSize);
                }
                if(!isDef && !isFD){
                    // Carta in piedi: nessuna rotazione; dimensione/posizione sono già impostate dalla sync
                    RenderUtils::setUpright(drawCard);
                }
                if(isDef){ color = RenderUtils::mixColors(color, sf::Color(150,200,255,color.a)); }
                ...
                if(game && game->getTurn().getPhase() == GamePhase::Battle){
                    ...
                    RenderUtils::drawAttackDot(window, fieldCards[i]);
                    }
                }
            }
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                if(isFD){ RenderUtils::applyBackTexture(drawCard, tex); }
                // Rotazione/posizionamento: difesa o coperta -> orizzontale; altrimenti in piedi capovolta
                if(isDef || isFD){
                    ...
                    if (slotIdx2 < 0) slotIdx2 = static_cast<int>(i);
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P2, slotIdx2);
                    // Calcola il top-left centrato nello slot in base alla size scalata
                    float s = RenderUtils::FieldScale;
                    sf::Vector2f scaledSize(slotSize.x * s, slotSize.y * s);
                    sf::Vector2f topLeft = slotPos + sf::Vector2f((slotSize.x - scaledSize.x) / 2.f,
                                                                  (slotSize.y - scaledSize.y) / 2.f);
                    RenderUtils::rotate180Centered(drawCard, topLeft, scaledSize);
                }
                if(isDef){ color = RenderUtils::mixColors(color, sf::Color(150,200,255,color.a)); }
                ...
                if(game && game->getTurn().getPhase() == GamePhase::Battle){
                    ...
                    if(cur == 1 && game->hasMonsterAlreadyAttacked(i)){
                        RenderUtils::drawAttackDot(window, oppFieldCards[i]);
                    }
                }
                drawCard.draw(window, color);
            }
