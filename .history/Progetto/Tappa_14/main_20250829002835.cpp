...
int main(){
    ...
    // Create a small MainInputHost implementation that forwards calls to the local lambdas/variables
    struct MainInputHost : IInputHost {
        std::function<void(size_t)> fnRestore;
        std::function<void()> fnUpdateHand;
        std::function<void()> fnSyncMonster;
        std::function<void()> fnSyncSpell;
        std::function<Game*()> fnGetGame;
        std::function<void(size_t)> fnConfirmDeckSend;
        std::function<std::optional<int>(sf::Vector2f)> fnFindMonsterSlot;

        // assignable action callbacks (set later)
        std::function<void()> onPlayClickedFunc;
        std::function<void()> onStartDeckSelectionFunc;
        std::function<void(int)> onSelectDeckIndexFunc;

        MainInputHost(
            std::function<void(size_t)> r,
            std::function<void()> u,
            std::function<void()> sm,
            std::function<void()> ss,
            std::function<Game*()> g,
            std::function<void(size_t)> cds,
            std::function<std::optional<int>(sf::Vector2f)> findSlot
        ) : fnRestore(std::move(r)), fnUpdateHand(std::move(u)), fnSyncMonster(std::move(sm)), fnSyncSpell(std::move(ss)), fnGetGame(std::move(g)), fnConfirmDeckSend(std::move(cds)), fnFindMonsterSlot(std::move(findSlot)) {}

        void restoreCardAtHandIndex(size_t idx) override { if(fnRestore) fnRestore(idx); }
        void updateHandLayout() override { if(fnUpdateHand) fnUpdateHand(); }
        void syncMonsterZoneToField() override { if(fnSyncMonster) fnSyncMonster(); }
        void syncSpellTrapZones() override { if(fnSyncSpell) fnSyncSpell(); }
        Game* getGame() override { return fnGetGame ? fnGetGame() : nullptr; }
        void confirmDeckSend(size_t idx) override { if(fnConfirmDeckSend) fnConfirmDeckSend(idx); }
        void onPlayClicked() override { if(onPlayClickedFunc) onPlayClickedFunc(); }
        void onStartDeckSelection() override { if(onStartDeckSelectionFunc) onStartDeckSelectionFunc(); }
        void onSelectDeckIndex(int idx) override { if(onSelectDeckIndexFunc) onSelectDeckIndexFunc(idx); }
        std::optional<int> findMonsterSlotIndexUnderMouse(sf::Vector2f mp) override { return fnFindMonsterSlot ? fnFindMonsterSlot(mp) : std::nullopt; }
    };

    // Prepare function wrappers for the host (capture locals by reference)
    std::function<void(size_t)> host_restore = [&](size_t idx){ restoreCardAtHandIndex(idx); };
    std::function<void()> host_updateHand = [&](){ updateHandLayout(); };
    std::function<void()> host_syncMonster = [&](){ syncMonsterZoneToField(); };
    std::function<void()> host_syncSpell = [&](){ syncSpellTrapZones(); };
    std::function<Game*()> host_getGame = [&](){ return game.get(); };
    std::function<void(size_t)> host_confirmDeckSend = [&](size_t idx){ if(game){ if(game->resolvePendingSendFromDeck(idx)){ deckSendChoiceActive = false; deckSendCandidates.clear(); deckSendSelected = 0; deckSendOwner = -1; battleFeedbackMsg = "Carta inviata al Cimitero"; battleFeedbackClock.restart(); } else { battleFeedbackMsg = "Selezione non valida"; battleFeedbackClock.restart(); } } };
    std::function<std::optional<int>(sf::Vector2f)> host_findMonsterSlot = [&](sf::Vector2f mp){ return findMonsterSlotIndexUnderMouse(mp, field, slotSize); };

    // Instantiate the host (callbacks like onPlayClicked will be set afterwards)
    MainInputHost mainHost(host_restore, host_updateHand, host_syncMonster, host_syncSpell, host_getGame, host_confirmDeckSend, host_findMonsterSlot);
    ...
    /*updateHandLayout*/[&](){ updateHandLayout(); },
        ...
        /*findSlotPosition*/[&](sf::Vector2f mp, const Card& c){ return findSlotPosition(mp, c, field, slotSize); },
        /*isDragging*/isDragging,
        /*isPotentialDrag*/isPotentialDrag,
        /*draggingCardIndex*/draggingCardIndex,
        /*potentialDragCardIndex*/potentialDragCardIndex,
        /*dragOffset*/dragOffset,
        /*initialMousePos*/initialMousePos,
    /*onPlayClicked*/[&](){} ,
    /*onStartDeckSelection*/[&](){} ,
    /*onSelectDeckIndex*/[&](int){ },
    /*host*/ &mainHost
    };
    // Callbacks that require lambdas or capture-local logic
    state.onPlayClicked = [&](){
        if(selectedDeckName.empty()){
            ...
            // preserve original main logic for starting a game
            ...
            p1STCards.clear(); p2STCards.clear(); p1FieldSpellCard.reset(); p2FieldSpellCard.reset();
            ...
            if(game){ int cur = game->getTurn().getCurrentPlayerIndex(); handPtr = (cur==0)? &game->current().getHand() : &game->opponent().getHand(); if(handPtr) handPtr->clear(); }
            gamestate = GameState::FieldLoading;
        }
    };
    state.onStartDeckSelection = [&](){
    ...
    state.onSelectDeckIndex = [&](int idx){
        if(idx == 0){ selectedDeckName = "Blu-Eyes"; deckSelected = true; deckSelectionScreen.setSelectedDeck(0); homeScreen.setDeckName(selectedDeckName); if(!deckSelectionScreen.isFading()) deckSelectionScreen.startFadeOut(AppConfig::Timing::FadeOutFastSec); }
        else if(idx == 1){ selectedDeckName = "Black-Magician"; deckSelected = true; deckSelectionScreen.setSelectedDeck(1); homeScreen.setDeckName(selectedDeckName); if(!deckSelectionScreen.isFading()) deckSelectionScreen.startFadeOut(AppConfig::Timing::FadeOutFastSec); }
        else if(idx == 2){ selectedDeckName = "Red-Eyes"; deckSelected = true; deckSelectionScreen.setSelectedDeck(2); homeScreen.setDeckName(selectedDeckName); if(!deckSelectionScreen.isFading()) deckSelectionScreen.startFadeOut(AppConfig::Timing::FadeOutFastSec); }
    };
    // Also wire the host to call same handlers (reduces need to set multiple callbacks in state)
    mainHost.onPlayClickedFunc = state.onPlayClicked;
    mainHost.onStartDeckSelectionFunc = state.onStartDeckSelection;
    mainHost.onSelectDeckIndexFunc = state.onSelectDeckIndex;
    ...
    while (window.isOpen()) {
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            // Altri tasti (ESC gestito dal Controller):
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                // Conferma Prompt/DeckSend gestita dal Controller
                // NOTE: gestione tasti C/X, E/P e navigazione prompt ora centralizzata in `InputController`.
                // Se necessario, controlla `Progetto/Tappa_14/InputController.cpp` per il comportamento esatto.
            }    
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            syncZones();
                            // populate the hoisted context and attach
                            appHandlersCtx.syncZones = [&](){ syncZones(); };
                            appHandlersCtx.syncMonsterZoneToField = [&](){ syncMonsterZoneToField(); };
                            appHandlersCtx.syncSpellTrapZones = [&](){ syncSpellTrapZones(); };
                            appHandlersCtx.updateHandLayout = [&](){ if(handPtr) updateHandLayout(); };
                            appHandlersCtx.openResponsePromptIfAny = [&](){ openResponsePromptIfAny(); };
                            appHandlersCtx.rebuildResponseActivables = [&](){ rebuildResponseActivables(); };
                            appHandlersCtx.closeResponsePrompt = [&](){ closeResponsePrompt(); };
                            appHandlersCtx.stopAllInputActions = [&](){ isDragging = false; isPotentialDrag = false; draggingCardIndex.reset(); potentialDragCardIndex.reset(); };
                            appHandlersCtx.resetDeckPositions = [&](){ 
                            ...
                            appHandlersCtx.setSelectingTributes = [&](bool v){ selectingTributes = v; };
                            appHandlersCtx.setTributesNeeded = [&](int v){ tributesNeeded = v; };
                            appHandlersCtx.setSsChoiceActive = [&](bool v){ ssChoiceActive = v; };
                            appHandlersCtx.setDeckSendChoiceActive = [&](bool v){ deckSendChoiceActive = v; };
                            appHandlersCtx.setDeckSendCandidates = [&](const std::vector<Card>& c){ deckSendCandidates = c; };
                            appHandlersCtx.setDeckSendOwner = [&](int o){ deckSendOwner = o; };
                            appHandlersCtx.pushBattleFeedbackMsg = [&](const std::string &s){ battleFeedbackMsg = s; battleFeedbackClock.restart(); };
