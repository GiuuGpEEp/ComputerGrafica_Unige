...
1. Estrarre funzioni di rendering rimanenti inline in `main.cpp` in `RenderUtils` o in un nuovo `UiRenderer` (HUD, overlay, disegno mano). Questo ridurrà ulteriormente la lunghezza del `main` e separerà logica da rendering.
2. Raggruppare la registrazione/sottoscrizione agli eventi del `Game` in una funzione helper `attachGameHandlers(Game&, ...)` per semplificare init/reset della partita.
3. (Opzionale) Creare un piccolo wrapper `App` che incapsula init/run/shutdown lasciando `main` molto corto. Consigliato solo se si è comodi con spostare init/teardown.

Se vuoi, procedo subito con il punto 1 e poi il punto 2; dimmi se preferisci che proceda ora.
