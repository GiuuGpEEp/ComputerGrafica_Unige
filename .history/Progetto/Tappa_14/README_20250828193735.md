...
**Aggiornamenti recenti (refactor e fix)

Di seguito sono elencate tutte le modifiche fatte rispetto all'ultima versione del README, incluse le correzioni emerse durante il refactor e il debugging successivo.

- Compatibilità SFML
	- Aggiunto `SFMLCompat.h` per centralizzare le differenze di enumerazioni fra versioni di SFML (mappando `M_MOUSE_*` e `M_KEY_*`).
	- Inclusi i mapping mancanti usati nel codice (`M_KEY_T`, `M_KEY_O`, ecc.) per evitare errori di compilazione dovuti a differenze di scope degli enum.

- InputController: comportamento e bugfix
	...
	- Fix per il flusso A/S (Attiva / Set):
		...
		- In entrambi i casi i rollback UI vengono gestiti tramite i callback forniti in `Input::Context` (`restoreCardAtHandIndex`, `syncSpellTrapZones`, `syncMonsterZoneToField`).
	- Hotkey centralizzate: `T` / `O` sono allineati alle azioni rapide già centralizzate (come `N`) e sono gestite direttamente dal controller.
...
- Texture / Config / TextureMap
	- Confermato e consolidato il caricamento centralizzato delle texture tramite la mappa costruita da `AppConfig::buildTextureMap(...)` e accesso tramite `AppConfig::findTextureInMap(...)`.
	- Questo elimina le texture locali in `main.cpp` e centralizza i path risorsa in `Config.h`.

- Quality/Build
	- Dopo le modifiche sono stati applicati fix incrementali seguenti i messaggi di compilazione (aggiunta macro compat, membro `updateHandLayout`, riordino initializer). Il target `mainTappa14` viene ora compilato con successo.

- Note operative e passi successivi consigliati
	- Runtime testing: testare manualmente i flussi critici nel gioco (A su carta dalla mano, S su ST dalla mano e S su mostro dalla mano, casi limite come zone piene o tributi richiesti).
	- Normalizzazione: rimangono alcune occorrenze storiche (.history) o file sparsi dove gli enum SFML non sono ancora normalizzati; si consiglia una sweep repo-wide per evitare regressioni future.
	- Documentazione: questa sezione riassume lo stato corrente; se vuoi posso aggiungere esempi rapidi di chiamate o uno step-by-step per riprodurre e testare i casi A/S.
