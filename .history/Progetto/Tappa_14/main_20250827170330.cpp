    ...
    while (window.isOpen()) {
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                // Nuovo toggle posizione: un solo tasto (C)if(gamestate == GameState::Playing && game && !selectingTributes && !chooseSummonOrSet){
            ...
            if(game){
                ...
                if(!battleFeedbackMsg.empty()){
                    ...
                    if(elapsed < BATTLE_FEEDBACK_DURATION){
                        RenderUtils::drawCenterFeedback(window, detailFont, windowSize, battleFeedbackMsg, 22, 70.f);
                    } else {
                        battleFeedbackMsg.clear();
                    }
                }
