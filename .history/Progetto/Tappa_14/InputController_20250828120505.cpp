...
bool Controller::handleEvent(const sf::Event& e){
    ...
    // Navigazione Prompt di risposta da tastiera
    if(state.responsePromptActive){
        if(const auto* kp = e.getIf<sf::Event::KeyPressed>()){
            if(kp->code == sf::Keyboard::Key::Up || kp->code == sf::Keyboard::Key::Left){
                if(state.responseOptionsCount && state.responseOptionsCount()>0){
                    size_t n = state.responseOptionsCount();
                    state.responseSelected = (state.responseSelected + n - 1) % n;
                }
                return true;
            }
            if(kp->code == sf::Keyboard::Key::Down || kp->code == sf::Keyboard::Key::Right){
                if(state.responseOptionsCount && state.responseOptionsCount()>0){
                    size_t n = state.responseOptionsCount();
                    state.responseSelected = (state.responseSelected + 1) % n;
                }
                return true;
            }
    ...
    // Overlay DeckSend: conferma con tastiera
    if(state.deckSendChoiceActive){
        if(const auto* kp = e.getIf<sf::Event::KeyPressed>()){
            if(kp->code == sf::Keyboard::Key::Enter){
