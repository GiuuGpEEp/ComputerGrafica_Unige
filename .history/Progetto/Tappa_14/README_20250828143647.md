...
- Il `main.cpp`, oltre ai precedenti argomenti al `Input::Context` i riferimenti agli overlay (`extraOverlay`, `graveyardOverlay`), le posizioni slot (`extraDeckSlotPos`, `graveyardSlotPos`) e i data provider (`getExtraCards()`, `getGraveyardCards()`).
