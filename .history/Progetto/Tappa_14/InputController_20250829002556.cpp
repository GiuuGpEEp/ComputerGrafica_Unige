...
bool Controller::handleEvent(const sf::Event& e){
    ...
    if(kp3->code == M_KEY_SPACE && state.gamestate == GameState::Playing){
        ...
        if(state.gamestate == GameState::Playing && !state.inputBlocked()){
            if(kp3->code == M_KEY_N){
                if(Game* game = state.host ? state.host->getGame() : (state.getGame ? state.getGame() : nullptr)){
                    game->advancePhase();
    ...
    if(kp3->code == M_KEY_A && state.gamestate == GameState::Playing && !state.inputBlocked()){
            Game* game = state.host ? state.host->getGame() : (state.getGame ? state.getGame() : nullptr);
    ...
    if(e.is<sf::Event::MouseButtonReleased>()){
        ...
        if(mb->button == M_MOUSE_LEFT){
            ...
            auto cardBackToHand = [&](){
                // Use the provided main callback to restore the card at the original hand index.
                if(!state.handPtr) return;
                if(state.restoreCardAtHandIndex){
                    state.restoreCardAtHandIndex(state.draggingCardIndex.value());
                } else {
                    // Fallback: if no callback is provided, perform a conservative restore by
                    // re-computing hand layout via updateHandLayout (main may already handle positions)
                    if(state.updateHandLayout) state.updateHandLayout();
                }
            };
