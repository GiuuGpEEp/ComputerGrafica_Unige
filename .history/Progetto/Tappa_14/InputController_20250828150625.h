...
struct Context {
    ...
    // Callback opzionali per tasti di debug (accentrati nel controller)
    std::function<void()> onAdvancePhaseDebug;      // N
    std::function<void()> onForceEndTurnDebug;      // T
    std::function<void()> onAddOpponentMonsterDebug; // O
};
