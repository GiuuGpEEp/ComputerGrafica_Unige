    ...
    // Se siamo nello stato StartScreen, disegna il testo sopra lo sfondo con animazione di dissolvenza
        ...
        // Disegna le carte sul campo (P1)
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(isFD){ 
                    drawCard.setTexture(AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack));
                    sf::Vector2u texSize = AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack).getSize();
            ...
            if(game){
                for(size_t i=0;i<p1STCards.size();++i){
                    ...
                    bool isFD = game->isSpellTrapFaceDownAt(0, i);
                    if(isFD){
                        c.setTexture(AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack));
                        sf::Vector2u ts = AppConfig::findTextureInMap(tex, AppConfig::TextureKey::CardBack).getSize();
                        c.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
                    }
                    c.draw(window);
                }
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                if(isFD){
                    drawCard.setTexture(AppConfig::findTextureInMap(textureManager, AppConfig::TextureKey::CardBack));
                    sf::Vector2u ts = AppConfig::findTextureInMap(textureManager, AppConfig::TextureKey::CardBack).getSize();
