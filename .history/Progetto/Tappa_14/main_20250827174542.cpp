            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                if(gamestate == GameState::Playing && game && !inputBlocked() && !chooseSummonOrSet){
                    if(keyPressed->code == sf::Keyboard::Key::C){
                        ...
                        sf::Vector2i mpos = sf::Mouse::getPosition(window);
                        bool triggered = false; // tenuto per uso futuro; non ha effetto quando il prompt è aperto
                        int cur = game->getTurn().getCurrentPlayerIndex();
                        if(cur == 0){
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                if(deckSendChoiceActive){
                    ...
                    size_t rows = (deckSendCandidates.empty()?0:((deckSendCandidates.size()-1)/cols + 1));
