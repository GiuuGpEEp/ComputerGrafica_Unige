...
int main(){
    ...
    Field field(
        AppConfig::findTextureInMap(textureManager, AppConfig::TextureKey::FieldBackground),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::MonsterText),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::SpellTrap),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::Deck),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::Graveyard),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::ExtraDeck),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::FieldSpell),
        windowSize
    );        
    ...
    if (!resourceManager.initialize(
            ...
            AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack))) {
    ...
    HomePage homeScreen(window, detailFont, windowSize,
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::HomeScreen), deck.getDeckName());
    DeckSelectionScreen deckSelectionScreen(window, textureManager, resourceManager,
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::DeckSelectionScreen), windowSize, detailFont);
    ...
    FieldLoadingAnimation fieldLoadingAnim = FieldLoadingAnimation(
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::HomeScreen),
        AppConfig::getTexture(textureManager, AppConfig::TextureKey::FieldBackground),
        windowSize);
    ...
    if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                drawController.update(deltaTime, mousePressed, forceSkipReveal, activeDeck, drawTargetHand, windowSize, cardSize, spacing, handYForAnim, HAND_MAXSIZE, activeDeckPos,
                    AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack), textureManager, &game->events());
    ...
    // Se siamo nello stato StartScreen, disegna il testo sopra lo sfondo con animazione di dissolvenza
        ...
        if(gamestate == GameState::StartScreen) drawStartScreen(window, detailFont, windowSize,
            AppConfig::getTexture(textureManager, AppConfig::TextureKey::StartScreen), startScreenElapsed);
        ...
        // Disegna le carte sul campo (P1)
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(isFD){ 
                    drawCard.setTexture(AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack));
                    sf::Vector2u texSize = AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack).getSize();
            ...
            if(game){
                for(size_t i=0;i<p1STCards.size();++i){
                    ...
                    bool isFD = game->isSpellTrapFaceDownAt(0, i);
                    if(isFD){
                        c.setTexture(AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack));
                        sf::Vector2u ts = AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack).getSize();
                        c.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
                    }
                    c.draw(window);
                }
            ...
            if(game){
                ...
                if(n > 0){
                    ...
                    for(size_t i=0;i<n;++i){
                        ...
                        copy.setTexture(AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack));
                        sf::Vector2u ts = AppConfig::getTexture(textureManager, AppConfig::TextureKey::CardBack).getSize();
                        copy.setTextureRect(sf::IntRect(sf::Vector2i(0,0), sf::Vector2i((int)ts.x, (int)ts.y)));
