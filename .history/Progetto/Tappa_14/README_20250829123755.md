...
Breve riepilogo delle modifiche fatte nell'ultima tranche di refactor e fix (locali a `Tappa_14`):

- Estratte ulteriori porzioni di rendering overlay da `main.cpp` in `RenderUtils.h` per ridurre codice inline:
	- Aggiunti gli helper `RenderUtils::drawDetailsPanel(...)` e `RenderUtils::drawOverlayDetails(...)` che incapsulano le chiamate a `showCardDetails` per la carta pescata, la carta selezionata e i pannelli degli overlay (Extra/Graveyard).
	- Sostituite le tre chiamate inline a `showCardDetails(...)` in `main.cpp` con i nuovi helper.

- Spostate le funzioni di sincronizzazione viste/zone (`syncMonsterZoneToField`, `syncSpellTrapZones`, wrapper `syncZones`) in `RenderUtils.h` e aggiornati i corrispondenti wrapper-lambda in `main.cpp` per mantenere il contratto di `Input::Context`.

- Rifattorizzato il wiring di inizializzazione verso `GameWiring.{h,cpp}`:
	- `registerDefaultEffects`, `createGameAndAttachResources` e `setupAppHandlers` sono state rimosse da `main.cpp` e centralizzate in `GameWiring`.
	- Corretto l'include per `DiscardController` e rimosso il doppio simbolo `setupAppHandlers` che causava linker error.
	- Aggiornato `CMakeLists.txt` per includere il nuovo translation unit nel target `mainTappa14`.

- Piccoli fix e pulizie:
	- Normalizzate include paths e firme delle funzioni per evitare mismatch tra TU.
	- Verificata la build (`mainTappa14`, Release) dopo le modifiche: linking OK e `bin\mainTappa14.exe` aggiornato.

Prossimi passi consigliati:
- Eseguire un rapido smoke-test manuale (Home → Gioca → seleziona mazzo → verifica pescata e pressione del tasto `E`) per confermare comportamento runtime.
- Se desideri, posso spostare implementazioni pesanti di `RenderUtils` in un `.cpp` separato per ridurre i tempi di compilazione.
