...
- Il `main.cpp`, oltre ai precedenti parametri al `Input::Context` i riferimenti agli overlay (`extraOverlay`, `graveyardOverlay`), le posizioni slot (`extraDeckSlotPos`, `graveyardSlotPos`) e i data provider (`getExtraCards()`, `getGraveyardCards()`).
...
In aggiunta, il Controller ora intercetta anche la pressione/rilascio del mouse per la logica di hold degli overlay Extra/Cimitero (vedi sezione sopra), riducendo ulteriormente il codice nel `main.cpp`.
