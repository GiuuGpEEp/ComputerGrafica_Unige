...
// CARD_MAXOFFSET kept for backward compatibility with comments; not used directly.
    ...
    while (window.isOpen()) {
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if(returnPopupActive){
                ...
                if(const auto* mb = event->getIf<sf::Event::MouseButtonPressed>()){
                    if(mb->button == sf::Mouse::Button::Left){
                        //La dimensioni dei bottoni ora è gestita interamente in RenderUtils, la funzione si limita a ritornare se è stato selezionato si o no
            ...
            if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                ...
                if(keyPressed->code == sf::Keyboard::Key::N && gamestate == GameState::Playing && game && !inputBlocked()){
                    game->advancePhase();
                ...
                if(keyPressed->code == sf::Keyboard::Key::T && gamestate == GameState::Playing && game && !inputBlocked()){
                ...
                if(keyPressed->code == sf::Keyboard::Key::A && gamestate == GameState::Playing && game && !inputBlocked()){
                    if(game->getTurn().getPhase() == GamePhase::Battle && selectedAttackerIndex.has_value()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(deckSendChoiceActive && but == sf::Mouse::Button::Left){
                    auto selectedCard = RenderUtils::deckSendHitIndex(windowSize, slotSize, deckSendCandidates.size(), sf::Mouse::getPosition(window));
                    if(selectedCard.has_value()) deckSendSelected = selectedCard.value();
                }
                ...
                if(but == sf::Mouse::Button::Right && gamestate == GameState::Playing && game && !inputBlocked() && game->getTurn().getPhase() == GamePhase::Battle){
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                if(chooseSummonOrSet && game && pendingHandIndexForPlay.has_value() && !inputBlocked()){
                ...
                if(gamestate == GameState::Playing && game && !inputBlocked() && !chooseSummonOrSet){
                    ...
                    } else if(keyPressed->code == sf::Keyboard::Key::X && !inputBlocked()){
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                if(deckSendChoiceActive){
                    size_t cols = AppConfig::Ui::DeckSendCols; size_t rows = (deckSendCandidates.empty()?0:((deckSendCandidates.size()-1)/cols + 1));
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(showNoDeckWarning){
                ...
                if(elapsed < NO_DECK_WARNING_DURATION){
                    ...
                    // Effetto fade-out negli ultimi AppConfig::Timing::NoDeckWarningFadeSec secondi
                    float alpha = 255.f;
                    float fadeTime = AppConfig::Timing::NoDeckWarningFadeSec;
