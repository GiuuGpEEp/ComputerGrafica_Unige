...
bool Controller::handleEvent(const sf::Event& e){
    ...
    if(const auto* kp3 = e.getIf<sf::Event::KeyPressed>()){
        ...
        // Tasti debug centralizzati (se forniti dal main)
        if(state.gamestate == GameState::Playing && !state.inputBlocked()){
            if(kp3->code == sf::Keyboard::Key::N){ if(state.onAdvancePhaseDebug) { state.onAdvancePhaseDebug(); return true; } }
            if(kp3->code == sf::Keyboard::Key::T){ if(state.onForceEndTurnDebug) { state.onForceEndTurnDebug(); return true; } }
            if(kp3->code == sf::Keyboard::Key::O){ if(state.onAddOpponentMonsterDebug) { state.onAddOpponentMonsterDebug(); return true; } }
        }
