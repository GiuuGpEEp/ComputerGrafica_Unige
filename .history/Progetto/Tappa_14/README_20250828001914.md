...
La classe controller contiene come campo un elemento state di tipo Contex. Contex è una struct composta da tutti i flag e le funzioni che servono per gestire gli eventi 
...
1) Creare un contesto con i riferimenti necessari (finestra, stato, flag overlay, callback `resetMatch`, predicato `inputBlocked`, schermate/overlay interessati, getter di `Game*`).
