                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            openingCurrentPlayer = -1;
                                ...
                                syncZones();
                                // Incapsula le sottoscrizioni agli eventi del Game in una funzione locale per chiarezza
                                auto attachGameHandlers = [&](Game &g){
                                    drawStartSubscriptionId = g.events().subscribe(GameEventType::DrawStart, [](){
                                        std::cout << "[Event] DrawStart" << std::endl;
                                    });
                                    drawEndSubscriptionId = g.events().subscribe(GameEventType::DrawEnd, [](){
                                        std::cout << "[Event] DrawEnd" << std::endl;
                                    });
                                    g.events().subscribe(GameEventType::NormalSummonTributeRequired, [&](){
                                        selectingTributes = true;
                                        selectedTributes.clear();
                                        tributesNeeded = g.getPendingTributesNeeded();
                                    ...
                                    g.events().subscribe(GameEventType::MonstersTributed, [&](){
                                        std::cout << "[Event] MonstersTributed" << std::endl;
                                        syncMonsterZoneToField();
                                    });
                                    g.events().subscribe(GameEventType::NormalSummon, [&](){
                                        std::cout << "[Event] NormalSummon" << std::endl;
                                        syncZones();
                                    });
                                    g.events().subscribe(GameEventType::NormalSet, [&](){
                                    ...
                                    g.events().subscribe(GameEventType::TurnStart, [&](){
                                        ...
                                        int cur2 = g.getTurn().getCurrentPlayerIndex();
                                        handPtr = (cur2==0) ? &g.current().getHand() : &g.opponent().getHand();
                                        // riallinea layout della mano visibile
                                        if(handPtr){ updateHandLayout(); }
                                    ...
                                    g.events().subscribe(GameEventType::PhaseChange, [&](){
                                        openResponsePromptIfAny();
                                    });
                                    g.events().subscribe(GameEventType::TurnEnd, [&](){
                                    ...
                                    // Eventi di battaglia
                                    attackDeclaredSubId = g.events().subscribe(GameEventType::AttackDeclared, [&](){
                                        std::cout << "[Event] AttackDeclared" << std::endl;
                                        // Apri il prompt di risposta per il giocatore non di turno, se ha ST attivabili
                                        openResponsePromptIfAny();
                                    });

                                    attackResolvedSubId = g.events().subscribe(GameEventType::AttackResolved, [&](){
                                        std::cout << "[Event] AttackResolved" << std::endl;
                                        syncMonsterZoneToField();
                                        selectedAttackerIndex.reset();
                                    });

                                    monsterDestroyedSubId = g.events().subscribe(GameEventType::MonsterDestroyed, [&](){
                                        std::cout << "[Event] MonsterDestroyed" << std::endl;
                                    });

                                    lifeChangedSubId = g.events().subscribe(GameEventType::LifePointsChanged, [&](){
                                    ...
                                    g.events().subscribe(GameEventType::MonsterFlipped, [&](){
                                    ...
                                    g.events().subscribe(GameEventType::SpellSet, [&](){
                                            battleFeedbackMsg = "Magia settata"; battleFeedbackClock.restart();
                                            syncSpellTrapZones();
                                    });
                                    g.events().subscribe(GameEventType::TrapSet, [&](){
                                    ...
                                    g.events().subscribe(GameEventType::DeckSendChoiceRequested, [&](){
                                        ...
                                        deckSendOwner = g.getTurn().getCurrentPlayerIndex();
                                        // Raccogli candidati Drago dal Deck del proprietario
                                        const Deck &d = g.getDeckOf(deckSendOwner);
                                    ...
                                    g.events().subscribe(GameEventType::SpecialSummonChoiceRequested, [&](){
                                        ...
                                        ssChoiceOwner = g.getTurn().getCurrentPlayerIndex();
                                        ...
                                        if(ssChoiceOwner == 1){
                                            ...
                                            (void)g.resolvePendingSpecialSummon(/*defense*/false);
                                        } else {
                                    ...
                                    directAttackSubId = g.events().subscribe(GameEventType::DirectAttack, [&](){
                                        std::cout << "[Event] DirectAttack" << std::endl;
                                    });
                                    // Vittoria/Sconfitta -> attiva l'overlay
                                    g.events().subscribe(GameEventType::Win, [&](){
                                        if(!gameOverActive){
                                            gameOverActive = true;
                                        }
                                    });
                                    g.events().subscribe(GameEventType::Lose, [&](){
                                        if(!gameOverActive){
                                            gameOverActive = true;
                                        }
                                    });
