...
- Wiring e callback aggiuntivi
	...
	### Stato build e test rapido

	- Target `mainTappa14` compilato e linkato localmente (Release). L'eseguibile avvia correttamente; i log di debug appaiono in console quando si verificano eventi di pesca.

	### Note e passi successivi raccomandati

	- Se vuoi che verifichi automaticamente le sequenze di pesca (player/opponent) posso aggiungere temporaneamente un test non invasivo che mette in coda una pescata all'avvio per catturare i log e poi rimuoverlo.
	- Se preferisci test manuale, esegui l'app e attiva una pescata (modalità gioco o tasti di debug) e incolla qui i log; io li rivedo e propongo ulteriori azioni se necessario.
