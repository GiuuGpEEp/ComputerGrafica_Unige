...
int main(){
    ...
    // Wire additional inputs/drag state for the controller
    state.mousePressed = mousePressed;
    state.handPtr = handPtr;
    state.fieldCards = fieldCards;
    state.findSlotPosition = [&](sf::Vector2f mp, const Card& c){ return findSlotPosition(mp, c, field, slotSize); };
    state.isDragging = isDragging;
    state.isPotentialDrag = isPotentialDrag;
    state.draggingCardIndex = draggingCardIndex;
    state.potentialDragCardIndex = potentialDragCardIndex;
    state.dragOffset = dragOffset;
    state.initialMousePos = initialMousePos;
