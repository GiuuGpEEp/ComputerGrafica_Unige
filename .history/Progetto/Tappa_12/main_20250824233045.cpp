...
int main(){
    ...
    auto syncMonsterZoneToField = [&](){
        if(!game) return;
        // Bottom (P1) should always render Player 0's zone, Top (P2) Player 1's zone,
        // even when it's Player 1's turn. Since Game exposes only current/opponent views,
        // derive absolute zones from current index.
    ...
    // Gestione del sollevamento delle carte in mano
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(game){
                    // Bottom always corresponds to Player 0
                ...
                // Draw attack dot only on current player's side (bottom if cur==0, top if cur==1)
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                bool isDef=false, isFD=false;
                if(game){
                    // Top always corresponds to Player 1
                ...
                // Attack dot for current player when current is Player 1 (top side)
                if(game && game->getTurn().getPhase() == GamePhase::Battle){
                    int cur = game->getTurn().getCurrentPlayerIndex();
