    ...
    if(maybeTargetIndex.has_value()){
        ...
        if(monsterZones[opp][tIdx].getValues().has_value()){
            if(monsterIsDefense[opp][tIdx] || monsterIsFaceDown[opp][tIdx]){
                atkOrDefB = monsterZones[opp][tIdx].getValues().value().second; // difesa
            } else {
                atkOrDefB = monsterZones[opp][tIdx].getValues().value().first; // attacco
            }
            atkOrDefB = monsterZones[opp][tIdx].getValues().value().first;
        }
        if(atkA > atkOrDefB){
