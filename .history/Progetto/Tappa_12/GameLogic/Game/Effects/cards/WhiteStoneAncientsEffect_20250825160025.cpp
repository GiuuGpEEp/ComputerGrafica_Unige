...
void WhiteStoneAncientsEffect::onEvent(GameEventType type, Game& game){
    ...
    if(type == GameEventType::PhaseChange){
        ...
        if(found.has_value()){
            bool ok = game.specialSummonToMonsterZone(ownerIdx, std::move(*found), /*defense*/false, /*faceDown*/false);
            if(!ok){
                std::cout << "[Effect] WSoAncients: Special Summon failed (zone full?)" << std::endl;
            } else {
                std::cout << "[Effect] WSoAncients: BEWD Special Summoned" << std::endl;
            }
        } else {
