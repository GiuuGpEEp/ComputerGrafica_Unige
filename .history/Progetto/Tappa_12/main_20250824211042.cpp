    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                // Cambio posizione rapido: D = Difesa scoperta, F = Attacco scoperta
                    ...
                    if(keyPressed->code == sf::Keyboard::Key::D){
                        if(selectedCardIsOnField && selectedCardIndex.has_value()){
                            if(game->setPosition(selectedCardIndex.value(), true, false)){
                                // Render si aggiorna usando query in draw
                            }
                        }
    ...
    // Gestione del sollevamento delle carte in mano
        ...
        if(!returnPopupActive){
            ...
            for(size_t i = 0; i<handPtr->size(); ++i){
                ...
                if(chooseSummonOrSet){
                    // Disabilita l'hover-lift mentre si sta scegliendo Evoca/Posiziona
                    targetCardOffset = 0.f;
                } else if(selectedCardIndex.has_value() && !selectedCardIsOnField && selectedCardIndex.value() == i) {
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(isFD){
                    drawCard.setTexture(texture);
                }
                if(isDef){
                    // Tinta azzurra leggera
