...
**Rendering carta coperta e cambio posizione (UI/UX)**
- Le carte Settate vengono disegnate con il retro: in draw, quando `isFaceDownAt(...)` è vero la carta usa `textureNonFlipped` (retro). Vale sia per P1 che per P2.
- Le carte in Difesa hanno una leggera tinta azzurra per distinguere la posizione.
- Durante il drop su uno slot valido, compare una scelta rapida: `E` = Evoca (Attacco scoperto), `P` = Posiziona (Difesa coperta). Con `ESC` si annulla e la carta torna automaticamente nella mano.
- Cambiare posizione di un mostro sul campo: `D` imposta Difesa scoperta, `F` imposta Attacco scoperto (vincoli di fase/regole ulteriori potranno essere aggiunti). Il rendering si aggiorna in base a `isDefenseAt`/`isFaceDownAt`.
- Micro-UX: l'hover-lift della mano è disabilitato mentre è attiva la scelta Evoca/Posiziona per evitare interazioni indesiderate.
