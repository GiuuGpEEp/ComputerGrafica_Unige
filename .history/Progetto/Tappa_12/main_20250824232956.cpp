...
int main(){
    ...
    bool chooseSummonOrSet = false; // quando droppo su slot valido
    ...
    auto syncMonsterZoneToField = [&](){
        if(!game) return;
        // Bottom (P1) = current player's field; Top (P2) = opponent
        const auto &p1Zone = game->getMonsterZone();
        const auto &p2Zone = game->getOpponentMonsterZone();
        ...
        for(size_t i=0;i<p1Zone.size();++i){
            Card c = p1Zone[i]; // copy for rendering
    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (handPtr && findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).has_value()) {
                            // Mostra scelta Evocazione/Posizionamento (scorciatoie: E per Evocazione, P per Posizionamento)
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                // Cambio posizione rapido: D (Difesa scoperta) / F (Attacco scoperto)
                if(gamestate == GameState::Playing && game && !selectingTributes && !chooseSummonOrSet){
                    if(keyPressed->code == sf::Keyboard::Key::D){
                        if(selectedCardIsOnField && selectedCardIndex.has_value()){
                            if(game->setPosition(selectedCardIndex.value(), true, false)){
                                battleFeedbackMsg = "Posizione: Difesa";
                                battleFeedbackClock.restart();
                            }
                        }
                    } else if(keyPressed->code == sf::Keyboard::Key::F){
                        if(selectedCardIsOnField && selectedCardIndex.has_value()){
                            if(game->setPosition(selectedCardIndex.value(), false, false)){
                                battleFeedbackMsg = "Posizione: Attacco";
                                battleFeedbackClock.restart();
                            }
                        }
                    }
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            game->events().subscribe(GameEventType::NormalSummonTributeRequired, [&](){
                                ...
                                // Stop any current drag to avoid moving hand cards during tribute selection
                            ...
                            // Turn change: resync zones so rendering aligns with current player index
                            ...
                            game->events().subscribe(GameEventType::TurnEnd, [&](){
                                ...
                                // Optional, but keeps UI consistent even before TurnStart
                                syncMonsterZoneToField();
                            });
                            // Eventi di battaglia
    ...
    // Gestione del sollevamento delle carte in mano
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(gamestate == GameState::FieldVisible || gamestate == GameState::Playing ) {
                bool enableHover = !extraOverlay.isOverlayVisible() && !returnPopupActive && !chooseSummonOrSet;
            ...
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(selectedAttackerIndex.has_value() && selectedAttackerIndex.value()==i && attackSelectionActive){
                    color = sf::Color(255,240,160); // light tint highlight
                }
                bool isDef=false, isFD=false;
                if(game){
                    // Bottom side shows current player's state since Game flags are indexed by current player
                    int curIdx = game->getTurn().getCurrentPlayerIndex();
                    isDef = game->isDefenseAt(curIdx, i);
                    isFD = game->isFaceDownAt(curIdx, i);
                }
                Card drawCard = fieldCards[i];
                // Face-down must use back texture
                if(isFD){ drawCard.setTexture(textureNonFlipped); }
                // Rotate horizontally when in DEF or face-down
                if(isDef || isFD){
                    // Use slot-centric placement and adjust size for horizontal layout
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P1, static_cast<int>(i));
                    sf::Vector2f slotCenter(slotPos.x + slotSize.x/2.f, slotPos.y + slotSize.y/2.f);
                    // Target size before rotation:
                    //  - width (thin) ~ 60% of slot height
                    //  - height (long) ~ 95% of slot width (so after 90° rotation the horizontal base is long)
                ...
                if(!isDef && !isFD){
                    // Upright card: ensure no rotation and correct size/position already set by sync
                ...
                if(game && game->getTurn().getPhase() == GamePhase::Battle && game->hasMonsterAlreadyAttacked(i)){
            ...
            for (size_t i=0;i<oppFieldCards.size();++i) {
                ...
                bool isDef=false, isFD=false;
                if(game){
                    int oppIdx = 1 - game->getTurn().getCurrentPlayerIndex();
                    isDef = game->isDefenseAt(oppIdx, i);
                    isFD = game->isFaceDownAt(oppIdx, i);
                }
                // Opponent face-down also uses back texture
                if(isFD){ drawCard.setTexture(textureNonFlipped); }
                // Rotate horizontally when in DEF or face-down
                if(isDef || isFD){
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P2, static_cast<int>(i));
