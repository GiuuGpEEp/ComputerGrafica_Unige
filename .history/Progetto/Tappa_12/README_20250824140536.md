...
Nella classe Game ho aggiunto quindi una modifica nel costruttore della classe stessa --> quando si verifica un qualsiasi tipo di evento vi è una nuova subscription che gestisce quel tipo di evento e delega il lavoro alla dispatchEvents.
In questo modo ogni volta che si verifica un evento di quel genere viene chiamato il Listener associato.

**Classe EffectSystem**
- Contiene la map  per associare il nome di una carta al suo effetto
- Contiene un booleano per evitare un ricontrollo
- Contiene un metodo per registrare una carta all'interno della mappa
- Contiene il metodo dispatch -> quando viene rilevato un evento il booleano 