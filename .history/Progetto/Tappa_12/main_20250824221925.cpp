...
int main(){
    ...
    float fieldCardScale = 0.75f;
        ...
        for(size_t i=0;i<zone.size();++i){
            Card c = zone[i]; // copia per il rendering
    ...
    // Gestione del sollevamento delle carte in mano
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            for (size_t i=0;i<fieldCards.size();++i) {
                ...
                if(isFD){ drawCard.setTexture(textureFlipped); }
                // Rotate horizontally when in DEF or face-down
                if(isDef || isFD){
                    sf::FloatRect gb = drawCard.getGlobalBounds();
                    sf::Vector2f center(gb.position.x + gb.size.x/2.f, gb.position.y + gb.size.y/2.f);
                    // Temporarily draw as rotated by swapping size and compensating position
                    // Shrink a bit to fit horizontally
                    sf::Vector2f curSize(gb.size.x, gb.size.y);
                    sf::Vector2f rotatedSize(curSize.y * 0.95f, curSize.x * 0.95f);
                    // Recenter to slot center
                    // Find slot center from field layout
                    sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, P1, static_cast<int>(i));
                    sf::Vector2f slotCenter(slotPos.x + slotSize.x/2.f, slotPos.y + slotSize.y/2.f);
                    drawCard.setSize(rotatedSize);
                    drawCard.setPosition(sf::Vector2f(slotCenter.x - rotatedSize.x/2.f, slotCenter.y - rotatedSize.y/2.f));
                }
                if(isDef){
