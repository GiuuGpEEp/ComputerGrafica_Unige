...
int main(){
    ...
    auto openResponsePromptIfAny = [&](){
        ...
        if(!responseActivables.empty()){
            ...
            responseSelected = 0;
            // Non bloccare altri input: prompt non-modale
        }
    };
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                if (keyPressed->code == sf::Keyboard::Key::Escape) {
                    // ESC chiude il prompt di risposta
    ...
    // Consenti il drag anche se il prompt di risposta è aperto (prompt non-modale)
    if (isPotentialDrag && potentialDragCardIndex.has_value() && !selectingTributes && !deckSendChoiceActive) {
