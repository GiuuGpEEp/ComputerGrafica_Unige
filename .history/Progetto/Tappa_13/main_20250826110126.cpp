...
int main(){
    ...
    if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                int cur = game->getTurn().getCurrentPlayerIndex();
                Deck &activeDeck = (cur == 0) ? deck : opponentDeck;
                sf::Vector2f activeDeckPos = (cur == 0) ? deckSlotPos : deckSlotPosTop;
                // La mano visibile resta P1; la mano di destinazione della pescata è quella del giocatore di turno
                std::vector<Card> &drawTargetHand = game->current().getHand();
                float handYForAnim = (cur == 0) ? y : 30.f; // posizione alta per la mano avversaria (non visibile)
