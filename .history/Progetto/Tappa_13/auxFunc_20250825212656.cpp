...
void showCardDetails(sf::RenderWindow& window, 
    ...
    auto featureToString = [](Feature f) {
        switch(f) {
            ...
            case Feature::Gemello: return "Gemello";
    ...
    // Attributo (cerchio) in alto a destra disegnato prima del titolo per poter calcolare lo spazio disponibile
    ...
    // Titolo: ridimensiona automaticamente o tronca con ellissi se troppo lungo per lo spazio disponibile
    float availableNameWidth = (attrCircle.getPosition().x) - (x) - 10.f; // margine prima del cerchio
    unsigned int baseTitleSize = detailFontSize + 10;
    unsigned int minTitleSize = detailFontSize + 2; // non scendere troppo
    sf::Text nameText(font, card.getName(), baseTitleSize);
    nameText.setFillColor(sf::Color::Black);
    nameText.setStyle(sf::Text::Bold);

    // Prima prova: riduci la dimensione finché entra o finché raggiungi il minimo
    while(nameText.getLocalBounds().size.x > availableNameWidth && baseTitleSize > minTitleSize){
        --baseTitleSize;
        nameText.setCharacterSize(baseTitleSize);
    }

    // Se ancora non entra, tronca con "..." mantenendo la dimensione attuale
    if(nameText.getLocalBounds().size.x > availableNameWidth){
        std::string full = card.getName();
        std::string ellipsis = "...";
        // Rimuovi progressivamente caratteri finché entra
        // (usa caratteri UTF-8 semplici: se presenti multibyte potrebbe essere necessario raffinamento)
        while(!full.empty()){
            full.pop_back();
            nameText.setString(full + ellipsis);
            if(nameText.getLocalBounds().size.x <= availableNameWidth) break;
        }
    }
...
void updateHandPositions(std::vector<Card>& cards, 
    ...
    // Imposta sempre dimensioni coerenti per le carte in mano.
    // - Se abbiamo troppe carte, riduci dimensione e spaziatura proporzionalmente.
    // - Altrimenti, ripristina la dimensione standard di mano (cardSize).
...
std::optional<int> findMonsterSlotIndexUnderMouse(sf::Vector2f mousePos, Field& field, const sf::Vector2f& slotSize){
    sf::Vector2f expanded = slotSize * 1.5f;
    sf::Vector2f half = expanded / 2.f;
    for(int i=0;i<5;++i){
        sf::Vector2f slotPos = field.getSlotPosition(Type::Monster, 1, i);
        if(slotPos == sf::Vector2f(0.f,0.f)) continue;
        Slot* slot = field.getSlotByPosition(slotPos);
        if(!slot) continue;
        if(slot->isOccupied()) continue;
        if(mousePos.x >= slotPos.x - half.x && mousePos.x <= slotPos.x + half.x &&
           mousePos.y >= slotPos.y - half.y && mousePos.y <= slotPos.y + half.y){
            return i;
        }
    }
    return std::nullopt;
