...
- Modifica dell'attivazione delle magie e trappola posizionate:
	...
	- `bool activateSetSpellTrapFor(int playerIdx, size_t zoneIndex);`
		...
		- Flusso: `canActivate` --> si guardano le precondizioni dell'effetto delle carte, scoperta carta (`stIsFaceDown=false`), evento `TrapActivated`/`SpellActivated`, risoluzione; se non-Continua, invio al Cimitero con rimozione voci parallele.
