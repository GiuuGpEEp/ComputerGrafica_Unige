                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            game->events().subscribe(GameEventType::TurnStart, [&](){
                                ...
                                syncMonsterZoneToField();
                                syncSpellTrapZones();
                                // Chiudi response window allo scattare del nuovo turno
                            ...
                            game->events().subscribe(GameEventType::MonsterFlipped, [&](){
                                ...
                                syncMonsterZoneToField();
                                syncSpellTrapZones();
                                // Chiusura prudente della response window su eventi importanti
                                responseWindowActive = false;
                            });
    ...
    // Se siamo nello stato StartScreen, disegna il testo sopra lo sfondo con animazione di dissolvenza
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(game){
                ...
                // Indicatore finestra risposta durante il turno avversario
                if(responseWindowActive){
                    sf::Text resp(detailFont, "Finestra risposta: X per attivare Trappole/Rapide settate (ESC per chiudere)", 20);
                    resp.setFillColor(sf::Color(200,255,200,240));
                    resp.setOutlineColor(sf::Color(0,0,0,180));
                    resp.setOutlineThickness(2.f);
                    resp.setPosition(sf::Vector2f(15.f, 150.f));
                    window.draw(resp);
                }
