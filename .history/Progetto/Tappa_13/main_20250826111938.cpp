...
int main(){
    ...
    if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                ...
                bool skipReveal = forceOpening ? (openingCurrentPlayer == 1) : (cur == 1);
                drawController.update(deltaTime, skipReveal, activeDeck, drawTargetHand, windowSize, cardSize, spacing, handYForAnim, HAND_MAXSIZE, activeDeckPos, textureFlipped, textureManager, &game->events());
