...
int main(){
    ...
    // Stato per gestire la mano iniziale del P2 (5 carte) subito dopo l'avvio partita
    bool p2OpeningDrawPending = false;   // finché non completa le 5 pescate
    bool p2OpeningDrawQueued = false;    // abbiamo messo in coda le 5 pescate per P2
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Tappa 13");
    ...
    auto openResponsePromptIfAny = [&](){
        if(!game) return;
        int cur = game->getTurn().getCurrentPlayerIndex();
        responseOwner = 1 - cur; // proponi sempre per il non-turno
        // Gating: mostra il prompt solo quando il non-turno e' il giocatore in basso (assoluto 0)
        if(responseOwner != 0) return;
    ...
    p2FieldSpellCard.reset();
        ...
        game.reset();
        handPtr = nullptr;
    // Reset opening draw flags
    p2OpeningDrawPending = false;
    p2OpeningDrawQueued = false;
                        ...
                        if (handPtr && findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).has_value()) {
                            ...
                            if(game && !selectingTributes){
                                ...
                                if(t == Type::Monster){
                                    ...
                                    battleFeedbackMsg = "Premi E per Evocare, P per Posizionare (sullo slot indicato)";
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                if(chooseSummonOrSet && game && pendingHandIndexForPlay.has_value() && !responsePromptActive){
                    if(keyPressed->code == sf::Keyboard::Key::E){
                        ...
                        bool ok = false;
                        if(slotIdxOpt.has_value()){
                            ok = game->tryNormalSummonToSlot(pendingHandIndexForPlay.value(), static_cast<size_t>(*slotIdxOpt));
                        } else {
                            ok = game->tryNormalSummon(pendingHandIndexForPlay.value());
                        }
                        if(ok){
                        ...
                        bool ok = false;
                        if(slotIdxOpt.has_value()){
                            ok = game->tryNormalSetToSlot(pendingHandIndexForPlay.value(), static_cast<size_t>(*slotIdxOpt));
                        } else {
                            ok = game->tryNormalSet(pendingHandIndexForPlay.value());
                        }
                        if(ok){
                            if(handPtr){ updateHandPositions(*handPtr, windowSize, cardSize, spacing, y, HAND_MAXSIZE); }
                        } else {
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            game->start();
                            // Game::start() mette in coda 5 pescate per il giocatore iniziale (P1);
                            // qui prepariamo le 5 pescate iniziali del P2 che verranno animate subito dopo.
                            p2OpeningDrawPending = true;
                            p2OpeningDrawQueued = false;
    ...
    if(gamestate == GameState::Playing && !returnPopupActive){
            if(game){
                // Esegui l'animazione di pescata per il giocatore di turno sul deck corretto,
                // con uno step speciale: dopo l'avvio partita, anche il P2 deve pescare 5 carte subito.
                int cur = game->getTurn().getCurrentPlayerIndex();

                // Se dobbiamo ancora dare la mano iniziale al P2, mettiamo in coda 5 pescate quando il controller è idle
                if(p2OpeningDrawPending && !p2OpeningDrawQueued && drawController.idle()){
                    drawController.queueDraw(5);
                    p2OpeningDrawQueued = true;
                }

                // Durante questa coda speciale, forza la pescata dal deck avversario alla mano dell'avversario (P2 in alto)
                bool forceOpponentDraw = p2OpeningDrawPending && p2OpeningDrawQueued;
                Deck &activeDeck = forceOpponentDraw ? opponentDeck : ((cur == 0) ? deck : opponentDeck);
                sf::Vector2f activeDeckPos = forceOpponentDraw ? deckSlotPosTop : ((cur == 0) ? deckSlotPos : deckSlotPosTop);
                // La mano visibile resta P1; la destinazione è la mano corrente o, se forzato, quella dell'avversario
                std::vector<Card> &drawTargetHand = forceOpponentDraw ? game->opponent().getHand() : game->current().getHand();
                float handYForAnim = forceOpponentDraw ? 30.f : ((cur == 0) ? y : 30.f); // top lane per mano avversaria
                drawController.update(deltaTime, mousePressed, activeDeck, drawTargetHand, windowSize, cardSize, spacing, handYForAnim, HAND_MAXSIZE, activeDeckPos, textureFlipped, textureManager, &game->events());
                ...
                // Se abbiamo terminato la coda speciale per P2, spegni il flag
                if(p2OpeningDrawPending && p2OpeningDrawQueued && drawController.idle()){
                    p2OpeningDrawPending = false;
                    p2OpeningDrawQueued = false;
                }
