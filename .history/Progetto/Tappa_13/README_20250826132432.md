...
- Aggiunto un Prompt di risposta, per attivare effetti nel turno avversario per Trap/Magie Rapide:
	...
	- Gating: il prompt compare solo nel turno di P2 e mostra esclusivamente le carte attivabili di P1 (giocatore locale).
	- IA sospesa: l’IA di P2 resta in pausa mentre il prompt è aperto; il prompt si riapre automaticamente ad ogni cambio fase del turno di P2.
...
- Battaglia (ATK = DEF in difesa): se ATK e DEF sono uguali, nessuna carta viene distrutta e non viene inflitto alcun danno.
- Scelte P2 su evocazioni speciali con scelta posizione: risolte automaticamente dal motore (default Attacco), senza mostrare prompt a P1.

### Effetti carta (White Stone)

- Pietra Bianca della Leggenda: quando viene mandata al Cimitero, permette di aggiungere un “Drago Bianco Occhi Blu” dal Deck alla mano del proprietario.
- Pietra Bianca degli Antichi: se mandata al Cimitero in questo turno, durante la End Phase permette di Evocare Specialmente un “Drago Bianco Occhi Blu” dal Deck del proprietario; se il proprietario è P2, la scelta Attacco/Difesa viene risolta automaticamente.
...
Evocazioni per Tributo con campo pieno
- La UI consente di iniziare il flusso di Evocazione/Posa con Tributi anche quando tutti gli slot Mostro sono occupati, se sono disponibili i tributi richiesti.
- Il motore gestisce i Tributi, libera gli slot e posiziona il mostro evocato nello slot scelto se rimasto libero dopo i tributi; in caso contrario usa il primo slot libero disponibile.
...
## Modifica del Main, Rendering Pescata e Deck
