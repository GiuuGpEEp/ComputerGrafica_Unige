...
void Game::mirrorExternalDeckRemoveByName(const std::string& name){
    if(!externalDeck) return;
    auto removed = externalDeck->removeFirstByName(name);
    (void)removed; // UI deck serves only for visuals; card object is discarded here
}
...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    if(isContinuous){
        ...
        dispatcher.emit(GameEventType::CardMoved);
        // Se l'oggetto continuo ha anche una risoluzione on-activate, eseguila ora
        if(act){ act->resolve(*this, ownerIdx); }
    } else {
        // Carta non-continua: risolvi effetto, poi manda al Cimitero
        bool ok = true;
...
bool Game::activateSetSpellTrapFor(int playerIdx, size_t zoneIndex){
    ...
    if(!isContinuous){
        ...
        // Esegui risoluzione prima di mandare al Cimitero
...
bool Game::resolvePendingSendFromDeck(size_t indexInFiltered){
    ...
    // Mantieni coerente il Deck esterno usato dall'UI
    mirrorExternalDeckRemoveByName(name);
