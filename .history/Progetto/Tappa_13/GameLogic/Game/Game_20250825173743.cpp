...
void Game::startTurn(){
    ...
    // Le carte M/T settate in un turno precedente diventano attivabili: azzera il flag "settata in questo turno"
    auto &stSet = stSetThisTurn[cur];
    std::fill(stSet.begin(), stSet.end(), false);
    }
    if(drawCtrl){
...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    if(turn.getPhase() != GamePhase::Main1 && turn.getPhase() != GamePhase::Main2) return false;
    // Attiva: manda al Cimitero dopo la risoluzione (per ora immediata)
    Card c = hand[handIndex];
    Attribute attr = c.getAttribute();
    hand.erase(hand.begin()+handIndex);
    // Emissione evento
    dispatcher.emit(attr == Attribute::Trappola ? GameEventType::TrapActivated : GameEventType::SpellActivated);
    // Effetto risolto: sposta al Cimitero (per continue si dovrebbe restare in campo, ma lo vedremo dopo)
    graveyard[turn.getCurrentPlayerIndex()].push_back(std::move(c));
    lastSentToGrave = LastSentToGrave{ graveyard[turn.getCurrentPlayerIndex()].back().getName(), turn.getCurrentPlayerIndex() };
    dispatcher.emit(GameEventType::CardSentToGrave);
    dispatcher.emit(GameEventType::CardMoved);
    return true;
}
...
bool Game::activateSetSpellTrap(size_t zoneIndex){
    int cur = turn.getCurrentPlayerIndex();
    if(zoneIndex >= spellTraps[cur].size()) return false;
    // Deve essere scoperta per attivarsi (al momento è coperta, l'attivazione la scopre)
    bool isFD = (zoneIndex < stIsFaceDown[cur].size() ? stIsFaceDown[cur][zoneIndex] : false);
    // Le Trappole e Magie Rapide settate non possono essere attivate nel turno in cui sono state settate
    bool setThisTurn = (zoneIndex < stSetThisTurn[cur].size() ? stSetThisTurn[cur][zoneIndex] : false);
    Attribute attr = spellTraps[cur][zoneIndex].getAttribute();
    const auto& features = spellTraps[cur][zoneIndex].getFeatures();
    ...
    // Fase: per semplicità, consenti attivazione durante Main1/Main2 del proprietario (estendere a turni avversario con chain system)
    if(turn.getPhase() != GamePhase::Main1 && turn.getPhase() != GamePhase::Main2) return false;
    // Scopri
    if(zoneIndex < stIsFaceDown[cur].size()) stIsFaceDown[cur][zoneIndex] = false;
    dispatcher.emit(attr == Attribute::Trappola ? GameEventType::TrapActivated : GameEventType::SpellActivated);
    // Risolvi: se Non-Continua, manda al Cimitero; se Continua, resta scoperta
    bool isContinuous = std::find(features.begin(), features.end(), Feature::Continua) != features.end();
    if(!isContinuous){
        Card c = spellTraps[cur][zoneIndex];
        spellTraps[cur].erase(spellTraps[cur].begin()+zoneIndex);
        if(zoneIndex < stIsFaceDown[cur].size()) stIsFaceDown[cur].erase(stIsFaceDown[cur].begin()+zoneIndex);
        if(zoneIndex < stSetThisTurn[cur].size()) stSetThisTurn[cur].erase(stSetThisTurn[cur].begin()+zoneIndex);
