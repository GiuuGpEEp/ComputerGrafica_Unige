...
void DrawController::update(
    ...
    if (cardsToDraw > 0 && animations.empty() && !deck.isEmpty()) {
        ...
        // If forcing skip reveal (opponent draw), ensure the card inside the animation uses the back texture immediately
        if (forceSkipReveal) {
            animations.back().getCard().setTexture(textureFlipped);
        }
        if(dispatcher) dispatcher->emit(GameEventType::DrawStart);
    }
    if (!animations.empty()) {
        bool skipPause = (mousePressed || forceSkipReveal);
        DrawAnimationPhases ph = animations.front().update(MOVEMENT_SPEED_INTERNAL, deltaTime, textureFlipped, hand, windowSize, cardSize, spacing, handY, handMaxSize, skipPause, textureManager);
