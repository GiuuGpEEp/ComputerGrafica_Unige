...
- Game
  ...
  - Modificato `activateSpellFromHand(...)`: per carte non-continue con `IActivationEffect`, al posto di chiamare il metodo resolve() per risolvere l'effetto usa `startChainWithActivation` e sposta la carta al Cimitero; niente `resolve()` immediato.
- EffectSystem
