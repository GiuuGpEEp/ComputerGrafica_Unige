...
- Correzione effetto `Pietra bianca della leggenda`:
 ...
 - Comportamento `Reliquiario del Drago` (Deck send choice):
    ...
    - Dettagli tecnici:
      ...
      - Dopo che la carta Drago è mandata al Cimitero con `resolvePendingSendFromDeck`, l'effetto del Reliquiario continua e il Reliquiario stesso viene spostato dalla Spell/Trap Zone al Cimitero tramite il flusso esistente.
    Quindi quando viene attivata la magia l'effetto della carta (gestito nella cartella effects) richiama la funzione requestSendFromDeck in game, che scatenerà un evento di tipo DeckSendChoiceRequested
