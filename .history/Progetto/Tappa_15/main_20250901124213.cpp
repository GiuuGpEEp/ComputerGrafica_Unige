...
int main(){
    ...
    bool selectedCardOnOppField = false;
    ...
    while (window.isOpen()) {
                ...
                if (!cardClicked) {
                    for (size_t i = 0; i < fieldCards.size(); ++i) {
                        if (fieldCards[i].isClicked(mousePos)) {
                            ...
                            break;
                        } if(oppFieldCards[i].isClicked(mousePos)) {
                            // Cliccata una carta avversaria: mostra dettagli ma non selezionabile
                            cardClicked = true;
                            selectedCardIndex = i; 
                            selectedCardOnOppField = true; // Carta sul campo
                            scrollOffset = 0.f;
                            // Non fare altro (non selezionabile)
                            break;
                        }    
                    }
                }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if (selectedCardIndex.has_value() && !extraOverlay.isOverlayVisible() && !graveyardOverlay.isOverlayVisible()) {
                ...
                } else if (selectedCardOnOppField && selectedCardIndex.value() < oppFieldCards.size()) {
                    selectedCard = &oppFieldCards[selectedCardIndex.value()];
                }
