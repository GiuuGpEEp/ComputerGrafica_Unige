...
GraveyardOverlay::GraveyardOverlay(const sf::Vector2u& winSize, const sf::Vector2f& slotSz, TextureManager& tm)
: windowSize(winSize), slotSize(slotSz), textureManager(tm) {}
...
void GraveyardOverlay::startHold(const sf::Vector2f& mousePos, const sf::FloatRect& gySlotBounds, GameState state, bool hasCards, int owner){
    ...
    if(state == GameState::FieldVisible || state == GameState::Playing) {
        if(gySlotBounds.contains(mousePos)) {
            currentOwner = owner;
            if(!showOverlay) { 
                holding = true; holdClock.restart(); 
            }else{ 
                holding = false; 
            }
        }
...
void GraveyardOverlay::handleMousePress(const sf::Vector2f& mousePos, const std::vector<Card>& gyCards){
    ...
    // Use the same ordering as draw() to map the clicked slot index to the correct card
    std::vector<Card> ordered = gyCards;
    for(size_t i=0; i<cardBounds.size(); ++i){
        if(cardBounds[i].contains(mousePos)){
            size_t idx = i;
            if(idx < ordered.size()){
                selectedCard = ordered[idx];
...
void GraveyardOverlay::handleMouseRelease(){ 
    holding = false; 
}
void GraveyardOverlay::handleScroll(float delta){ 
    if(detailsShown){ 
        scrollOffset -= delta * 20.f; 
        if(scrollOffset < 0.f) scrollOffset = 0.f; 
    } 
}
void GraveyardOverlay::handleEscape(){ 
    if(detailsShown) detailsShown = false; 
    else if(showOverlay) closeOverlay(); 
}
void GraveyardOverlay::openOverlay(){ 
    showOverlay = true; 
}
void GraveyardOverlay::closeOverlay(){ 
    showOverlay = false; 
    detailsShown = false; 
    holding = false; 
}

void GraveyardOverlay::reset(){
    showOverlay = false;
    detailsShown = false;
    holding = false;
    cardBounds.clear();
...
bool GraveyardOverlay::isOverlayVisible() const { 
    return showOverlay; 
}
bool GraveyardOverlay::detailsActive() const { 
    return detailsShown; 
}
const Card* GraveyardOverlay::getSelectedCard() const { 
    return detailsShown ? &selectedCard : nullptr; 
}

int GraveyardOverlay::getCurrentOwner() const { return currentOwner; }
