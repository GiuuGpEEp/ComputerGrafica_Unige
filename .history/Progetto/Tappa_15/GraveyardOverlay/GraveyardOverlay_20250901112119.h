...
public:
    ...
    GraveyardOverlay(const sf::Vector2u& windowSize, const sf::Vector2f& slotSize, TextureManager& texMgr);
    GraveyardOverlay(const sf::Vector2u& windowSize, const sf::Vector2f& slotSize, TextureManager& texMgr, const Config& cfg);

    void startHold(const sf::Vector2f& mousePos, const sf::FloatRect& gySlotBounds, GameState state, bool hasCards, int owner);
    ...
    // Reset completo dello stato dell'overlay (chiude overlay, cancella selezione e cache)
    void reset();
