...
public:
    ...
    // Accesso al Deck (copia const) per ricerca; e helper per aggiungere in mano
    ...
    // Sync helper: quando effetti rimuovono carte dal Deck logico (non tramite DrawController),
    // riflette la rimozione anche nel Deck esterno usato dall'UI (se collegato)
    // Rimuove visualmente dal Deck esterno (UI) la prima carta con il nome indicato.
    // Se `sourceDeck` e' fornito, la rimozione sul Deck esterno avviene solo se
    // `externalDeck` non punta alla stessa istanza per evitare double-remove.
...
private:
    ...
    // Collega il Deck esterno usato nel layer UI per un giocatore specifico
    void attachExternalDeck(Deck* deckPtr, int playerIdx);
    // Vecchia firma compat: collega il deck come external per il player 0
    void attachExternalDeck(Deck* deckPtr) { attachExternalDeck(deckPtr, 0); }
