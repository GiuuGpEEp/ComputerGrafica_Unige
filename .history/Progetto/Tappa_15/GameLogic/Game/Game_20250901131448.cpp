...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    if(act){ effects.pushActivationRequest(hand[handIndex].getName(), ownerIdx); }
    // Risolvi eventuali attivazioni impilate (LIFO)
...
bool Game::activateSetSpellTrapFor(int playerIdx, size_t zoneIndex){
    ...
    if(act){ effects.pushActivationRequest(spellTraps[playerIdx][zoneIndex].getName(), playerIdx); }
    effects.resolveAllActivations(*this);
