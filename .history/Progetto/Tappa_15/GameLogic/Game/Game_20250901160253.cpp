...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    // Se l'oggetto continuo ha anche una risoluzione on-activate, pusha la richiesta
        ...
        // Carta non-continua: se ha un effetto di attivazione, apri una response window
        // (push dell'attivazione nella catena) e poi sposta la carta al Cimitero
        // subito come nel caso di attivazione da zona. La risoluzione verrà eseguita
        // più tardi quando la chain verrà risolta (LIFO), così la UI di scelta
        // dell'effetto non compare prima della finestra di risposta.
        if(act){
            // usa il nome della carta temporanea prima di muoverla
