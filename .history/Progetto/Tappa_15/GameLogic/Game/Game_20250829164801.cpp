...
void Game::mirrorExternalDeckRemoveByName(const std::string& name, const Deck* sourceDeck){
    // If no external decks attached, nothing to mirror
    ...
    if(sourceDeck){
        for(int i=0;i<2;++i){
            if(externalDecks[i] && externalDecks[i] == sourceDeck){
                // Same instance: assume UI already reflects removal or will be driven by same reference
                return;
            }
        }
    }
    // Otherwise, conservatively remove from both external decks (visual only)
    for(int i=0;i<2;++i){
        if(externalDecks[i]){
