...
void Game::startChainWithActivation(const std::string& cardName, int ownerIdx){
    ...
    std::cout << "[DEBUG] Game::startChainWithActivation - started chain with '" << cardName << "' owner=" << ownerIdx << std::endl;
...
bool Game::playerRespondWithActivation(int playerIdx, const std::string& cardName){
    ...
    // Signal UI/UI event
    std::cout << "[DEBUG] Game::playerRespondWithActivation - player " << playerIdx << " responded with '" << cardName << "'" << std::endl;
    dispatcher.emit(GameEventType::SpellActivated);
    return true;
}
...
void Game::playerPassOnChain(int playerIdx){
    ...
    std::cout << "[DEBUG] Game::playerPassOnChain - both players passed, resolving chain" << std::endl;
    effects.resolveAllActivations(*this);
        chainActive = false;
        lastResponder = -1;
        // After full resolution, move any pending cards to graveyard
        for(int owner = 0; owner < 2; ++owner){
            if(pendingMoveToGrave[owner].empty()) continue;
            for(auto &card : pendingMoveToGrave[owner]){
                graveyard[owner].push_back(std::move(card));
                lastSentToGrave = LastSentToGrave{ graveyard[owner].back().getName(), owner };
                dispatcher.emit(GameEventType::CardSentToGrave);
            }
            pendingMoveToGrave[owner].clear();
            dispatcher.emit(GameEventType::CardMoved);
        }
    } else {
...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    // Se l'oggetto continuo ha anche una risoluzione on-activate, pusha la richiesta
        if(act){
            ...
            // Nota: non usare hand[handIndex] qui perché la carta è stata rimossa dalla mano.
            // Recupera il nome dall'oggetto appena inserito nella zona ST.
            startChainWithActivation(spellTraps[cur].back().getName(), ownerIdx);
        }
    } else {
        // Carta non-continua: se ha effetto di attivazione, pusha l'attivazione e rimanda
        // lo spostamento al Cimitero fino alla risoluzione completa della chain.
        if(act){
            startChainWithActivation(c.getName(), ownerIdx);
            // Accoda la carta da mandare al grave dopo che la chain sarà risolta
            pendingMoveToGrave[ownerIdx].push_back(std::move(c));
            return true;
        }
        // Se non ha effetto registrato, risolvi immediatamente
        bool ok = true;
        if(act){ ok = act->resolve(*this, ownerIdx); }
        int cur = turn.getCurrentPlayerIndex();
...
bool Game::activateSetSpellTrapFor(int playerIdx, size_t zoneIndex){
    ...
    // Esegui risoluzione tramite chain: pusha e risolvi LIFO
        if(act){
            ...
            // move to pending instead of immediate grave
            pendingMoveToGrave[playerIdx].push_back(std::move(c));
            ...
            // do not emit CardSentToGrave now; will be emitted when pending flushed
        } else {
            // no activation effect: move immediately to grave as before
            spellTraps[playerIdx].erase(spellTraps[playerIdx].begin()+zoneIndex);
            if(zoneIndex < stIsFaceDown[playerIdx].size()) stIsFaceDown[playerIdx].erase(stIsFaceDown[playerIdx].begin()+zoneIndex);
            if(zoneIndex < stSetThisTurn[playerIdx].size()) stSetThisTurn[playerIdx].erase(stSetThisTurn[playerIdx].begin()+zoneIndex);
