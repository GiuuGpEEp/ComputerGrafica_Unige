...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    // Se l'oggetto continuo ha anche una risoluzione on-activate, pusha la richiesta
        ...
        int cur = turn.getCurrentPlayerIndex();
        // Prova a piazzare la carta nella zona M/T scoperta come fatto per le carte continue
        if(spellTraps[cur].size() >= ST_ZONE_SIZE){
            // Nessuno spazio: rimetti in mano per non perdere la carta
            hand.insert(hand.begin()+handIndex, c);
            return false;
        }
        // Metti la carta in campo scoperta (simula l'attivazione dalla mano)
        spellTraps[cur].push_back(std::move(c));
        stIsFaceDown[cur].push_back(false);
        stSetThisTurn[cur].push_back(false);
        dispatcher.emit(GameEventType::CardMoved);
        // Avvia la response window per questa attivazione
        if(act){
            startChainWithActivation(spellTraps[cur].back().getName(), ownerIdx);
        } else {
            // Nessun effetto di attivazione: rimuovi la carta immediatamente al Cimitero
            Card temp = std::move(spellTraps[cur].back());
            spellTraps[cur].pop_back();
            if(cur < stIsFaceDown[cur].size()) stIsFaceDown[cur].pop_back();
            if(cur < stSetThisTurn[cur].size()) stSetThisTurn[cur].pop_back();
            graveyard[cur].push_back(std::move(temp));
            lastSentToGrave = LastSentToGrave{ graveyard[cur].back().getName(), cur };
        ...
        // Per le carte non-continue, dopo aver avviato la chain rimuoviamo la carta dal campo
        // e la mandiamo al Cimitero (comportamento equivalente ad attivazione da zona)
        Card finish = std::move(spellTraps[cur].back());
        spellTraps[cur].pop_back();
        if(cur < stIsFaceDown[cur].size()) stIsFaceDown[cur].pop_back();
        if(cur < stSetThisTurn[cur].size()) stSetThisTurn[cur].pop_back();
        graveyard[cur].push_back(std::move(finish));
        lastSentToGrave = LastSentToGrave{ graveyard[cur].back().getName(), cur };
        std::cout << "[DEBUG] Game::activateSpellFromHand - lastSentToGrave name='" << lastSentToGrave->name << "' owner=" << lastSentToGrave->ownerIdx << " (activateSpellFromHand)" << std::endl;
        dispatcher.emit(GameEventType::CardSentToGrave);
        dispatcher.emit(GameEventType::CardMoved);
