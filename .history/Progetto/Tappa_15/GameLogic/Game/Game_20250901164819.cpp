...
bool Game::activateSpellFromHand(size_t handIndex){
    ...
    // Se l'oggetto continuo ha anche una risoluzione on-activate, pusha la richiesta
        ...
        // Carta non-continua: se ha un effetto di attivazione, pusha la richiesta nella chain
        // e poi sposta la carta al Cimitero; la risoluzione effettiva avverrà quando la
        // chain verrà risolta (LIFO). Questo evita che la UI specifica dell'effetto
        // venga mostrata prima della response window.
        if(act){
            startChainWithActivation(c.getName(), ownerIdx);
        } else {
            // Nessun effetto da risolvere: non c'è nulla da pusha, ma comunque
            // la carta va mandata al cimitero (effetto vuoto)
        }
        int cur = turn.getCurrentPlayerIndex();
