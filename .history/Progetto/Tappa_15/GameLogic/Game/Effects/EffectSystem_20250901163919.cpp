...
void EffectSystem::pushActivationRequest(const std::string& cardName, int ownerIdx){
    ...
    // debug
    std::cout << "[DEBUG] EffectSystem::pushActivationRequest - pushed '" << cardName << "' owner=" << ownerIdx << " chainSize=" << activationChain.size() << std::endl;
}
...
bool EffectSystem::resolveTopActivation(Game& game){
    ...
    std::cout << "[DEBUG] EffectSystem::resolveTopActivation - resolving '" << req.cardName << "' owner=" << req.ownerIdx << " remainingChainSize=" << activationChain.size() << std::endl;
    bool res = act->resolve(game, req.ownerIdx);
    std::cout << "[DEBUG] EffectSystem::resolveTopActivation - resolved '" << req.cardName << "' result=" << res << std::endl;
    return res;
}

void EffectSystem::resolveAllActivations(Game& game){
    while(!activationChain.empty()){
        // resolveTopActivation pops the top
