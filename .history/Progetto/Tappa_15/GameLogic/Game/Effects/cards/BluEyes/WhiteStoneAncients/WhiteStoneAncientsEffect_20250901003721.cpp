...
void WhiteStoneAncientsEffect::onEvent(GameEventType type, Game& game){
    ...
    if(type == GameEventType::CardSentToGrave){
        ...
        if(last->name == std::string("Pietra Bianca degli Antichi")){
            // Determine which player's graveyard actually received the card now
            int actualOwner = -1;
            for(int i=0;i<2;++i){
                auto &gy = game.getGraveyardOf(i);
                if(!gy.empty() && gy.back().getName() == last->name){ actualOwner = i; break; }
            }
            if(actualOwner < 0) {
                // Fallback: use the recorded ownerIdx from lastSentToGrave if present
                actualOwner = last->ownerIdx;
            }
    ...
    if(type == GameEventType::PhaseChange){
        ...
        for(int i=0;i<std::min(8, deck.getSize()); ++i){
            auto maybe = deck.findFirst([&](const Card& c){ return false; }); // dummy to access internals via collectWhere not available here
        }
