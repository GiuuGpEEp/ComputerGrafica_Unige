...
void DrawController::update(
    ...
    if (!animations.empty()) {
    // mousePressed: permette uscita anticipata dall'attesa al centro; forceSkipReveal: salta completamente la rivelazione al centro
    DrawAnimationPhases ph = animations.front().update(MOVEMENT_SPEED_INTERNAL, deltaTime, textureFlipped, hand, windowSize, cardSize, spacing, handY, handMaxSize, mousePressed, forceSkipReveal, textureManager);
        ...
        if (ph == DrawAnimationPhases::Done) {
            ...
            // Prepara la carta finale da aggiungere alla mano: assicura la texture frontale per mani visibili,
            // o la texture posteriore per mani nascoste (forceSkipReveal indica pescata dell'avversario)
