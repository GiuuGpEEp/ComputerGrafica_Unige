## Aggiornamento 2025-09-01 — Correzioni e note

- Fix: emissione evento `TurnStart` spostata dopo l'azzeramento dei flag `stSetThisTurn` per il giocatore che ha appena concluso il turno. Questo evita che il rebuild della UI legga uno stato transitorio e nasconda Quick Spells/Traps che dovrebbero essere attivabili come risposta.
- Fix: `Pianto di Argento` aggiornato per usare `Game::getGraveyardOf(ownerIdx)` invece di API relative al giocatore corrente; questo garantisce che l'effetto cerchi il bersaglio nel cimitero del proprietario della carta.
- Temporanei messaggi di debug sono stati aggiunti per tracciare `Pianto di Argento::canActivate`/`resolve` durante il test e sono stati rimossi dopo la verifica.

Per dettagli tecnici e file toccati, vedere le sezioni più sotto in questo README.
