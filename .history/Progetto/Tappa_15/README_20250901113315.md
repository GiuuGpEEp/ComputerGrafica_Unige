...
- Overlay Cimitero (hold on opponent): aggiunta la possibilità di premere/tenere sullo slot cimitero dell'avversario per aprire l'overlay che mostra il suo graveyard.

  Dettagli implementativi principali:
  - `Input::Context` ora espone anche la posizione dello slot top (`graveyardSlotPosTop`) oltre a quello bottom. Questo permette di distinguere i due slot durante l'hit-test.
  - `InputController` esegue l'hit-test su entrambi i rect (bottom/top). Se il mouse è dentro lo slot top passa `owner=1`, se è dentro lo slot bottom passa `owner=0`.
  ...
  - Convenzione owner: 0 = bottom (P1), 1 = top (P2). Questa mappatura è stata mantenuta coerente con le API esistenti (`getGraveyardOf(int ownerIdx)`).

  Comportamento e fallback:
  - Se il click/hold non è su nessuno dei due rect (es. area intrn), l'overlay mantiene il comportamento precedente (usa la funzione `getGraveyardCards` fornita dal `main` come fallback).
  - Se `game` non è inizializzato al momento del press, il codice evita crash e non apre l'overlay.

  Come testare rapidamente:
  1. Avvia una partita; assicurati che ci siano carte nel graveyard di entrambi i giocatori.
  2. Tieni premuto (hold) sullo slot cimitero in basso: deve apparire l'overlay con il tuo cimitero.
  3. Tieni premuto sullo slot cimitero in alto: ora l'overlay deve mostrare le carte del giocatore in alto (avversario).
  4. Prova click breve, apertura dettagli e rilascio per verificare che non ci siano regressioni.

Verifiche già eseguite:
- Compilazione locale: nessun errore sui file coinvolti.
- Test rapido in-app: overlay P2 mostra ora il graveyard corretto quando si tiene premuto sul suo slot.

Note:
- Ho rimosso i log di debug temporanei dopo la verifica; se vuoi, posso riattivarli temporaneamente per indagare casi specifici.
