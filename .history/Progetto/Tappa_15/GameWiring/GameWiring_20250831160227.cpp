...
void createGameAndAttachResources(
    ...
    // Pulisce le cache UI di ST/FieldSpell
    p1STCards.clear();
    p2STCards.clear();
...
void setupAppHandlers(
    ...
    if(game){
        ...
        // Imposta il limite della mano e la callback per lo scarto (usa lo stesso limite di main: 7)
        game->setHandLimit(7);
        game->setDiscardCallback([&](std::vector<Card>&& excess){
            if(!handPtr) return;
            if(excess.empty()) return;
            // Determina il proprietario (indice assoluto del giocatore) la cui mano ha prodotto l'eccesso (0 = P1, 1 = P2)
            ...
            int fieldPlayer = ownerIdx + 1; // converte 0->1, 1->2
            // Calcola la posizione dello slot del cimitero per quel proprietario
            ...
            // Avvia l'animazione di scarto verso il cimitero specifico del proprietario
            discardController.start(std::move(excess), gyCenter, ownerIdx);
            if(updateHandLayout) updateHandLayout();
            std::cout << "[AutoDiscard] Scarto animato avviato per proprietario: " << ownerIdx << std::endl;
        });
    }
}
