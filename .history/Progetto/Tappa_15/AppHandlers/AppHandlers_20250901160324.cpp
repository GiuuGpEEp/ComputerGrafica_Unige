...
void AppHandlers::attachGameHandlers(Game &g, const Context &ctx){
    ...
    g.events().subscribe(GameEventType::SpellActivated, [&](){ if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Magia attivata"); if(ctx.syncSpellTrapZones) ctx.syncSpellTrapZones(); if(ctx.rebuildResponseActivables) { ctx.rebuildResponseActivables(); } if(ctx.openResponsePromptIfAny) ctx.openResponsePromptIfAny(); if(ctx.closeResponsePrompt) { /* handled by caller if empty */ } });
    g.events().subscribe(GameEventType::TrapActivated, [&](){ if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Trappola attivata"); if(ctx.syncSpellTrapZones) ctx.syncSpellTrapZones(); if(ctx.rebuildResponseActivables) { ctx.rebuildResponseActivables(); } if(ctx.openResponsePromptIfAny) ctx.openResponsePromptIfAny(); if(ctx.closeResponsePrompt) { /* handled by caller if empty */ } });
    ...
    g.events().subscribe(GameEventType::DeckSendChoiceRequested, [&](){
        ...
        // Before opening the deck-send UI, give the response prompt a chance to appear.
        // Callbacks may set a modal prompt; if so, defer the deck UI until chain resolution.
        if(ctx.rebuildResponseActivables) ctx.rebuildResponseActivables();
        if(ctx.openResponsePromptIfAny) ctx.openResponsePromptIfAny();
        // If the response prompt opened, do not show deck-send UI now; it will be shown
        // after the chain resolves (pendingSend remains set in Game).
        // We detect this by checking the callback state: if ctx.setDeckSendChoiceActive is not
        // being invoked now and the prompt is active, skip opening.
    ...
    // Otherwise (owner is human) open the UI panel as before, but only if the
    // response prompt isn't currently active (we rely on the prompt callbacks above).
    // There's no direct way to query responsePromptActive here, so we make a best-effort
    // heuristic: if openResponsePromptIfAny was called and resulted in a prompt, the UI
    // should have become modal and inputBlocked; in that case we avoid opening deck UI.
    // For simplicity, if openResponsePromptIfAny returned without clearing the pending send,
    // we still proceed. This is a pragmatic compromise for now.
    ...
    // file-scope RNG for AI decisions
    static std::mt19937 _apphandlers_rand(std::random_device{}());
