...
int main(){
    ...
    auto rebuildResponseActivables = [&](){
        ...
        const auto &ownerZoneRef = (responseOwner == cur) ? game->getSpellTrapZone() : game->getOpponentSpellTrapZone();
        for (size_t i = 0; i < Game::ST_ZONE_SIZE; ++i) {
            bool can = game->canActivateSetSpellTrapFor(responseOwner, i);
            // Debug print: slot presence and canActivate result
            if (i >= ownerZoneRef.size()) {
                std::cout << "[RESP DEBUG] owner=" << responseOwner << " slot=" << i << " -- no card present; can=" << can << std::endl;
                if (!can) continue;
            } else {
                const Card &cd = ownerZoneRef[i];
                const auto &feats = cd.getFeatures();
                bool isQuick = std::find(feats.begin(), feats.end(), Feature::Rapida) != feats.end();
                Attribute attr = cd.getAttribute();
                std::cout << "[RESP DEBUG] owner=" << responseOwner << " slot=" << i
                          << " name='" << cd.getName() << "' attr=" << (attr==Attribute::Trappola?"Trap":"Spell")
                          << " isQuick=" << isQuick << " can=" << can << std::endl;
                if (!can) continue;
                // Defensive UI check: when it's the opponent's turn, ensure only Traps or Quick spells are listed
                if (cur != responseOwner) {
                    if (!(attr == Attribute::Trappola || isQuick)) {
                        std::cout << "[RESP DEBUG] slot " << i << " filtered out by UI rule (not Trap or Quick)" << std::endl;
                        continue;
                    }
                }
            }
        ...
        // Print summary of activables for easier runtime diagnosis
        std::cout << "[RESP DEBUG] summary: responseActivables.size=" << responseActivables.size();
        if(!responseActivables.empty()){
            std::cout << " contents=";
            for (size_t idx : responseActivables) std::cout << idx << ",";
        }
        std::cout << std::endl;
    };
    ...
    std::cout << "[RESP DEBUG] openResponsePromptIfAny after rebuild: cur=" << cur << " responseOwner=" << responseOwner << " activables=" << responseActivables.size() << std::endl;
        if (!responseActivables.empty()) {
