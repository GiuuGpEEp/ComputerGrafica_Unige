...
## Aggiornamento recente: comportamento e pulizia debug per `Pietra Bianca degli Antichi`

- L'effetto di `Pietra Bianca degli Antichi` è stato modificato: ora, se in un dato turno la carta viene mandata al cimitero (con qualsiasi metodo), il suo effetto verrà eseguito nell'End Phase di quello stesso turno indipendentemente dal fatto che sia l'ultima carta mandata al cimitero o meno.
- Sono stati rimossi i log di debug aggiunti durante le fasi precedenti di sviluppo dall'implementazione dell'effetto (`std::cout` eliminati). Questo rende l'output più pulito durante l'esecuzione normale.
- I commenti nel sorgente dell'effetto sono stati tradotti in italiano per maggiore leggibilità del codice nel contesto del progetto.
- La logica mantiene la difesa che preferisce `lastSentToGrave.ownerIdx` come fonte primaria dell'ownership e, se necessario, esegue una scansione difensiva dei cimiteri per trovare la carta.

Se vuoi, posso aggiungere una piccola nota o un test descrittivo che mostra passo-passo come riprodurre lo scenario (es.: distruggi la pietra durante il turno e verifica che la Special Summon venga proposta al proprietario in End Phase).
