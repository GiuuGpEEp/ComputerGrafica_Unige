...
Dopo aver sistemato un minimo il codice, in questa tappa cerco di fixare tutte le problematiche individuate all'interno del pro
...
- Correzione texture per effetti che aggiungono carte in mano
  - Il problema: quando l'effetto di "Pietra Bianca della Leggenda" aggiungeva un "Drago Bianco Occhi Blu" dalla pesca diretta dal Deck alla mano, la carta appariva con la texture del retro perché il percorso effetto bypassava il `DrawController` (che normalmente applica la texture frontale). 
  - Soluzione introdotta: l'effetto `WhiteStoneLegendEffect` ora riceve un riferimento a `TextureManager` (tramite il wiring in `GameWiring`/`main.cpp`) e chiama `RenderUtils::applyFrontTextureIfAny(...)` prima di aggiungere la carta alla mano. Questo garantisce che la carta inserita dalla logica dell'effetto abbia la texture frontale corretta.
  - Nota: la soluzione è localizzata all'effetto per minimizzare l'impatto; se preferisci una normalizzazione centrale, si può spostare la logica dentro `Game::addToHand` in una modifica successiva.
