...
## Aggiornamento 2025-09-01 — Fix rapidi e UI

Breve riepilogo delle modifiche applicate il 2025-09-01 per risolvere bug di timing/effetti e migliorare l'UX del cimitero:

- TurnStart/event ordering: la emissione dell'evento di inizio turno è stata rimossa prima della pulizia di alcuni flag interni (es. `stSetThisTurn`) e adesso viene emessa dopo che lo stato interno è coerente — questo evita che la UI ricostruisca la lista delle risposte con stato obsoleto.
- `Pianto di Argento`: l'effetto ora accede esplicitamente al graveyard del proprietario (`getGraveyardOf(ownerIdx)`) risolvendo casi in cui il bersaglio veniva cercato nel graveyard del turno corrente per errore.
- Overlay Cimitero (hold on opponent): aggiunta la possibilità di premere/tenere sullo slot cimitero dell'avversario per aprire l'overlay che mostra il suo graveyard; il controller di input ora determina quale slot (bottom/top) è stato premuto e passa a `GraveyardOverlay` le carte corrette.

Verifiche eseguite:
- Compilazione locale: non sono emersi errori di compilazione sui file modificati.
- Test rapido: confermato che il prompt di risposta mostra ora le carte Quick/Trap attivabili al momento giusto e che l'overlay del cimitero mostra le carte dell'avversario quando si tiene premuto sul suo slot.

Note:
- Sono stati rimossi i log di debug temporanei dopo la verifica; se vuoi i log per ulteriori diagnostiche posso reinserirli temporaneamente.
