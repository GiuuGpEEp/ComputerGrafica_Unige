...
- Overlay Cimitero: aggiunta la possibilità di tener premuto sullo slot cimitero dell'avversario per aprire l'overlay che mostra il suo graveyard.

  Dettagli implementativi principali:
  - Ho aggiornato la struct `Input::Context` inserendo anche la posizione dello slot top (`graveyardSlotPosTop`) oltre a quello bottom, che viene passato dal main. Questo permette di distinguere i due slot durante l'hit-test.
