...
int main(){
    ...
    auto openResponsePromptIfAny = [&](){
        if (!game) return;
        // Apri il prompt solo se esiste una chain attiva nel modello di gioco
        if(!game->isChainActive()) return;
        int cur = game->getTurn().getCurrentPlayerIndex();
        // Il giocatore che ha la priorità per rispondere e' l'altro giocatore (non-current)
        responseOwner = 1 - cur;
        ...
        if (!responseActivables.empty()) {
            ...
            if(responsePromptActive) RenderUtils::closeResponsePrompt();
        }
    };
