...
- Correzione effetto `Pietra bianca della leggenda`:
 ...
 - Comportamento `Pietra bianca degli antichi` (Evocazione Speciale con scelta posizione):
 	...
 	- Modifica introdotta in Game: ho aggiunto un getter `Game::getPendingSpecialSummonOwner()` in modo tale da capire chi sta effettuando l'evocazione speciale tra il player e l'AI, così facendo distinguo le due logiche e ho aggiornato il gestore in `AppHandlers` che risponde a `SpecialSummonChoiceRequested`.
