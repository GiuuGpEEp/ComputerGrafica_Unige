...
- Correzione effetto `Pietra bianca della leggenda`:
 ...
 - Comportamento `Pietra bianca degli antichi` (Evocazione Speciale con scelta posizione):
 	...
 	- Modifica introdotta in Game: ho aggiunto un getter `Game::getPendingSpecialSummonOwner()` in modo tale da capire chi sta effettuando l'evocazione speciale tra il player e l'AI, così facendo distinguo le due logiche --> aggiornamento della logica `AppHandlers` in risposta ad un evento `SpecialSummonChoiceRequested`:
 		- Se il proprietario è il giocatore umano (P1) il comportamento UI resta invariato: viene mostrato il prompt per scegliere Attacco o Difesa (`ctx.setSsChoiceActive(true)`).
 		- Se il proprietario è il giocatore AI (P2) l'AI risolve automaticamente la scelta senza aprire il prompt. In questa prima implementazione l'AI seleziona autonomamente Attacco o Difesa con una scelta casuale (file-scope RNG in `AppHandlers`).
 	- Motivazione e dettagli tecnici: usare `getPendingSpecialSummonOwner()` evita di basarsi sul `turn.getCurrentPlayerIndex()` al momento dell'evento (possibili discrepanze di contesto). La risoluzione chiama `Game::resolvePendingSpecialSummon(choiceDefense)` che esegue la `specialSummonToMonsterZone(...)` con la posizione scelta.
 	- Possibili evoluzioni: la scelta AI è attualmente casuale; possiamo sostituirla con una piccola euristica (board-aware) o rendere il seed RNG configurabile per test riproducibili.
