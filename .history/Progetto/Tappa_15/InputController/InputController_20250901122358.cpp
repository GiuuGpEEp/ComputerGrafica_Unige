        ...
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
            if(mbp->button == M_MOUSE_LEFT){
                ...
                if(state.getGame){
                    ...
                    // If none selected yet, check P2 ST row (absolute owner 1)
                    if(!state.selectedCardIsSpellTrap){
                        for(size_t i=0;i<state.p2STCards.size();++i){
                            if(state.p2STCards[i].getGlobalBounds().contains(mousePosF)){
                                state.selectedCardIndex = i;
                                state.selectedCardIsOnField = false;
                                state.selectedCardIsSpellTrap = true;
                                state.selectedCardSTOwner = 1;
            ...
            if(kp->code == M_KEY_LEFT || kp->code == M_KEY_UP){
                if(!state.deckSendCandidates.empty()){
                    ...
                    if(kp->code == M_KEY_LEFT){
                        ...
                        if(state.deckSendSelected >= (int)cols) state.deckSendSelected -= cols;
                    }
                }
                return true;
            }
            if(kp->code == M_KEY_RIGHT || kp->code == M_KEY_DOWN){
                if(!state.deckSendCandidates.empty()){
                    ...
                    if(kp->code == M_KEY_RIGHT){
                        if(state.deckSendSelected + 1 < (int)state.deckSendCandidates.size()) state.deckSendSelected++;
                    } else { // Down
                        if(state.deckSendSelected + (int)cols < (int)state.deckSendCandidates.size()) state.deckSendSelected += cols;
                    }
                }
                return true;
            }
