    ...
    if(!state.inputBlocked() && (e.is<sf::Event::MouseButtonPressed>() || e.is<sf::Event::MouseButtonReleased>())){
        ...
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
            if(mbp->button == M_MOUSE_LEFT){
                ...
                // Decide se l'hold/click sul cimitero riguarda il player basso (P1) o quello alto (P2)
                bool gyHasCardsP1 = false;
                bool gyHasCardsP2 = false;
                if(state.getGame){ 
                    if(Game* g = state.getGame()){
                        gyHasCardsP1 = !g->getGraveyard().empty();
                        gyHasCardsP2 = !g->getOpponentGraveyard().empty();
                    }
                }
                // Area per il cimitero avversario (top) -> usa graveyardSlotPosTop
                sf::FloatRect gyRectTop(state.graveyardSlotPosTop, state.slotSize);
                // Start hold sull'overlay condiviso: passiamo il flag hasCards relativo allo slot sotto il mouse
                if(gyRectTop.contains(mousePosF)){
                    state.graveyardOverlay.startHold(mousePosF, gyRectTop, state.gamestate, gyHasCardsP2);
                } else {
                    state.graveyardOverlay.startHold(mousePosF, gyRect, state.gamestate, gyHasCardsP1);
                }
                ...
                if(gyRectTop.contains(mousePosF)){
                    if(state.getOpponentGraveyardCards) state.graveyardOverlay.handleMousePress(mousePosF, state.getOpponentGraveyardCards());
                    else state.graveyardOverlay.handleMousePress(mousePosF, std::vector<Card>{});
                } else {
                    if(state.getGraveyardCards) state.graveyardOverlay.handleMousePress(mousePosF, state.getGraveyardCards());
                    else state.graveyardOverlay.handleMousePress(mousePosF, std::vector<Card>{});
                }
