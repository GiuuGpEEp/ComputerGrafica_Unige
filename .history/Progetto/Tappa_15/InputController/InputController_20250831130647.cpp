...
bool Controller::handleEvent(const sf::Event& e){
    ...
    if(kp3->code == M_KEY_SPACE && state.gamestate == GameState::Playing){
        ...
        // Se stiamo selezionando tributi, Invio conferma la scelta
        //if(kp3->code == M_KEY_ENTER && state.gamestate == GameState::Playing && state.selectingTributes){
        //    Game* game = state.getGame ? state.getGame() : nullptr;
        //    if(game){
        //        if(game->completePendingNormalSummon(state.selectedTributes)){
        //            state.selectingTributes = false;
        //            state.selectedTributes.clear();
        //            state.tributesNeeded = 0;
        //            if(state.syncMonsterZoneToField) state.syncMonsterZoneToField();
        //            state.battleFeedbackMsg = "Evocazione completata"; state.battleFeedbackClock.restart();
        //        } else {
        //            state.battleFeedbackMsg = "Selezione tributi non valida"; state.battleFeedbackClock.restart();
        //        }
        //    }
        //    return true;
        //}
