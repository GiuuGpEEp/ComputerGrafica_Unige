...
bool Controller::handleEvent(const sf::Event& e){
    ...
    if(const auto* kpEsc = e.getIf<sf::Event::KeyPressed>()){
        if(kpEsc->code == M_KEY_ESCAPE){
            ...
            if(state.gamestate != GameState::DeckSelection){
                // Deseleziona carta e resetta scroll (incluse ST selection flags)
                state.selectedCardIndex.reset();
                state.selectedCardIsOnField = false;
                state.selectedCardIsSpellTrap = false;
                state.selectedCardSTOwner = -1;
    ...
    sf::FloatRect gyRectTop(state.graveyardSlotPosTop, state.slotSize);
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
                ...
                // Additionally, perform hit-test for P1/P2 SpellTrap cards to allow selection and details
                if(state.getGame){
                    Game* g = state.getGame();
                    int cur = g ? g->getTurn().getCurrentPlayerIndex() : 0;
                    // Check P1 ST row (absolute owner 0)
                    for(size_t i=0;i<state.p1STCards.size();++i){
                        if(state.p1STCards[i].getGlobalBounds().contains(mousePosF)){
                            state.selectedCardIndex = i;
                            state.selectedCardIsOnField = false; // not a monster zone
                            state.selectedCardIsSpellTrap = true;
                            state.selectedCardSTOwner = 0;
                            state.scrollOffset = 0.f;
                            break;
                        }
                    }
                    // Check P2 ST row (absolute owner 1)
                    if(!state.selectedCardIsSpellTrap){
                        for(size_t i=0;i<state.getGraveyardCards ? state.getGraveyardCards().size() : 0; ++i){}
                        for(size_t i=0;i<state.p1STCards.size();++i){} // no-op to keep structure
                        for(size_t i=0;i<state.p1STCards.size();++i){} // no-op
                        // We must use p2STCards from the main render sync; input Context doesn't expose it currently.
                        // Fallback: we only allow P1 ST selection via click for now.
                    }
                }
