...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Tappa 15");
    ...
    bool selectedCardIsSpellTrap = false; // true se la selezione attuale riguarda una Magia/Trappola
    int selectedCardSTOwner = -1; // owner for selected ST card (0=P1 bottom, 1=P2 top)
    ...
    /*selectedCardIsSpellTrap*/selectedCardIsSpellTrap,
    /*selectedCardSTOwner*/selectedCardSTOwner,
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if (selectedCardIndex.has_value()) {
                ...
                if (selectedCard != nullptr) {
                    // If the selection is a Spell/Trap zone card, override selectedCard with the proper ST card
                    if(!selectedCardIsOnField && state.selectedCardIsSpellTrap) {
                        int owner = state.selectedCardSTOwner;
                        size_t idx = selectedCardIndex.value();
                        if(owner == 0 && idx < p1STCards.size()){
                            selectedCard = &p1STCards[idx];
                        } else if(owner == 1 && idx < p2STCards.size()){
                            selectedCard = &p2STCards[idx];
                        }
                    }
                    // Se la carta è sul campo del P1 ed è coperta, permetti al proprietario
                    // di visualizzarne il fronte nei dettagli creando una copia con texture frontale.
                    if (selectedCardIsOnField && game && selectedCardIndex.has_value() && game->isFaceDownAt(0, selectedCardIndex.value())) {
                        Card tmp = *selectedCard;
                        RenderUtils::applyFrontTextureIfAny(tmp, textureManager);
                        RenderUtils::drawDetailsPanel(window, tmp, detailFont, windowSize, scrollOffset);
                    } else if(state.selectedCardIsSpellTrap){
                        // Spell/Trap selection: if owner is bottom (0) and the card is face-down, reveal front for details only
                        int owner = state.selectedCardSTOwner;
                        size_t idx = selectedCardIndex.value();
                        const Card* stCardPtr = nullptr;
                        if(owner == 0 && idx < p1STCards.size()) stCardPtr = &p1STCards[idx];
                        else if(owner == 1 && idx < p2STCards.size()) stCardPtr = &p2STCards[idx];
                        if(stCardPtr){
                            if(game && owner == 0 && game->isSpellTrapFaceDownAt(0, idx)){
                                Card tmp = *stCardPtr;
                                RenderUtils::applyFrontTextureIfAny(tmp, textureManager);
                                RenderUtils::drawDetailsPanel(window, tmp, detailFont, windowSize, scrollOffset);
                            } else {
                                RenderUtils::drawDetailsPanel(window, *stCardPtr, detailFont, windowSize, scrollOffset);
                            }
                        }
                    } else {
