...
int main(){
    ...
    auto rebuildResponseActivables = [&](){
        ...
        for (size_t i = 0; i < Game::ST_ZONE_SIZE; ++i) {
            if (!game->canActivateSetSpellTrapFor(responseOwner, i)) continue;
            // Defensive UI check: when it's the opponent's turn, ensure only Traps or Quick spells are listed
            if (cur != responseOwner) {
                const auto &zone = (responseOwner == game->getTurn().getCurrentPlayerIndex()) ? game->getSpellTrapZone() : game->getOpponentSpellTrapZone();
                if (i >= zone.size()) continue;
                Attribute attr = zone[i].getAttribute();
                const auto &feats = zone[i].getFeatures();
                bool isQuick = std::find(feats.begin(), feats.end(), Feature::Rapida) != feats.end();
                if (!(attr == Attribute::Trappola || isQuick)) continue;
            }
