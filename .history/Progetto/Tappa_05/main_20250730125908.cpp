...
int main(){
    ...
    while(window.isOpen()){
        ...
        // Disegna il deck (ora con posizioni pre-calcolate)
        for(int i = 0; i < static_cast<int>(deck.size()); ++i) {
            // Le carte sono già posizionate correttamente, disegniamo dall'ultima alla prima
            // per mantenere l'ordine di rendering corretto (top card in front)
            deck[deck.size() - 1 - i].draw(window);
        }
