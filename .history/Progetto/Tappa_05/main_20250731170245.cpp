...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Tappa 05 - Field + Card");
    ...
    while(window.isOpen()){
        while (const std::optional event = window.pollEvent()){
            ...
            if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                if(keyPressed->code == sf::Keyboard::Key::Space && gamestate == GameState::Intro) {
            ...
            if(gamestate == GameState::FieldVisible && clock.getElapsedTime().asSeconds() > 2.f) {
                std::cout << "Passaggio allo stato Playing..." << std::endl;
                gamestate = GameState::Playing; // Passa allo stato di gioco dopo 2 secondi
