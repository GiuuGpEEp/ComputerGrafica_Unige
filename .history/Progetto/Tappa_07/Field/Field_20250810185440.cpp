...
sf::Vector2f Field::getSlotPosition(Type slotType, int player, int slotIndex) const {
    ...
    std::cout << "getSlotPosition chiamato per tipo: " << static_cast<int>(slotType) << ", giocatore: " << player << ", indice: " << slotIndex << std::endl;
    
    // Raccogli tutte le posizioni per il tipo di slot richiesto
    for (const auto& slot : slots) {
        if (slot.getType() == slotType) {
            sf::Vector2f pos = slot.getPosition();
            std::cout << "Trovato slot di tipo " << static_cast<int>(slotType) << " alla posizione: " << pos.x << ", " << pos.y << std::endl;
            ...
            if (pos.y > windowSize.y / 2.0f) {
                ...
                std::cout << "Assegnato al giocatore 1" << std::endl;
            } else {
                player2Slots.push_back(pos);
                std::cout << "Assegnato al giocatore 2" << std::endl;
            }
        }
    }
    
    std::cout << "Player 1 ha " << player1Slots.size() << " slot di questo tipo" << std::endl;
    std::cout << "Player 2 ha " << player2Slots.size() << " slot di questo tipo" << std::endl;
    ...
    std::cout << "Selezionato il vettore per il giocatore " << player << " con " << targetSlots.size() << " slot" << std::endl;
    
    // Verifica che l'indice sia valido
    if (slotIndex >= 0 && slotIndex < static_cast<int>(targetSlots.size())) {
        sf::Vector2f result = targetSlots[slotIndex];
        std::cout << "Ritorno posizione: " << result.x << ", " << result.y << std::endl;
        return result;
    }
    
    // Se l'indice non è valido, restituisce (0,0)
    std::cout << "Indice non valido! Ritorno (0,0)" << std::endl;
