        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        slots.emplace_back(sf::Vector2f(x, p1MonsterY), monsterTexture, Type::Monster, slotSize);
    }
    
    // 3 slot magie/trappole giocatore 1
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p1SpellY), spellTrapTexture, Type::SpellTrap, slotSize);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), rotatedMonsterTexture, Type::Monster, slotSize);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), rotatedSpellTrapTexture, Type::SpellTrap, slotSize);
    }
    ...
    slots.emplace_back(sf::Vector2f(rightX, p2MonsterY), rotatedExtraDeckTexture, Type::Extra, slotSize);
    slots.emplace_back(sf::Vector2f(rightX, p2SpellY), rotatedFieldSpellTexture, Type::FieldSpell, slotSize);

    // Extra Deck e Field Spell a sinistra
    slots.emplace_back(sf::Vector2f(leftX, p1MonsterY), fieldSpellTexture, Type::FieldSpell, slotSize);
    ...
    slots.emplace_back(sf::Vector2f(leftX, p2MonsterY), rotatedDeckTexture, Type::Deck, slotSize);
    slots.emplace_back(sf::Vector2f(leftX, p2SpellY), rotatedGraveyardTexture, Type::Graveyard, slotSize);
}
...
bool Field::setSlotOccupied(Type slotType, int player, int slotIndex, bool occupied) {
    // Trova gli slot per tipo e giocatore
    std::vector<int> player1Indices;
    std::vector<int> player2Indices;
    
    for (size_t i = 0; i < slots.size(); ++i) {
        if (slots[i].getType() == slotType) {
            sf::Vector2f pos = slots[i].getPosition();
            
            // Determina a quale giocatore appartiene lo slot basandosi sulla posizione Y
            if (pos.y > windowSize.y / 2.0f) {
                player1Indices.push_back(static_cast<int>(i));
            } else {
                player2Indices.push_back(static_cast<int>(i));
            }
        }
    }
    
    // Seleziona gli indici corretti in base al giocatore
    const std::vector<int>& targetIndices = (player == 1) ? player1Indices : player2Indices;
    
    // Verifica che l'indice sia valido
    if (slotIndex >= 0 && slotIndex < static_cast<int>(targetIndices.size())) {
        int actualSlotIndex = targetIndices[slotIndex];
        slots[actualSlotIndex].setOccupied(occupied);
        return true;
    }
...
bool Field::isSlotOccupied(Type slotType, int player, int slotIndex) const {
    // Trova gli slot per tipo e giocatore
    std::vector<int> player1Indices;
    std::vector<int> player2Indices;
    ...
    if (slotIndex >= 0 && slotIndex < static_cast<int>(targetIndices.size())) {
        int actualSlotIndex = targetIndices[slotIndex];
        return slots[actualSlotIndex].isOccupied();
    }
    
    return true; // Se l'indice non è valido, consideriamo lo slot come occupato
}

int Field::findFreeSlotIndex(Type slotType, int player) const {
    // Trova gli slot per tipo e giocatore
    std::vector<int> player1Indices;
    std::vector<int> player2Indices;
    ...
    for (size_t i = 0; i < targetIndices.size(); ++i) {
        int actualSlotIndex = targetIndices[i];
        if (!slots[actualSlotIndex].isOccupied()) {
            return static_cast<int>(i); // Ritorna l'indice relativo al giocatore
        }
    }
