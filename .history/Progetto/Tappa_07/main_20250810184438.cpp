            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonReleased>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (findSlotPosition(mousePosF, cards[draggingCardIndex.value()], field, slotSize).has_value()) {
                            // Posizione valida: sposta la carta al centro dello slot
                            sf::Vector2f slotPos = findSlotPosition(mousePosF, cards[draggingCardIndex.value()], field, slotSize).value();
