...
int main(){
    ...
    // Inizializzo le zone di drop per il campo di gioco
    std::vector<DropZone> dropZones = createDropZones(field, slotSize);
    ...
    // Variabili per le drop zones
    bool isReturningToHand = false;
    std::optional<size_t> returningCardIndex;
    sf::Vector2f originalHandPosition; // Posizione originale nella mano
    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            // Mouse released: gestisce drop zones e ritorno in mano
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonReleased>()) {
                sf::Mouse::Button but = mouseButton->button;
                if(but == sf::Mouse::Button::Left) {
                    // Se stavamo trascinando una carta, controlla la drop zone
                    if (isDragging && draggingCardIndex.has_value()) {
                        sf::Vector2i mousePos = sf::Mouse::getPosition(window);
                        sf::Vector2f mousePosF = static_cast<sf::Vector2f>(mousePos);
                        
                        const DropZone* dropZone = findDropZone(mousePosF, dropZones);
                        
                        if (dropZone && canDropCard(cards[draggingCardIndex.value()], *dropZone)) {
                            // Carta può essere piazzata: posiziona la carta nella zona
                            cards[draggingCardIndex.value()].setPosition(dropZone->position);
                            std::cout << "Carta piazzata nello slot!" << std::endl;
                        } else {
                            // Carta non può essere piazzata: avvia animazione di ritorno
                            isReturningToHand = true;
                            returningCardIndex = draggingCardIndex;
                            originalHandPosition = cards[draggingCardIndex.value()].getPosition(); // Verrà aggiornata sotto
                            
                            // Calcola la posizione originale nella mano
                            if (returningCardIndex.has_value()) {
                                // Rimuovi temporaneamente la carta per calcolare la sua posizione finale
                                Card tempCard = cards[returningCardIndex.value()];
                                cards.erase(cards.begin() + returningCardIndex.value());
                                originalHandPosition = setHandPos(cards, tempCard, windowSize, cardSize, spacing, y, HAND_MAXSIZE);
                                cards.insert(cards.begin() + returningCardIndex.value(), tempCard);
                            }
                            
                            std::cout << "Zona non valida, carta torna in mano!" << std::endl;
                        }
                    }
        ...
        // Gestione animazione di ritorno alla mano
        if (isReturningToHand && returningCardIndex.has_value()) {
            if (returningCardIndex.value() < cards.size()) {
                bool returnCompleted = animateCardReturn(cards[returningCardIndex.value()], originalHandPosition, deltaTime);
                if (returnCompleted) {
                    isReturningToHand = false;
                    returningCardIndex.reset();
                    // Riorganizza le carte in mano dopo il ritorno
                    updateHandPositions(cards, windowSize, cardSize, spacing, y, HAND_MAXSIZE);
                }
            } else {
                // Indice non valido, termina l'animazione
                isReturningToHand = false;
                returningCardIndex.reset();
            }
        }
