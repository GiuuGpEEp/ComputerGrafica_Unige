    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if(mousePressed && !isDragging) {
                ...
                // Controlla solo le carte in mano, non quelle sul campo
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (findSlotPosition(mousePos, cards[draggingCardIndex.value()], field, slotSize).has_value()) {
                            ...
                            cardToField.setPosition(slotPos);
        ...
        if(gamestate == GameState::Intro) drawStartScreen(window, detailFont, windowSize);
            ...
            // Disegna le carte sul campo (senza animazione di sollevamento)
            for (auto& fieldCard : fieldCards) {
                fieldCard.draw(window);
            }

            // Disegna le carte in mano (con animazione di sollevamento)
            for (size_t i = 0; i < cards.size(); ++i) {
