...
int main(){
    ...
    while(window.isOpen()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonReleased>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (isValidDropPosition(mousePos, cards[draggingCardIndex.value()], field, slotSize)) {
                            // Posizione valida: trova lo slot specifico e marcalo come occupato
                            auto slotPosOpt = findSlotPosition(mousePos, cards[draggingCardIndex.value()], field, slotSize);
                            if (slotPosOpt.has_value()) {
                                sf::Vector2f slotPos = slotPosOpt.value();
                                cards[draggingCardIndex.value()].setPosition(slotPos);
                                
                                // Trova l'indice dello slot e marcalo come occupato
                                Type cardType = cards[draggingCardIndex.value()].getType();
                                
                                // Trova quale slot corrisponde alla posizione
                                for (int i = 0; i < 5; ++i) {
                                    if (cardType == Type::Monster) {
                                        sf::Vector2f checkPos = field.getSlotPosition(Type::Monster, 1, i);
                                        if (abs(checkPos.x - slotPos.x) < 1.0f && abs(checkPos.y - slotPos.y) < 1.0f) {
                                            field.setSlotOccupied(Type::Monster, 1, i, true);
                                            break;
                                        }
                                    } else if (cardType == Type::SpellTrap) {
                                        sf::Vector2f checkPos = field.getSlotPosition(Type::SpellTrap, 1, i);
                                        if (abs(checkPos.x - slotPos.x) < 1.0f && abs(checkPos.y - slotPos.y) < 1.0f) {
                                            field.setSlotOccupied(Type::SpellTrap, 1, i, true);
                                            break;
                                        }
                                    }
                                }
                                ...
                                if (cardType == Type::SpellTrap || cardType == Type::FieldSpell) {
                                    sf::Vector2f fieldPos = field.getSlotPosition(Type::FieldSpell, 1);
                                    if (abs(fieldPos.x - slotPos.x) < 1.0f && abs(fieldPos.y - slotPos.y) < 1.0f) {
                                        field.setSlotOccupied(Type::FieldSpell, 1, 0, true);
                                    }
                                }
        ...
        Card tmpcard = Card("", "", 0, 0, sf::Vector2f(0.f, 0.f), sf::Vector2f(0.f, 0.f), textureFlipped, Type::Monster);
