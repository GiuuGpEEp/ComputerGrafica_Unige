...
int main(){
    ...
    const float dragThreshold = 25.0f; // Soglia in pixel per attivare il dragging
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    // Controllo immediato se clicchiamo su una carta
                    for (size_t i = 0; i < cards.size(); ++i) {
                        if (cards[i].isClicked(mousePos)) {
                            isPotentialDrag = true;
                            potentialDragCardIndex = i; // CORREZIONE: era isPotentialDrag = i;
            ...
            // Gestione hover mentre il mouse è premuto (per l'effetto di sollevamento)
            if(mousePressed && !isDragging){
                sf::Vector2i mousePos = sf::Mouse::getPosition(window);
                bool cardHovered = false;
                for (size_t i = 0; i < cards.size(); ++i) {
                    if (cards[i].isClicked(mousePos)) {
                        // Aggiorna la selezione per l'hover effect solo se non stiamo già trascinando
                        if (!selectedCardIndex.has_value() || selectedCardIndex.value() != i) {
                            scrollOffset = 0.f;
                        }
                        selectedCardIndex = i;
                ...
                if (!cardHovered && !isPotentialDrag) {
                    selectedCardIndex.reset();
                    scrollOffset = 0.f;
                } 
            }
