...
bool isValidDropPosition(sf::Vector2f mousePos, const Card& card, const Field& field, const sf::Vector2f& slotSize) {
    ...
    switch (cardType){
        case Type::Monster:
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                // Controlla se il mouse è dentro questo slot E se lo slot è libero
                if (mousePos.x >= slotPos.x - halfSize.x && mousePos.x <= slotPos.x + halfSize.x &&
                    ...
                    !field.isSlotOccupied(Type::Monster, 1, i)) {
                    return true;
                }
            }
            break;
        ...
        case Type::SpellTrap:
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                if (mousePos.x >= slotPos.x - halfSize.x && mousePos.x <= slotPos.x + halfSize.x &&
                    mousePos.y >= slotPos.y - halfSize.y && mousePos.y <= slotPos.y + halfSize.y &&
                    !field.isSlotOccupied(Type::SpellTrap, 1, i)) {
                    return true;
                }
            }
            ...
            if (mousePos.x >= fieldPos.x - halfSize.x && mousePos.x <= fieldPos.x + halfSize.x &&
                ...
                !field.isSlotOccupied(Type::FieldSpell, 1, 0)) {
                return true;
            }
            break;
    ...
    return false; // Se non trova uno slot valido e libero, ritorna false
}
