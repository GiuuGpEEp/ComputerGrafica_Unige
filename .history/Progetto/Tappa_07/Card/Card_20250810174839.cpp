...
Card::Card(const std::string& name, const std::string& description, int atk, int def, sf::Vector2f pos, sf::Vector2f size, sf::Texture& textureRef, Type cardType)
    : name(name), description(description), attack(atk), defense(def), sprite(textureRef), position(pos), type(cardType) //Inizializzo sprite con textureRef e position
{
...
//Restituisco il valore di attacco della carta
int Card::getAttack() const {
    return attack;
}

//Restituisco il valore di difesa della carta
