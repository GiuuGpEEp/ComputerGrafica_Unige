...
int main(){
    ...
    sf::Vector2f deckSlotSize = calculateSlotSize(windowSize);
    float deckScaleFactor = 0.9f; // Le carte del deck sono il 90% della dimensione dello slot
    sf::Vector2f deckCardSize(deckSlotSize.x * deckScaleFactor, deckSlotSize.y * deckScaleFactor);
    
    for(int i = 0; i < 25; ++i) {
        deck.emplace_back("Carta " + std::to_string(i + 1), "Descrizione della carta " + std::to_string(i + 1), 
                          1000, 800, sf::Vector2f(0.f, 0.f), deckCardSize, textureFlipped);
    }
    ...
    while(window.isOpen()){
        ...
        // Disegna il deck (solo le carte visibili)
        //int visibleCard = std::min(3, static_cast<int>(deck.size()));
        int visibleCard = static_cast<int>(deck.size());
        sf::Vector2f slotSize = calculateSlotSize(windowSize); // Ottieni la dimensione degli slot
        float deckScaleFactor = 0.9f; // Stesso fattore di scala usato nella creazione
        sf::Vector2f deckCardSize(slotSize.x * deckScaleFactor, slotSize.y * deckScaleFactor);
        
        for(int i = 0; i < visibleCard; ++i) {
            sf::Vector2f offset(0.f, -5.f * i); // Leggero spazio verticale tra le carte
            Card& currentCard = deck[deck.size() - 1 - i];
            ...
            // Centra la carta scalata rispetto al centro dello slot
            sf::Vector2f centeredPos = slotCenter - sf::Vector2f(deckCardSize.x / 2.f, deckCardSize.y / 2.f) + offset;
            
            currentCard.setPosition(centeredPos);
            currentCard.draw(window);
        }

        // Disegna il testo al centro del deck solo se il mouse è sopra l'area del deck
        sf::Vector2f mouseFloat(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y));
        sf::FloatRect deckArea(deckSlotPos, slotSize);
        
        if (deckArea.contains(mouseFloat)) {
            sf::Text deckLabel(detailFont, "", 16);
            deckLabel.setFillColor(sf::Color::White);
            deckLabel.setStyle(sf::Text::Bold);
            deckLabel.setString(std::to_string(deck.size()));
            
            // Calcola il centro del deck per posizionare il testo
            sf::Vector2f deckCenter = deckSlotPos + sf::Vector2f(slotSize.x / 2.f, slotSize.y / 2.f + 5.f);
            sf::FloatRect textBounds = deckLabel.getLocalBounds();
            deckLabel.setPosition(deckCenter - sf::Vector2f(textBounds.size.x / 2.f, textBounds.size.y / 2.f));
