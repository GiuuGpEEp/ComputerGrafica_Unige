...
int main(){
    ...
    while(window.isOpen()){
        ...
        for(int i = 0; i < visibleCard; ++i) {
            ...
            // Calcola il centro dello slot
            sf::Vector2f slotCenter = deckSlotPos + sf::Vector2f(slotSize.x / 2.f, slotSize.y / 2.f);
            
            // Ottieni le dimensioni della carta (assumendo che siano 100x150 come nel costruttore)
            sf::Vector2f cardSize(100.f, 150.f);
            
            // Centra la carta rispetto al centro dello slot
            sf::Vector2f centeredPos = slotCenter - sf::Vector2f(cardSize.x / 2.f, cardSize.y / 2.f) + offset;
            
            currentCard.setPosition(centeredPos);
            currentCard.draw(window);
        }

        // Disegna il testo sopra il deck
        sf::Text deckLabel(detailFont, "", 14);
        deckLabel.setFillColor(sf::Color::White);
        deckLabel.setStyle(sf::Text::Bold);
        deckLabel.setString(std::to_string(deck.size()));
        deckLabel.setPosition(deckSlotPos + sf::Vector2f(25.f, -20.f));
