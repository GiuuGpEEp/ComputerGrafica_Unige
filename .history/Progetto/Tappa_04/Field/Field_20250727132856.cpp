        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), rotatedMonsterTexture, Slot::Type::Monster, slotSize, p2Transf, p2Transf);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), spellTrapTexture, Slot::Type::SpellTrap, slotSize, p2Transf, p2Transf);
    }
