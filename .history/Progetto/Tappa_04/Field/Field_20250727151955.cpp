        ...
        ) : background(fieldTexture), windowSize(windowSize)
{
    ...
    //------- ZONE SPECIALI -------
    // Margine dinamico dai bordi per prospettiva
    float marginP1 = std::max(windowSize.x * 0.04f, 20.0f); 
    float marginP2 = std::max(windowSize.x * 0.06f, 30.0f); // Margine maggiore per P2
    
    // Posizioni per zone speciali con prospettiva
    float rightXP1 = windowSize.x - slotSizeP1.x - marginP1;
    float leftXP1 = marginP1;
    float rightXP2 = windowSize.x - slotSizeP2.x - marginP2;
    float leftXP2 = marginP2;
    
    // Verifica sovrapposizioni per P1
    float p1ZoneRight = p1StartX + p1MainZoneWidth;
    if (rightXP1 < p1ZoneRight + spacingP1 * 2) {
        rightXP1 = std::min(windowSize.x - slotSizeP1.x - 10.0f, p1ZoneRight + spacingP1 * 2);
    }
    if (leftXP1 + slotSizeP1.x + spacingP1 * 2 > p1StartX) {
        leftXP1 = std::max(10.0f, p1StartX - slotSizeP1.x - spacingP1 * 2);
    }
    
    // Zone speciali giocatore 1 (dimensioni normali)
    slots.emplace_back(sf::Vector2f(rightXP1, p1MonsterY), graveyardTexture, Slot::Type::Graveyard, slotSizeP1, p1Transf);
    slots.emplace_back(sf::Vector2f(rightXP1, p1SpellY), deckTexture, Slot::Type::Deck, slotSizeP1, p1Transf);
    slots.emplace_back(sf::Vector2f(leftXP1, p1MonsterY), fieldSpellTexture, Slot::Type::FieldSpell, slotSizeP1, p1Transf);
    slots.emplace_back(sf::Vector2f(leftXP1, p1SpellY), extraDeckTexture, Slot::Type::Extra, slotSizeP1, p1Transf);

    // Crea le texture ruotate per le zone speciali del giocatore 2
    ...
    // Zone speciali giocatore 2 (dimensioni ridotte per prospettiva)
    slots.emplace_back(sf::Vector2f(rightXP2, p2MonsterY), rotatedExtraDeckTexture, Slot::Type::Extra, slotSizeP2, p2Transf);
    slots.emplace_back(sf::Vector2f(rightXP2, p2SpellY), rotatedFieldSpellTexture, Slot::Type::FieldSpell, slotSizeP2, p2Transf);
    slots.emplace_back(sf::Vector2f(leftXP2, p2MonsterY), rotatedDeckTexture, Slot::Type::Deck, slotSizeP2, p2Transf);
    slots.emplace_back(sf::Vector2f(leftXP2, p2SpellY), rotatedGraveyardTexture, Slot::Type::Graveyard, slotSizeP2, p2Transf);
}
