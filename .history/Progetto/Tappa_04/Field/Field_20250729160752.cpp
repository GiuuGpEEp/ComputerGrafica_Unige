...
sf::Vector2f Field::getSlotPosition(Slot::Type slotType, int player) const {
    int slotCount = 0;
    // Cerca lo slot del tipo richiesto per il giocatore specificato
    for (const auto& slot : slots) {
        if (slot.getType() == slotType) {
            slotCount++;
            // Per il giocatore 1, prendi il primo slot del tipo trovato
            // Per il giocatore 2, prendi il secondo slot del tipo trovato
            if (slotCount == player) {
                return slot.getPosition();
            }
        }
    }
    // Se non trova lo slot, restituisce (0,0)
