...
{
    // Aggiunta delle trasfonmazioni 
    sf::Transform p1Transf;
    p1Transf.scale(sf::Vector2f(1.1f, 0.8f));

    sf::Transform p2Transf;
    p2Transf.scale(sf::Vector2f(0.85f, 0.6f));
    p2Transf.translate(sf::Vector2f(0.f, -30.f));
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        slots.emplace_back(sf::Vector2f(x, p1MonsterY), monsterTexture, Slot::Type::Monster, slotSize, p1Transf);
    }
    
    // 3 slot magie/trappole giocatore 1
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p1SpellY), spellTrapTexture, Slot::Type::SpellTrap, slotSize, p1Transf);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2SpellY), rotatedMonsterTexture, Slot::Type::Monster, slotSize, p2Transf);
    }
    ...
    for (int i = 0; i < 3; ++i) {
        float x = mainStartX + i * (slotSize.x + spacing);
        slots.emplace_back(sf::Vector2f(x, p2MonsterY), rotatedSpellTrapTexture, Slot::Type::SpellTrap, slotSize, p2Transf);
    }
    ...
    slots.emplace_back(sf::Vector2f(rightX, p2MonsterY), rotatedExtraDeckTexture, Slot::Type::Extra, slotSize, p);
