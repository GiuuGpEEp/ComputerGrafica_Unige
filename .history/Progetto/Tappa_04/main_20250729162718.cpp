...
int main(){
    ...
    //Visualizzo il deck (ovviemente non per intero, ma solo un paio di carte a livello "simbolico", se la dimensione del deck è minore di 3, visualizzo solo quelle)
    ...
    for(int i = 0; i < visibleCard; ++i) {
        ...
        deck[deck.size() - i].draw(window);
    }

    //Visualizzo il testo sopra il deck
    sf::Text deckCountText(detailFont, "", 14);
    deckCountText.setFillColor(sf::Color::White);
    deckCountText.setStyle(sf::Text::Bold);
    deckCountText.setString(std::to_string(deck.size()));
    deckCountText.setPosition(deckSlotPos + sf::Vector2f(25.f, -20.f));
