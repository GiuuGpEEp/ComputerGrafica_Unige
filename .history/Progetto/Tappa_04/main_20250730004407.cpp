...
int main(){
    ...
    // Calcola le dimensioni per le carte del deck (usando slotSize)
    ...
    for(int i = 0; i < 25; ++i) {
        deck.emplace_back("Carta " + std::to_string(i + 1), "Descrizione della carta " + std::to_string(i + 1), 
                          1000, 800, sf::Vector2f(0.f, 0.f), deckSlotSize, textureFlipped);
    }
    ...
    while(window.isOpen()){
        ...
        for(int i = 0; i < visibleCard; ++i) {
            ...
            // Usa le dimensioni dinamiche degli slot per le carte del deck
            sf::Vector2f cardSize = slotSize;
