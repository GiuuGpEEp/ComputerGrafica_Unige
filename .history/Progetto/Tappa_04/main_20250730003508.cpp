...
int main(){
    ...
    while(window.isOpen()){
        ...
        for(int i = 0; i < visibleCard; ++i) {
            ...
            // Centrare la carta rispetto allo slot
            sf::Vector2f cardSize = currentCard.getSize();
            sf::Vector2f centeredPos = deckSlotPos - sf::Vector2f(cardSize.x / 2.f, cardSize.y / 2.f) + offset;
            currentCard.setPosition(centeredPos);
            currentCard.draw(window);

            // Testo sovrapposto al centro della carta in cima allo stack
            if (i == 0) {
                sf::Text counterText;
                counterText.setFont(detailFont);
                counterText.setCharacterSize(18);
                counterText.setFillColor(sf::Color::White);
                counterText.setStyle(sf::Text::Bold);
                counterText.setString(std::to_string(deck.size()));

                // Ottieni bounds e centra il testo sulla carta
                sf::FloatRect textBounds = counterText.getLocalBounds();
                counterText.setOrigin(textBounds.left + textBounds.width / 2.f, textBounds.top + textBounds.height / 2.f);
                counterText.setPosition(centeredPos + cardSize / 2.f);
