...
int main() {
    ...
    while(window.isOpen()){
        ...
        if (selectedCardIndex.has_value()) {
            ...
            // Crea il testo SFML
            sf::Text detailTextObj(detailFont, "", );
            detailTextObj.setFont(detailFont);
            detailTextObj.setCharacterSize(14);
            ...
            for (const char& ch : detailRaw) {
                ...
                if (ch == '\n' || detailTextObj.getLocalBounds().width > maxWidth) {
                    if (ch != '\n') {
