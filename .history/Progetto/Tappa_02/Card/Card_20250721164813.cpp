    ...
    : name(name), description(description), attack(atk), defense(def) //Notazione alternativa per l'inizializzazione dei membri = a this -> name = name
{
    ...
    // Carico le texture come membri della classe
    if (!textureFlipped.loadFromFile("../../Progetto/resources/Texture1.png")) {
        std::cerr << "ERRORE: Impossibile caricare la texture flipped!" << std::endl;
    }
    if (!textureNotFlipped.loadFromFile("../../Progetto/resources/CardNotSet.png")) {
        std::cerr << "ERRORE: Impossibile caricare la texture not flipped!" << std::endl;
    }
