...
int main() {
    ...
    while(window.isOpen()){
        ...
        if (selectedCardIndex.has_value()) {
            ...
            sf::View textView;
            textView.setSize(panelSize.x - 20.f, panelSize.y - 20.f); // Margine interno
            textView.setCenter((panelPos.x + panelSize.x/2.f), (panelPos.y + panelSize.y/2.f));
            textView.setViewport(sf::FloatRect(
                (panelPos.x + 10.f) / window.getSize().x,    // x normalizzato
                (panelPos.y + 10.f) / window.getSize().y,    // y normalizzato  
                (panelSize.x - 20.f) / window.getSize().x,   // width normalizzato
                (panelSize.y - 20.f) / window.getSize().y    // height normalizzato
            ));
