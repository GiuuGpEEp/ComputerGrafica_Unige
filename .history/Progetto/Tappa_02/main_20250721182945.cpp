...
// Funzione per il word wrapping del testo
    ...
    while (words >> word) {
        ...
        if (tempText.getLocalBounds().size.x <= maxWidth) {
            ...
            if (!currentLine.empty()) {
                ...
                // Parola troppo lunga per una riga, la dividiamo
                wrappedText += word + "\n";
            }
        }
    }
...
int main() {
    ...
    while(window.isOpen()){
        ...
        if (selectedCardIndex.has_value()) {
            ...
            // Applica word wrapping al testo
            ...
            // Crea il testo SFML con il testo già formattato
            ...
            // Disegna prima il pannello con la view principale
            window.draw(detailPanel);

            // Salva la view corrente
            sf::View originalView = window.getView();
            
            // Crea una view limitata al pannello per il clipping del testo
            sf::View textView;
            textView.setSize(sf::Vector2f(panelSize.x - 20.f, panelSize.y - 20.f)); // Margine interno
            textView.setCenter(sf::Vector2f(panelPos.x + panelSize.x/2.f, panelPos.y + panelSize.y/2.f));
            textView.setViewport(sf::FloatRect(
                sf::Vector2f((panelPos.x + 10.f) / window.getSize().x, (panelPos.y + 10.f) / window.getSize().y),    // posizione normalizzata
                sf::Vector2f((panelSize.x - 20.f) / window.getSize().x, (panelSize.y - 20.f) / window.getSize().y)   // dimensione normalizzata
            ));

            // Imposta la view per il testo e disegnalo
