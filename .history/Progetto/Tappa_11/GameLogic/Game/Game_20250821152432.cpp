...
void Game::start() {
    ...
    started = true;
    // Pescata iniziale: il giocatore che inizia NON pesca (regola), quindi nessuna draw qui.
    // Se vuoi carte iniziali per entrambi (es. 5), potresti distribuirle qui manualmente ignorando la regola; per ora lasciamo vuoto.
    dispatcher.emit(GameEventType::PhaseChange); // notifica Draw phase iniziale (senza pescata)
} 
...
void Game::processEnterStandby(){
    ...
    if(shouldSkip){
        dispatcher.emit(GameEventType::PhaseChange); // informiamo dello skip
        // Avanza direttamente a Main1
        turn.nextPhase();
