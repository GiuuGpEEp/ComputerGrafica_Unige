...
bool Game::tryNormalSummon(size_t handIndex){
    ...
    if(tributesNeeded > 0){
        // Non abbiamo ancora UI per selezionare i tributi -> per ora fallisce se necessari (placeholder)
        // In un prossimo step integreremo la selezione: qui potremmo automaticamente tributare i primi N
        if(monsterZones[turn.getCurrentPlayerIndex()].size() < static_cast<size_t>(tributesNeeded)) return false;
        // Auto-tributa i primi N slot (semplice placeholder)
        std::vector<size_t> tributeIdx;
        for(int i=0;i<tributesNeeded;++i) tributeIdx.push_back(i);
        if(!tributeMonsters(tributeIdx)) return false;
    }
