...
int main(){
    ...
    int phaseChangeSubscriptionId = -1; // subscription evento cambio fase per auto pescata
    int drawStartSubscriptionId = -1;   // subscription evento DrawStart
    int drawEndSubscriptionId = -1;     // subscription evento DrawEnd
    // Animazioni di scarto
    struct DiscardAnim { Card card; sf::Vector2f start; sf::Vector2f end; float t=0.f; float duration=0.4f; };
    std::vector<DiscardAnim> discardAnims; std::vector<Card> pendingToGrave; // carte in viaggio verso graveyard
    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                ...
                if(keyPressed->code == sf::Keyboard::Key::T && gamestate == GameState::Playing && game){
                    // Prima di endTurn gestiamo eventuali scarti animati
                    if(handPtr){
                        auto excess = game->extractExcessCards(HAND_MAXSIZE); // rimuove dalla mano le eccedenti
                        if(!excess.empty()){
                            // Posizione slot graveyard (player 1 per ora)
                            ...
                            for(auto &c : excess){
                                DiscardAnim anim{c, c.getPosition(), gyCenter, 0.f, 0.45f};
                                discardAnims.push_back(anim);
                                pendingToGrave.push_back(c); // trasferiremo nel vector graveyard al termine animazione
                            }
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            game->start();
                            // Queue pescate iniziali (5 carte)
                            drawController.queueDraw(5);
                            // Listener cambio fase -> pescata automatica in Draw
                            phaseChangeSubscriptionId = game->events().subscribe(GameEventType::PhaseChange, [&](){
                                if(!game) return;
                                if(game->getTurn().getPhase() == GamePhase::Draw && drawController.pending()==0 && !deck.isEmpty()){
                                    drawController.queueDraw(1);
                                }
                            });
                            // Listener DrawStart/DrawEnd (debug/log)
        ...
        // Update animazioni scarto
        if(!discardAnims.empty()){
            for(auto &da : discardAnims){
                da.t += deltaTime/da.duration;
                float tt = std::min(1.f, da.t);
                // easing semplice (quadratic out)
                float e = 1.f - (1.f-tt)*(1.f-tt);
                sf::Vector2f pos = da.start + (da.end - da.start)*e;
                da.card.setPosition(pos - sf::Vector2f(da.card.getSize().x/2.f, da.card.getSize().y/2.f));
            }
            // Se la prima (tutte hanno durata simile) finita, trasferisci tutte le concluse al graveyard
            auto it = std::remove_if(discardAnims.begin(), discardAnims.end(), [&](const DiscardAnim &d){ return d.t>=1.f; });
            if(it != discardAnims.begin()){
                // trasferisci concluse
                if(game){
                    auto &gy = game->getGraveyard();
                    // pendingToGrave contiene in ordine; spostiamo tutte se tutte concluse
                    if(std::all_of(discardAnims.begin(), discardAnims.end(), [](const DiscardAnim &d){ return d.t>=1.f; })){
                        for(auto &c : pendingToGrave) gy.push_back(c);
                        pendingToGrave.clear();
                        discardAnims.clear();
                    }
                }
            }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(game && !game->getGraveyard().empty()){
                ...
                Card copy = top; // draw copy at slot center scaled
                sf::Vector2f gyPos = field.getSlotPosition(Type::Graveyard, P1);
                float scale = 0.75f;
            ...
            // Disegna animazioni di scarto in volo
            for(auto &da : discardAnims){ da.card.draw(window); }
