...
int main(){
    ...
    bool pendingAutoTurnEnd = false; // dopo scarto automatico
    ...
    while(window.isOpen()){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
                ...
                if(keyPressed->code == sf::Keyboard::Key::T && gamestate == GameState::Playing && game){
                    // Se non siamo ancora in End Phase, fast-forward fino a End (senza terminare subito il turno)
                    if(game->getTurn().getPhase() != GamePhase::End){
                        while(game->getTurn().getPhase() != GamePhase::End){
                            game->advancePhase();
                        }
                        // Se non servono scarti, pianifica auto end turno
                        if(!(handPtr && handPtr->size() > HAND_MAXSIZE)){
                            pendingAutoTurnEnd = true; // verrà processato nel loop update (così l'utente vede l'End Phase per un frame)
                        }
                    } else {
                        // Già in End Phase: se nessuna animazione scarto attiva, termina turno
                                ...
                                if(game->getTurn().getPhase() == GamePhase::End && handPtr && !discardController.active()){
                                    if(handPtr->size() > HAND_MAXSIZE){
                                        ...
                                        if(!excess.empty()){
                                            ...
                                            // End turn verrà completato dopo animazione
                                        }
                                    } else {
                                        // Nessun eccesso: programma fine turno automatica
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            if(game && handPtr){
                ...
                // Se siamo in End Phase e non ci sono più animazioni di scarto e pendingAutoTurnEnd è true -> termina turno
                if(game->getTurn().getPhase() == GamePhase::End && !discardController.active() && pendingAutoTurnEnd){
                    pendingAutoTurnEnd = false;
                    game->endTurn();
                    std::cout << "[AutoTurn] Fine turno automatica dopo End Phase." << std::endl;
                }
            }
        }
