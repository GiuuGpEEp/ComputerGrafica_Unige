...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Tappa 11");
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                // Tasto destro nella deck selection: fade out
                    ...
                    // Battle targeting: right-click on opponent monster to target
                    if(but == sf::Mouse::Button::Right && gamestate == GameState::Playing && game && game->getTurn().getPhase() == GamePhase::Battle){
                ...
                if (!cardClicked) {
                    for (size_t i = 0; i < fieldCards.size(); ++i) {
                        if (fieldCards[i].isClicked(mousePos)) {
                            ...
                            // Battle: select attacker on click
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (handPtr && findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).has_value()) {
                            ...
                            if(game && !selectingTributes && game->tryNormalSummon(draggingCardIndex.value())){
                                // Normal Summon senza tributi: il render verrà aggiornato dall'evento NormalSummon
                                updateHandPositions(*handPtr, windowSize, cardSize, spacing, y, HAND_MAXSIZE);
                            } else if(selectingTributes){
                                // Summon in sospeso: la carta resta in mano finché completePendingNormalSummon
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(game){
                ...
                if(game->getTurn().getPhase() == GamePhase::Battle){
                    sf::Text hint(detailFont, "Battle: clicca un tuo mostro, poi A per attacco diretto (se possibile)", 20);
