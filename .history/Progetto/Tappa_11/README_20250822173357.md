...
Ho aggiunto nel main la parte di visualizzazione e trigger della battlephase. 

- Se siamo in battle phase Clic su un mostro sul campo per selezionarlo (tinta giallo chiaro per evidenziare).
- Premi A per effettuare un Attacco Diretto se il campo avversario è vuoto.
- Premi clickDestro per targettare il mostro nemico
- Ogni mostro può attaccare una volta per turno; i Life Points sono mostrati in alto a destra (Current | Opp).
- Aggiunto anche altre subscription per la gestione degli eventi
...
Aggiunto nel main anche la visualizzazione di una carta dummy nel campo dell'avversario, per testare se funziona il target di attacco. Ho aggiunto anche quindi un metodo di debug in game per aggiungere questo mostro

**Fix Tribute e Battle**
Facendo qualche test ho notato che se eravamo nella selezione dei tributi e muovevo qualche carta in mano quest'ultima poteva essere mossa e inoltre la visualizzazione si fermava.
- Ho fatto quindi si che se siamo nella selezione tributi non è possibile fare nessun'altra azione se non selezionare i mostri da tributare

Altra cosa che ho notato è che la pressione del tasto esc durante la selezione dei tributi o del mostro con cui attaccare d