...
1. Introdurre un evento `DiscardEnd` per sganciare del tutto il polling nel main.
2. Creare `GameHUD` per rimuovere l'HUD inline dal main.
3. Definire interfaccia effetti (`ICardEffect`) + registry e aggancio ad `EventDispatcher`.
4. API movimento carte generica (es. `moveCard(Zone from, Zone to, Card c)`) con emissione eventi.
5. Stub AI / secondo giocatore passivo per test cicli completi.
6. Piccoli test logici (unit senza grafica) su draw/advance/discard.

---

### Nota compatibilità
Se dovessi ripristinare il comportamento precedente (pescata o scarto gestiti manualmente) basta non chiamare:
```
game->attachDrawController(...);
game->setDiscardCallback(...);
```
e rimuovere le nuove API dal main. Il codice rimane retro-compatibile perché senza callback lo scarto in End Phase viene forzato direttamente nel graveyard senza animazione.

---

Ultimo allineamento: i punti 2 e 3 del piano (estrazione logica pescata e centralizzazione state machine) risultano completati; future modifiche si concentreranno su effetti, HUD e test.