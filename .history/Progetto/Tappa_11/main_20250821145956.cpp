...
int main(){
    ...
    while(window.isOpen()){
        ...
        static bool wasDiscardActive = false; // tracking fine animazione scarto
        if(gamestate == GameState::Playing && !returnPopupActive){
            if(game && handPtr){
                ...
                bool wasActiveBefore = discardController.active();
                discardController.update(deltaTime, *game);
                bool isActiveNow = discardController.active();
                if(game->getTurn().getPhase() == GamePhase::End){
                    // Caso: nessuno scarto necessario -> auto end turn
                    if(!wasActiveBefore && !isActiveNow && game->shouldAutoEndTurn()){
                        game->onDiscardAnimationFinished();
                        std::cout << "[AutoTurn] Fine turno automatica (nessuno scarto)." << std::endl;
                    }
                    // Caso: animazione appena terminata (c'erano scarti)
