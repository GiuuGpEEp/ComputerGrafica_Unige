                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            // subscribe auto draw on draw phase entry
                            phaseChangeSubscriptionId = game->events().subscribe(GameEventType::PhaseChange, [&, deckSlotPos](){
                                if(!game) return;
                                if(game->getTurn().getPhase() == GamePhase::Draw){
                                    if(drawController.pending() == 0 && !deck.isEmpty()){
