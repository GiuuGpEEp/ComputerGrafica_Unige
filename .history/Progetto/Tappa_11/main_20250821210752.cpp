        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    // Se siamo in gioco e stiamo scegliendo tributi, rendi la selezione reattiva al primo click
                    if((gamestate == GameState::Playing || gamestate == GameState::FieldVisible) && selectingTributes){
                        sf::Vector2i mousePosNow = sf::Mouse::getPosition(window);
                        for (size_t i = 0; i < fieldCards.size(); ++i) {
                            if (fieldCards[i].isClicked(mousePosNow)) {
                                ...
                                // Se raggiunto numero richiesto prova a completare subito
                                ...
                                break; // gestito il click su una carta campo
                            }
                        }
                    }
                }
