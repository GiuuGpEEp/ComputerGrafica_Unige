...
Questo tipo di evocazione conta come una normal summon normale, quindi all'interno di tryNormalSummon usiamo il nuovo metodo `requiredTributesFor` che restituisce il numero di tributi necessari per giocare una carta. Se il numero restituito è 0 allora lasciamo tutto così, mentre se è > 0 deleghiamo il compito a un'altra funzione ovvero `processEnterStandby`