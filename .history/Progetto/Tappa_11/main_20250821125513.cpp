...
int main(){
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if(game){
                        sf::Vector2f gyPos = field.getSlotPosition(Type::Graveyard, P1);
                        graveyardOverlay.startHold(static_cast<sf::Vector2f>(mousePos), sf::FloatRect(gyPos, slotSize), gamestate, !game->getGraveyard().empty());
                    }
                    // Gestione click su overlay
                    extraOverlay.handleMousePress(static_cast<sf::Vector2f>(mousePos), deck.getExtraCards());
                    if(game) graveyardOverlay.handleMousePress(static_cast<sf::Vector2f>(mousePos), game->getGraveyard());
    ...
    // (Aggiornamento animazioni scarto spostato nel blocco precedente via discardController.update)
        ...
        if(!returnPopupActive){
            ...
            if(game) graveyardOverlay.update(deltaTime, isDragging, gamestate);
        }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(game) graveyardOverlay.draw(window, detailFont, game->getGraveyard(), scrollOffset);
