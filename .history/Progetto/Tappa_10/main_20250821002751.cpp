                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (handPtr && findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).has_value()) {
                            // Debug: stampa fase e flag normalSummon
                            if(game){
                                std::cout << "Fase attuale: " << phaseToString(game->getTurn().getPhase()) << std::endl;
                                std::cout << "normalSummonUsed: " << (game->canNormalSummon() ? "false" : "true") << std::endl;
                                std::cout << "Mano size: " << handPtr->size() << ", monsterZone size: " << game->getMonsterZone().size() << std::endl;
                                std::cout << "Indice carta: " << draggingCardIndex.value() << std::endl;
                            }
