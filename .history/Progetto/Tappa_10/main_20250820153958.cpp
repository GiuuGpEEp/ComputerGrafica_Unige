    ...
    while(window.isOpen()){
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            game->start();
                            // Allineo mano grafica a mano logica: per ora usiamo la mano grafica esistente
                            // (potremmo in futuro sincronizzare Player.hand -> cards)
                            phaseJustEntered = true;
                        }
                    }
                }
            ...
            if(game){
                if(phaseJustEntered){
                    if(game->getTurn().getPhase() == GamePhase::Draw){
                        // Pesca una carta se disponibile (usa deck grafico già esistente)
                        if(cardsToDraw == 0 && !deck.isEmpty() && animations.empty()){
                            ++cardsToDraw; // trigger ciclo animazione di pescata già presente
                        }
                    }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(game){
                ...
                hudText.setPosition(sf::Vec15.f, 10.f);
                window.draw(hudText);
            }
        }
