...
int main(){
    ...
    // (Deprecato) animations & cardsToDraw sostituiti da DrawController
    int cardsToDraw = 0; // legacy counter (verrà rimosso quando tutte le pescate migrate)
    ...
    std::vector<Card>* handPtr = nullptr; // riferimento alla mano del Player corrente
    ...
    DrawController drawController = DrawController(); // nuovo controller animazioni pescata
    ...
    // Pesche iniziali demandate a game->start / DrawController
    };
            ...
            if(field.isAnimationFinished()) {
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        // Avvia la partita (pesche iniziali già gestite separatamente nel flusso grafico)
                        if(game && !game->isStarted()){
                            game->start(); // pesca iniziali popolano Player.hand
    ...
    // (Legacy pescata iniziale sostituita da DrawController nella fase Draw)
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            ...
            if(game){
                ...
                if(handPtr){
                    drawController.update(deltaTime, mousePressed, deck, *handPtr, windowSize, cardSize, spacing, y, HAND_MAXSIZE, deckSlotPos, textureNonFlipped, textureFlipped, textureManager);
