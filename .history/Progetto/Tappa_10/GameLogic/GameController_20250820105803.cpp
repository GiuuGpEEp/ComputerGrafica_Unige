...
void GameController::drawUI(sf::RenderWindow& win, sf::Font& font) {
    ...
    bar.setPosition(sf::Vector2f.f, 20.f);
...
bool GameController::tryNormalSummon(size_t handIndex, const sf::Vector2f& mousePos) {
    ...
    // Trova uno slot mostri libero sul tuo lato (uso Type::Monster + player id=1 ipotesi)
    // Qui semplifico: usa il primo slot libero di player 1
    // (Sostituisci con enumerazione degli slot effettivi per P1/P2)
    Slot* freeSlot = nullptr;
    auto monsterSlots = field.getMonsterSlotsForPlayer(1); // supponi esista; se non c'è dovrai implementarlo
