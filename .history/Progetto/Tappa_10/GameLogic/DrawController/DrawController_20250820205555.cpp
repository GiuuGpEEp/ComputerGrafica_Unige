...
void DrawController::update(
    ...
    if (cardsToDraw > 0 && animations.empty() && !deck.isEmpty() && static_cast<int>(hand.size()) < handMaxSize) {
    ...
    } else if(cardsToDraw > 0 && static_cast<int>(hand.size()) >= handMaxSize){
        // Mano piena: annulla eventuali pescate eccedenti
        cardsToDraw = 0;
    }
    if (!animations.empty()) {
        ...
        if (ph == DrawAnimationPhases::Done) {
            ...
            // Carta finale dell'animazione
            Card finished = animations.front().getCard();
            // Imposta texture frontale (se la logica richiede altro, sostituire qui)
            
            finished.setTexture(textureNonFlipped);
            hand.push_back(finished);
