    ...
    if(const auto* mb = ev.getIf<sf::Event::MouseButtonPressed>()) {
        if(mb->button == sf::Mouse::Button::Left) {
            if(phase == Phase::Main1 || phase == Phase::Main2) {
                ...
                auto& hand = *players[activePlayer].hand;
...
void GameController::doDrawPhase() {
    ...
    auto& deck = *players[activePlayer].deck;
    if(deck.isEmpty()) {
...
bool GameController::tryNormalSummon(size_t handIndex, const sf::Vector2f& mousePos) {
    ...
    //Sfrutto la funzione findSlotPosition che mi permette dato un tipo di carta di verificare se il mouse si trova sopra 
    //Uno slot libero adatto al tipo di carta, e ne ritorna la posizione
    auto& hand = *players[activePlayer].hand;
    if(findSlotPosition(mousePos, hand[handIndex], field, slotSize).has_value()) {
        sf::Vector2f slotPos = findSlotPosition(mousePos, hand[handIndex], field, slotSize).value();
        freeSlot = field.getSlotByPosition(slotPos);
    }

    if(!freeSlot) {
        std::cout << "[GameController] Nessuno slot libero trovato." << std::endl;
        return false;
    }
    ...
    auto& fields = *players[activePlayer].fieldCards;
    fields.push_back(c);
...
void GameController::doBattlePhaseAttack() {
    ...
    auto& fields = *players[activePlayer].fields;
