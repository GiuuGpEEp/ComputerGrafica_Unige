...
int main(){
    ...
    bool phaseJustEntered = false; // per trigger azioni one-shot (es. pescata)
    ...
    while(window.isOpen()){
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if (isDragging && draggingCardIndex.has_value()) {
                        ...
                        if (handPtr && findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).has_value()) {
                            // Tentativo di Normal Summon (solo nelle Main phases e se non già usata)
                            if(game && game->tryNormalSummon(draggingCardIndex.value())){
                                sf::Vector2f slotPos = findSlotPosition(mousePos, (*handPtr)[draggingCardIndex.value()], field, slotSize).value();
                                float fieldCardScale = 0.85f;
                                sf::Vector2f fieldCardSize(slotSize.x * fieldCardScale, slotSize.y * fieldCardScale);
                                // La carta è stata rimossa dalla mano dentro tryNormalSummon, recuperala da monsterZone per posizionarla graficamente
                                const auto& mz = game->getMonsterZone();
                                const Card& placed = mz.back();
                                Card visual = placed; // copia per dimensione/posizione locale
                                visual.setSize(fieldCardSize);
                                ...
                                visual.setPosition(centeredPos);
                                fieldCards.push_back(visual);
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (deck.isShuffleAnimationAdvancedFinished()) {
                        ...
                        if(game && !game->isStarted()){
                            ...
                            phaseJustEntered = true; // entrerà in Draw Phase => nessuna auto pescata addizionale finché c'è coda
                        }
                    }
                }
            }
        ...
        if(gamestate == GameState::Playing && !returnPopupActive){
            // Gestione automazioni fase Draw (una volta all'ingresso fase)
            if(game){
                if(phaseJustEntered){
                    if(game->getTurn().getPhase() == GamePhase::Draw && !deck.isEmpty()){
                        // Pescata automatica solo se non ci sono già pescate in coda (evita doppio trigger iniziale)
                        if(drawController.pending() == 0){
                ...
                if(handPtr){
                    drawController.update(deltaTime, mousePressed, deck, *handPtr, windowSize, cardSize, spacing, y, HAND_MAXSIZE, deckSlotPos, textureFlipped, textureManager);
