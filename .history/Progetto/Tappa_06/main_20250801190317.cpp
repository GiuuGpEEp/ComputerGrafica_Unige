...
struct CardAnimation{
    ...
    float duration;
    float elapsed;
    bool handSet = false; //Se true la carta è nella fase della transizione in cui passa in mano
};
...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize), "Progetto Tappa 06");
    ...
    //Vettore temporaneo per le animazioni delle carte
    std::vector<CardAnimation> cardsAnimations;
            ...
            if(gamestate == GameState::FieldVisible && clock.getElapsedTime().asSeconds() > 1.f) {
                ...
                for(int i=0; i<initialCard; ++i){
                    if(!deck.isEmpty()){
                        ...
                        //L'idea dell'animazione è di avere un punto intermedio, ovvero fuori dalla finestra
                        sf::Vector2f midPos = deckSlotPos - sf::Vector2f(0.f, 300.f); 

                        //Il punto finale invece è lo slot delle carte in mano
                        sf::Vector2f endPos = deckSlotPos - sf::Vector2f(0.f, 300.f); 
