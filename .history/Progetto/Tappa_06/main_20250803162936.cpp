...
int main(){
    ...
    // Variabili per gestire le animazioni delle carte
    std::vector<CardDrawAnimation> activeAnimations;
    activeAnimations.reserve(10);
    sf::Clock animationClock;
    int cardsToDrawCount = 0; // Contatore delle carte da pescare
    float cardDrawDelay = 0.3f; // Ritardo tra una carta e l'altra
    float lastCardDrawTime = 0.0f;
    ...
    while(window.isOpen()){
        while (const std::optional event = window.pollEvent()){
            ...
            if(gamestate == GameState::FieldVisible && clock.getElapsedTime().asSeconds() > 1.f) {
                ...
                // Inizia l'animazione di pesca delle carte iniziali
                cardsToDrawCount = initialCard;
                animationClock.restart();
                lastCardDrawTime = 0.0f;
            }
        ...
        // Gestione delle animazioni di pesca delle carte
        if (gamestate == GameState::Playing) {
            float animationTime = animationClock.getElapsedTime().asSeconds();
            float deltaTime = animationClock.restart().asSeconds();
            
            // Inizia una nuova animazione se necessario
            if (cardsToDrawCount > 0 && (animationTime - lastCardDrawTime) >= cardDrawDelay) {
                if (!deck.isEmpty()) {
                    Card drawnCard = deck.drawCard();
                    drawnCard.setSize(cardSize);
                    
                    // Calcola la posizione target nella mano
                    sf::Vector2f targetPos;
                    size_t futureIndex = cards.size() + activeAnimations.size();
                    
                    // Calcola temporaneamente la posizione (sarà ricalcolata quando la carta viene aggiunta)
                    float tempSpacing = spacing;
                    if (futureIndex >= HAND_MAXSIZE) {
                        float scalingFactor = static_cast<float>(HAND_MAXSIZE) / (futureIndex + 1);
                        tempSpacing = spacing * scalingFactor;
                    }
                    
                    float totalWidth = (futureIndex + 1) * cardSize.x + futureIndex * tempSpacing;
                    float startX = (windowSize.x - totalWidth) / 2.f;
                    targetPos = sf::Vector2f(startX + futureIndex * (cardSize.x + tempSpacing), y);
                    
                    // Crea e inizia l'animazione
                    CardDrawAnimation newAnimation;
                    startCardDrawAnimation(newAnimation, drawnCard, deckSlotPos, targetPos, 
                                         &textureFlipped, &textureNonFlipped, 1.5f);
                    
                    activeAnimations.push_back(newAnimation);
                    cardsToDrawCount--;
                    lastCardDrawTime = animationTime;
                }
            }
            
            // Aggiorna tutte le animazioni attive
            for (auto it = activeAnimations.begin(); it != activeAnimations.end();) {
                bool completed = updateCardDrawAnimation(*it, deltaTime, windowSize);
                if (completed) {
                    // Animazione completata, aggiungi la carta alla mano
                    cards.push_back(it->card);
                    it = activeAnimations.erase(it);
                    
                    // Ricalcola le posizioni della mano
        ...
        if(gamestate == GameState::Intro) {
            ...
            // Disegna le animazioni delle carte in corso
            for (const auto& animation : activeAnimations) {
                drawCardAnimation(window, animation);
            }
