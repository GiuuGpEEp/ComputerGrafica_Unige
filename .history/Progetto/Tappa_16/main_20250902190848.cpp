...
int main(){
    ...
    while (window.isOpen()) {
        ...
        while (const std::optional event = window.pollEvent()){
            ...
            // Click sinistro: se c'è una richiesta di selezione del proprio mostro pendente, consuma il click su una carta di campo valida
            if (const auto *mousePress = event->getIf<sf::Event::MouseButtonPressed>()){
                if(mousePress->button == M_MOUSE_LEFT && game && gamestate == GameState::Playing && !inputBlocked()){
                    if(game->hasPendingOwnMonsterChoice()){
                        int owner = game->getPendingOwnMonsterOwner().value_or(game->getTurn().getCurrentPlayerIndex());
                        sf::Vector2i mpos = sf::Mouse::getPosition(window);
                        if(owner == 0){
                            for(size_t i=0;i<fieldCards.size();++i){
                                if(fieldCards[i].isClicked(mpos)){
                                    // i è l'indice nella rappresentazione P1: mappa a indice zona logica (owner 0)
                                    int slotIdx = game->getMonsterSlotIndexAt(0, i);
                                    if(slotIdx < 0) slotIdx = static_cast<int>(i);
                                    // trova l'indice nella zona del proprietario che ha quello slot
                                    size_t zoneSize = game->getMonsterZone().size();
                                    size_t found = zoneSize;
                                    for(size_t z=0; z<zoneSize; ++z){ if(game->getMonsterSlotIndexAt(0, z) == slotIdx){ found = z; break; } }
                                    if(found < zoneSize){ (void)game->resolvePendingSelectOwnMonster(found); }
                                    break;
                                }
                            }
                        } else {
                            for(size_t i=0;i<oppFieldCards.size();++i){
                                if(oppFieldCards[i].isClicked(mpos)){
                                    ...
                                    size_t zoneSize2 = game->getOpponentMonsterZone().size();
                                    size_t found2 = zoneSize2;
                                    for(size_t z=0; z<zoneSize2; ++z){ if(game->getMonsterSlotIndexAt(1, z) == slotIdx2){ found2 = z; break; } }
                                    if(found2 < zoneSize2){ (void)game->resolvePendingSelectOwnMonster(found2); }
                                    break;
                                }
                            }
                        }
