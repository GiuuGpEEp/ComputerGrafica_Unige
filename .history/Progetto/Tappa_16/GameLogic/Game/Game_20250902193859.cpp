...
bool Game::tryNormalSummon(size_t handIndex){
    ...
    // Mark flags for newly summoned monster
    {
        ...
        // Track last normal summon (owner absolute idx + name)
        if(!monsterZones[cur].empty()){
            const auto &summoned = monsterZones[cur].back();
            lastNormalSummon = LastNormalSummon{ summoned.getName(), cur };
        }
    }
...
bool Game::resolvePendingAddFromDeck(size_t indexInFiltered){
    ...
    Deck &d = getDeckOf(owner);
    // Seleziona filtro in base alla modalità corrente
    auto filter = melodiaFilter;
    if(pendingAddMode.has_value() && *pendingAddMode == AddChoiceMode::Level1Dragon){
        filter = [](const Card& c){
    ...
    auto candidates = d.collectWhere(filter);
    if(indexInFiltered >= candidates.size()) return false;
    const std::string pickName = candidates[indexInFiltered].getName();
    auto picked = d.removeFirstByName(pickName);
    if(!picked.has_value()) return false;
    ...
    // Gestisci modalità
    if(pendingAddMode.has_value() && *pendingAddMode == AddChoiceMode::Melodia){
    ...
    pendingAdd.reset();
    pendingAddMode.reset();
    dispatcher.emit(GameEventType::CardMoved);
    return true;
}

void Game::cancelPendingAddFromDeck(){ pendingAdd.reset(); pendingAddMode.reset(); melodiaAddsRemaining = 0; }

std::optional<Game::AddChoiceMode> Game::getPendingAddMode() const{ return pendingAddMode; }

bool Game::requestAddLevel1DragonToHand(int playerIdx){
    if(playerIdx < 0 || playerIdx > 1) return false;
    if(pendingAdd.has_value() || pendingSend.has_value() || pendingGY.has_value() || pendingOwnMonster.has_value() || pendingSS.has_value()) return false;
    const Deck &d = getDeckOf(playerIdx);
    auto candidates = d.collectWhere([](const Card& c){
        if(c.getType() != Type::Monster) return false;
        const auto &feats = c.getFeatures();
    ...
    if(candidates.empty()) return false;
    pendingAdd = PendingAddDeck{ playerIdx };
    pendingAddMode = AddChoiceMode::Level1Dragon;
    dispatcher.emit(GameEventType::DeckAddChoiceRequested);
    return true;
}
...
bool Game::activateMonsterFromHand(size_t handIndex){
    ...
    IActivationEffect* act = effects.getActivationFor(hand[handIndex].getName());
    if(!act) return false;
    if(!act->canActivate(*this, cur)) return false;
    // Costo: manda questa carta dalla mano al Cimitero (scarto)
    Card self = hand[handIndex];
    hand.erase(hand.begin()+handIndex);
    graveyard[cur].push_back(self);
    lastSentToGrave = LastSentToGrave{ graveyard[cur].back().getName(), cur };
    dispatcher.emit(GameEventType::CardSentToGrave);
    dispatcher.emit(GameEventType::CardMoved);
    return act->resolve(*this, cur);
}
...
// ---- Nuove API: scelta di un proprio mostro sul campo ----
bool Game::requestSelectOwnMonsterOnField(int ownerIdx){
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    if(pendingOwnMonster.has_value() || pendingGY.has_value() || pendingSS.has_value() || pendingSend.has_value() || pendingDiscard.has_value() || pendingAdd.has_value()) return false;
    if(monsterZones[ownerIdx].empty()) return false;
    pendingOwnMonster = PendingOwnMonster{ ownerIdx };
    dispatcher.emit(GameEventType::OwnMonsterChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingOwnMonsterOwner() const{
    if(!pendingOwnMonster.has_value()) return std::nullopt;
    return pendingOwnMonster->ownerIdx;
}
...
bool Game::resolvePendingSelectOwnMonster(size_t indexInOwnerZone){
    ...
    if(indexInOwnerZone >= monsterZones[owner].size()) return false;
    // Sposta il mostro selezionato al Cimitero del suo owner
    destroyMonster(owner, indexInOwnerZone);
...
static inline bool _isBlueEyesMonster(const Card& c){
    if(c.getType() != Type::Monster) return false;
    return c.getName().find("Occhi di Blu") != std::string::npos;
}

bool Game::addFirstBlueEyesMonsterToHand(int ownerIdx){
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    Deck &d = getDeckOf(ownerIdx);
    auto candidates = d.collectWhere(_isBlueEyesMonster);
    if(candidates.empty()) return false;
    const std::string pickName = candidates.front().getName();
    auto picked = d.removeFirstByName(pickName);
    if(!picked.has_value()) return false;
    mirrorExternalDeckRemoveByName(pickName, &d);
    addToHand(ownerIdx, std::move(*picked));
    return true;
}
