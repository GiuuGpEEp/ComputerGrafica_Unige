...
static bool isBlueEyesOrSpirit(const Card& c){
    ...
    return (n == std::string("Drago Bianco Occhi Blu")) || (n == std::string("Drago Spirito Occhi Blu"));
}
...
bool Game::declareAttack(size_t attackerIndex, std::optional<size_t> maybeTargetIndex){
    ...
    if(maybeTargetIndex.has_value()){
        ...
        // Maiden with Eyes of Blue (Fanciulla) — trigger when targeted for an attack
        // If the target is Maiden, prompt the defending player to SS a Blue-Eyes/Spirit from Hand/Deck/GY.
        {
            const std::string &tName = monsterZones[opp][tIdx].getName();
            if(tName == std::string("Fanciulla con gli Occhi di Blu") || tName == std::string("Fanciulla con Occhi di Blu")){
                if(!hasPendingBlueEyesSSChoice()){
                    (void)requestBlueEyesOrSpiritChoice(opp);
                }
            }
        }
        ...
        // Trigger semplificato: se il bersaglio dell'attacco è "Fanciulla con gli Occhi di Blu",
        // richiedi la scelta per Evocare Specialmente un Drago Bianco/Spirito dagli slot (mano/deck/cimitero)
        // Nota: per ora non nega l'attacco e non gestisce il trigger "bersagliata da un effetto".
        {
            const std::string &targetName = monsterZones[opp][tIdx].getName();
            if(targetName == std::string("Fanciulla con gli Occhi di Blu")){
                // Evita di sovrapporre richieste se è già attivo un flusso BE/Spirit
                if(!hasPendingBlueEyesSSChoice()){
                    (void)requestBlueEyesOrSpiritChoice(opp);
                }
            }
