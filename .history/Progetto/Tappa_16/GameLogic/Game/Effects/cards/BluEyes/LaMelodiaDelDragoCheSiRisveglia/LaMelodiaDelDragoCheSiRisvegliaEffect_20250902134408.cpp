...
bool LaMelodiaDelDragoCheSiRisvegliaEffect::resolve(Game& game, int ownerIdx){
    // 1) Richiedi scarto dalla mano del proprietario
    if(!game.requestHandDiscard(ownerIdx)) return false;
    // 2) Dopo lo scarto, la Game chiamerà automaticamente requestAddFromDeck(ownerIdx) (vedi resolvePendingHandDiscard)
    // Imposta fino a due aggiunte possibili; l'utente potra' fermarsi con ESC dall'overlay
    game.setMelodiaAddsRemaining(2);
