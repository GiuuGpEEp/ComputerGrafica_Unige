...
Game::Game(const Player& p1, const Player& p2)
    ...
    // Forward anche per SpecialSummon post-move
    dispatcher.subscribe(GameEventType::MonsterSpecialSummoned, [this]() { this->dispatchEffects(GameEventType::MonsterSpecialSummoned); });
...
bool Game::tryNormalSummon(size_t handIndex){
    ...
    // Delega a tryNormalSummonToSlot che esegue il move, setta i flag e emette l'evento
    return tryNormalSummonToSlot(handIndex, *freeSlot);
}
...
bool Game::resolvePendingHandDiscard(size_t handIndex){
    ...
    if(nextAddAfterDiscardOwner.has_value()){
        ...
        // Avvia richiesta di aggiunta dal Deck (filtrata per Melodia). Il contatore melodiaAddsRemaining
        // verra' impostato dall'effetto che ha richiesto lo scarto.
...
// ---- Nuove API: scelta aggiunta dal Deck alla mano ----
 // file-scope helper per filtro Melodia
...
bool Game::resolvePendingAddFromDeck(size_t indexInFiltered){
    ...
    pendingAdd.reset();
    // Se stiamo gestendo La Melodia, possiamo permettere una seconda aggiunta
    if(melodiaAddsRemaining > 0){
        melodiaAddsRemaining -= 1;
        if(melodiaAddsRemaining > 0){
            // Riapri la scelta per la seconda carta se ci sono ancora candidati
            const auto more = d.collectWhere(melodiaFilter);
            if(!more.empty()){
                pendingAdd = PendingAddDeck{ owner };
                dispatcher.emit(GameEventType::DeckAddChoiceRequested);
            } else {
                melodiaAddsRemaining = 0;
            }
        }
    }
    return true;
...
void Game::cancelPendingAddFromDeck(){ pendingAdd.reset(); melodiaAddsRemaining = 0; }

// ---- Selezione proprio mostro sul terreno (per Saggio) ----
bool Game::requestSelectOwnMonsterOnField(int ownerIdx, OwnMonsterFollowup followup){
    if(pendingOwnMonster.has_value()) return false;
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    // Richiedi solo se esiste almeno un mostro sul terreno dell'owner
    const auto &zone = (ownerIdx == turn.getCurrentPlayerIndex()) ? getMonsterZone() : getOpponentMonsterZone();
    if(zone.empty()) return false;
    pendingOwnMonster = PendingOwnMonster{ ownerIdx, followup };
    dispatcher.emit(GameEventType::OwnMonsterChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingOwnMonsterOwner() const{
    if(!pendingOwnMonster.has_value()) return std::nullopt; return pendingOwnMonster->ownerIdx;
}

bool Game::resolvePendingSelectOwnMonster(size_t indexInOwnerZone){
    if(!pendingOwnMonster.has_value()) return false;
    int owner = pendingOwnMonster->ownerIdx;
    OwnMonsterFollowup f = pendingOwnMonster->followup;
    // Mappa l'indice rispetto al lato corrente del turno
    int cur = turn.getCurrentPlayerIndex();
    if(owner != cur){
        // Per semplicità limitiamo la selezione al giocatore di turno
        return false;
    }
    auto &zone = monsterZones[cur];
    if(indexInOwnerZone >= zone.size()) return false;
    // Per l'uso Saggio: manda al Cimitero il mostro scelto
    if(f == OwnMonsterFollowup::SageSacrifice){
        Card sacrificed = zone[indexInOwnerZone];
        zone.erase(zone.begin()+indexInOwnerZone);
        // Allinea flag
        if(indexInOwnerZone < monsterHasAttacked[cur].size()) monsterHasAttacked[cur].erase(monsterHasAttacked[cur].begin()+indexInOwnerZone);
        if(indexInOwnerZone < monsterIsDefense[cur].size()) monsterIsDefense[cur].erase(monsterIsDefense[cur].begin()+indexInOwnerZone);
        if(indexInOwnerZone < monsterIsFaceDown[cur].size()) monsterIsFaceDown[cur].erase(monsterIsFaceDown[cur].begin()+indexInOwnerZone);
        if(indexInOwnerZone < monsterSummonedThisTurn[cur].size()) monsterSummonedThisTurn[cur].erase(monsterSummonedThisTurn[cur].begin()+indexInOwnerZone);
        if(indexInOwnerZone < monsterPositionChangedThisTurn[cur].size()) monsterPositionChangedThisTurn[cur].erase(monsterPositionChangedThisTurn[cur].begin()+indexInOwnerZone);
        // Allinea anche la mappatura degli slot: libera lo slot occupato dal mostro rimosso
        if(indexInOwnerZone < monsterSlotIndex[cur].size()){
            monsterSlotIndex[cur].erase(monsterSlotIndex[cur].begin()+indexInOwnerZone);
        }
        dispatcher.emit(GameEventType::CardMoved);
        graveyard[cur].push_back(std::move(sacrificed));
        lastSentToGrave = LastSentToGrave{ graveyard[cur].back().getName(), cur };
        dispatcher.emit(GameEventType::CardSentToGrave);
    // Dopo il sacrificio richiesto da Saggio, avvia la scelta del BEWD/Spirit da Mano/Deck/Cimitero
    (void)requestBlueEyesOrSpiritChoice(cur);
    }
    pendingOwnMonster.reset();
    return true;
}

void Game::cancelPendingSelectOwnMonster(){ pendingOwnMonster.reset(); }

// ---- Add Drago Livello 1 dal Deck (Saggio on-summon) ----
static bool isLevel1Dragon(const Card& c){
...
bool Game::requestAddLevel1DragonFromDeck(int playerIdx){
    if(pendingAddLvl1.has_value() || pendingAdd.has_value()) return false;
    if(playerIdx < 0 || playerIdx > 1) return false;
    const Deck &d = getDeckOf(playerIdx);
    auto candidates = d.collectWhere(isLevel1Dragon);
    if(candidates.empty()) return false;
    pendingAddLvl1 = PendingAddLvl1{ playerIdx };
    dispatcher.emit(GameEventType::DeckAddLevel1DragonChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingAddLevel1DragonOwner() const{
    if(!pendingAddLvl1.has_value()) return std::nullopt; return pendingAddLvl1->ownerIdx;
}

bool Game::resolvePendingAddLevel1Dragon(size_t indexInFiltered){
    if(!pendingAddLvl1.has_value()) return false;
    int owner = pendingAddLvl1->ownerIdx;
    Deck &d = getDeckOf(owner);
    auto candidates = d.collectWhere(isLevel1Dragon);
    if(indexInFiltered >= candidates.size()) return false;
    const std::string pickName = candidates[indexInFiltered].getName();
    ...
    mirrorExternalDeckRemoveByName(pickName, &d);
    addToHand(owner, std::move(*picked));
    pendingAddLvl1.reset();
    return true;
}

void Game::cancelPendingAddLevel1Dragon(){ pendingAddLvl1.reset(); }

// ---- Attivazione effetto Mostro dalla mano (Saggio) ----
bool Game::canActivateMonsterFromHand(size_t handIndex){
    const auto &hand = players[turn.getCurrentPlayerIndex()].getHand();
    if(handIndex >= hand.size()) return false;
    if(hand[handIndex].getType() != Type::Monster) return false;
    const std::string &n = hand[handIndex].getName();
    if(!(n == std::string("Saggio con Occhi di Blu") || n == std::string("Saggio con gli Occhi Blu"))) return false;
    // serve almeno 1 mostro controllato e spazio per SS
    int cur = turn.getCurrentPlayerIndex();
    if(monsterZones[cur].empty()) return false;
    if(monsterZones[cur].size() >= MONSTER_ZONE_SIZE) return false; // serve spazio per l'evocazione che segue
    return true;
}

bool Game::activateMonsterFromHand(size_t handIndex){
    if(!canActivateMonsterFromHand(handIndex)) return false;
    int cur = turn.getCurrentPlayerIndex();
    auto &hand = players[cur].getHand();
    // Scarta la carta come costo
    Card self = hand[handIndex];
    hand.erase(hand.begin()+handIndex);
    graveyard[cur].push_back(std::move(self));
    lastSentToGrave = LastSentToGrave{ graveyard[cur].back().getName(), cur };
    dispatcher.emit(GameEventType::CardSentToGrave);
    dispatcher.emit(GameEventType::CardMoved);
    // Richiedi selezione di un proprio mostro da mandare al Cimitero
    (void)requestSelectOwnMonsterOnField(cur, OwnMonsterFollowup::SageSacrifice);
    // La successiva SS dei BEWD/Spirit verrà gestita dall'IActivationEffect dedicato (in futuro)
    return true;
}

// ---- Scelta BEWD/Spirit post Saggio e SS con scelta posizione ----
static bool isBlueEyesOrSpirit(const Card& c){
    if(c.getType() != Type::Monster) return false;
    const std::string &n = c.getName();
    return (n == std::string("Drago Bianco Occhi Blu")) || (n == std::string("Drago Spirito del Bianco"));
}

bool Game::requestBlueEyesOrSpiritChoice(int ownerIdx){
    if(pendingBlueEyesChoice.has_value()) return false;
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    // Serve spazio nella zona mostri per poter evocare specialmente
    if(monsterZones[ownerIdx].size() >= MONSTER_ZONE_SIZE) return false;
    // Raccogli candidati da Mano / Deck / Cimitero
    auto &hand = players[ownerIdx].getHand();
    std::vector<Card> handCands; handCands.reserve(hand.size());
    for(const auto &c : hand){ if(isBlueEyesOrSpirit(c)) handCands.push_back(c); }
    const Deck &d = getDeckOf(ownerIdx);
    auto deckCands = d.collectWhere(isBlueEyesOrSpirit);
    auto &gy = graveyard[ownerIdx];
    std::vector<Card> gyCands; gyCands.reserve(gy.size());
    for(const auto &c : gy){ if(isBlueEyesOrSpirit(c)) gyCands.push_back(c); }
    if(handCands.empty() && deckCands.empty() && gyCands.empty()) return false;
    pendingBlueEyesChoice = PendingBlueEyesChoice{ ownerIdx, std::move(handCands), std::move(deckCands), std::move(gyCands) };
    dispatcher.emit(GameEventType::BlueEyesSSChoiceRequested);
    return true;
}

bool Game::resolvePendingBlueEyesOrSpiritChoice(size_t indexInFlattened){
    if(!pendingBlueEyesChoice.has_value()) return false;
    int owner = pendingBlueEyesChoice->ownerIdx;
    size_t handCount = pendingBlueEyesChoice->fromHand.size();
    size_t deckCount = pendingBlueEyesChoice->fromDeck.size();
    size_t gyCount   = pendingBlueEyesChoice->fromGY.size();
    size_t total = handCount + deckCount + gyCount;
    if(indexInFlattened >= total) return false;
    enum class Src { Hand, Deck, GY } src;
    size_t localIdx = indexInFlattened;
    if(localIdx < handCount){ src = Src::Hand; }
    else if((localIdx -= handCount) < deckCount){ src = Src::Deck; }
    else { localIdx -= deckCount; src = Src::GY; }
    // Determina il nome selezionato dalla snapshot per stabilità
    std::string selName;
    switch(src){
        case Src::Hand: selName = pendingBlueEyesChoice->fromHand[localIdx].getName(); break;
        case Src::Deck: selName = pendingBlueEyesChoice->fromDeck[localIdx].getName(); break;
        case Src::GY:   selName = pendingBlueEyesChoice->fromGY[localIdx].getName(); break;
    }
    // Estrai la carta dalla sorgente reale
    std::optional<Card> picked;
    if(src == Src::Hand){
        auto &hand = players[owner].getHand();
        auto it = std::find_if(hand.begin(), hand.end(), [&](const Card& c){ return c.getName() == selName; });
        if(it == hand.end()) return false;
        picked.emplace(*it);
        hand.erase(it);
        dispatcher.emit(GameEventType::CardMoved);
    } else if(src == Src::Deck){
        Deck &deckRef = getDeckOf(owner);
        auto removed = deckRef.removeFirstByName(selName);
        if(!removed.has_value()) return false;
        mirrorExternalDeckRemoveByName(selName, &deckRef);
        picked.emplace(std::move(*removed));
    } else { // GY
        auto &gy = graveyard[owner];
        auto it = std::find_if(gy.begin(), gy.end(), [&](const Card& c){ return c.getName() == selName; });
        if(it == gy.end()) return false;
        picked.emplace(*it);
        gy.erase(it);
        dispatcher.emit(GameEventType::CardMoved);
    }
    // Chiudi pending e avvia la SS con scelta posizione
    pendingBlueEyesChoice.reset();
    if(!picked.has_value()) return false;
    return requestSpecialSummonWithChoice(owner, std::move(*picked));
}

void Game::cancelPendingBlueEyesOrSpiritChoice(){ pendingBlueEyesChoice.reset(); }

// ---- Nuove API: scelta dal Cimitero (Richiamo del Posseduto) ----
bool Game::requestSelectFromGraveyard(int playerIdx){
    if(pendingGY.has_value() || pendingSS.has_value() || pendingSend.has_value() || pendingDiscard.has_value() || pendingAdd.has_value()) return false;
    if(playerIdx < 0 || playerIdx > 1) return false;
    auto &gy = getGraveyardOf(playerIdx);
    // Richiedi solo se esiste almeno un mostro nel cimitero
    bool hasMonster = std::any_of(gy.begin(), gy.end(), [](const Card& c){ return c.getType() == Type::Monster; });
    if(!hasMonster) return false;
    pendingGY = PendingGY{ playerIdx };
    dispatcher.emit(GameEventType::GraveyardChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingGraveyardOwner() const{
    if(!pendingGY.has_value()) return std::nullopt;
    return pendingGY->ownerIdx;
}

bool Game::resolvePendingSelectFromGraveyard(size_t indexInGY){
    if(!pendingGY.has_value()) return false;
    int owner = pendingGY->ownerIdx;
    auto &gy = getGraveyardOf(owner);
    if(indexInGY >= gy.size()) return false;
    if(gy[indexInGY].getType() != Type::Monster) return false; // per ora permettiamo solo mostri
    // Estrai la carta scelta
    Card picked = gy[indexInGY];
    gy.erase(gy.begin() + indexInGY);
    pendingGY.reset();
    // Evoca direttamente in Attacco scoperto (semplificazione dell'effetto)
    return specialSummonToMonsterZone(owner, std::move(picked), /*defense*/false, /*faceDown*/false);
}

void Game::cancelPendingSelectFromGraveyard(){ pendingGY.reset(); }

// ---- Selezione M/T dell'avversario (inclusa Magia Terreno) ----
bool Game::requestSelectOpponentSpellTrap(int ownerIdx){
    if(pendingOppST.has_value()) return false;
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    // Verifica esistenza di almeno una M/T o Field Spell sul lato dell'avversario
    int opp = 1 - ownerIdx;
    const auto &oppST = spellTraps[opp];
    bool hasAny = !oppST.empty() || fieldSpell[opp].has_value();
    if(!hasAny) return false;
    pendingOppST = PendingOpponentST{ ownerIdx };
    dispatcher.emit(GameEventType::OpponentSpellTrapChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingOpponentSTOwner() const{
    if(!pendingOppST.has_value()) return std::nullopt; return pendingOppST->ownerIdx;
}

bool Game::resolvePendingSelectOpponentSpellTrap(size_t indexInFlattened){
    if(!pendingOppST.has_value()) return false;
    int owner = pendingOppST->ownerIdx;
    int opp = 1 - owner;
    size_t stCount = spellTraps[opp].size();
    bool hasField = fieldSpell[opp].has_value();
    size_t total = stCount + (hasField ? 1 : 0);
    if(indexInFlattened >= total) return false;
    // Se indice < stCount -> banish dalla zona ST avversaria; altrimenti banish la Field Spell
    if(indexInFlattened < stCount){
        // Per poter usare banishFrom, dobbiamo essere "sul lato" dell'avversario? moveCard/banish usano current().
        // Qui usiamo una rimozione manuale coerente con gli helper interni, mantenendo flag sincronizzati.
        // Switch temporaneo di prospettiva non esiste; quindi operiamo direttamente sui vettori dell'avversario.
        size_t idx = indexInFlattened;
        if(idx >= spellTraps[opp].size()) return false;
        // Rimuovi flags paralleli
        if(idx < stIsFaceDown[opp].size()) stIsFaceDown[opp].erase(stIsFaceDown[opp].begin()+idx);
        if(idx < stSetThisTurn[opp].size()) stSetThisTurn[opp].erase(stSetThisTurn[opp].begin()+idx);
        // Sposta carta in banished dell'avversario
        Card c = spellTraps[opp][idx];
        spellTraps[opp].erase(spellTraps[opp].begin()+idx);
        banished[opp].push_back(std::move(c));
        dispatcher.emit(GameEventType::CardMoved);
    } else {
        // Field Spell
        if(!hasField) return false;
        // Sposta Field Spell in banished dell'avversario
        banished[opp].push_back(std::move(*fieldSpell[opp]));
        fieldSpell[opp].reset();
        dispatcher.emit(GameEventType::CardMoved);
    }
    pendingOppST.reset();
    return true;
...
void Game::cancelPendingSelectOpponentSpellTrap(){ pendingOppST.reset(); }

// ---- Selezione M/T avversaria (per Drago Spirito del Bianco) ----
bool Game::requestSelectOpponentSpellTrap(int ownerIdx, bool includeFieldSpell){
    if(pendingOppST.has_value() || pendingSS.has_value() || pendingSend.has_value() || pendingDiscard.has_value() || pendingAdd.has_value() || pendingGY.has_value() || pendingOwnMonster.has_value()) return false;
    if(ownerIdx < 0 || ownerIdx > 1) return false;
    // Avversario assoluto
    int opponentIdx = 1 - ownerIdx;
    const auto &oppST = spellTraps[opponentIdx];
    bool hasAny = !oppST.empty();
    if(includeFieldSpell){ hasAny = hasAny || fieldSpell[opponentIdx].has_value(); }
    if(!hasAny) return false;
    pendingOppST = PendingOpponentST{ ownerIdx, includeFieldSpell };
    dispatcher.emit(GameEventType::OpponentSpellTrapChoiceRequested);
    return true;
}

std::optional<int> Game::getPendingOpponentSpellTrapOwner() const{
    if(!pendingOppST.has_value()) return std::nullopt;
    return pendingOppST->ownerIdx;
}

bool Game::resolvePendingSelectOpponentSpellTrap(size_t indexInList){
    if(!pendingOppST.has_value()) return false;
    int owner = pendingOppST->ownerIdx;
    int opp = 1 - owner;
    // Lista virtuale: prima tutte le M/T in ordine, poi (se includeField e presente) il Field Spell come ultimo elemento
    size_t stCount = spellTraps[opp].size();
    bool includeField = pendingOppST->includeField;
    bool hasField = includeField && fieldSpell[opp].has_value();
    size_t total = stCount + (hasField ? 1 : 0);
    if(total == 0) return false;
    if(indexInList >= total) return false;
    if(indexInList < stCount){
        // Rimuovi dalla zona ST dell'avversario: usiamo la prospettiva del giocatore CORRENTE per moveCard, ma qui potremmo non essere nel turno di owner
        // moveCard opera sul current(). Per semplicità, effettuiamo una rimozione manuale coerente con moveCard per ST avversario.
        // Hack: cambia temporaneamente il turno logico per riusare moveCard. Evitiamo: meglio rimuovere manualmente replicando le parti necessarie.
        // Eseguiamo una banish manuale sulla ST dell'avversario
        if(indexInList >= spellTraps[opp].size()) return false;
        Card c = spellTraps[opp][indexInList];
        spellTraps[opp].erase(spellTraps[opp].begin()+indexInList);
        if(indexInList < stIsFaceDown[opp].size()) stIsFaceDown[opp].erase(stIsFaceDown[opp].begin()+indexInList);
        if(indexInList < stSetThisTurn[opp].size()) stSetThisTurn[opp].erase(stSetThisTurn[opp].begin()+indexInList);
        // Metti nella banished dell'OPPONENTE (poiché la carta era dell'opponente)
        banished[opp].push_back(std::move(c));
        dispatcher.emit(GameEventType::CardMoved);
    } else {
        // Field Spell come ultimo elemento
        if(!hasField) return false;
        // Sposta la field spell avversaria in banished
        Card c = *fieldSpell[opp];
        fieldSpell[opp].reset();
        banished[opp].push_back(std::move(c));
        dispatcher.emit(GameEventType::CardMoved);
    }
...
bool Game::declareAttack(size_t attackerIndex, std::optional<size_t> maybeTargetIndex){
    ...
    int cur = turn.getCurrentPlayerIndex();
    int opp = 1 - cur;
    // Salva il contesto e inoltra l'evento al dispatcher ed al sistema effetti
    lastDeclaredAttack = std::optional<Game::AttackContext>{ Game::AttackContext{ cur, attackerIndex, opp, maybeTargetIndex } };
    ...
    // Rimozione: gestione inline della Fanciulla eliminata (ora in effetto separato)
...
bool Game::activateSetSpellTrapFor(int playerIdx, size_t zoneIndex){
    ...
    // Non consentire di "riattivare" carte gia' scoperte nello ST (es. Continue rimaste scoperte)
    bool isFaceDown = (zoneIndex < stIsFaceDown[playerIdx].size() ? stIsFaceDown[playerIdx][zoneIndex] : false);
...
bool Game::canActivateSetSpellTrapFor(int playerIdx, size_t zoneIndex){
    if(playerIdx < 0 || playerIdx > 1) return false;
    if(zoneIndex >= spellTraps[playerIdx].size()) return false;
    // Deve essere coperta per poter essere attivata dalla zona ST
    bool isFaceDown = (zoneIndex < stIsFaceDown[playerIdx].size() ? stIsFaceDown[playerIdx][zoneIndex] : false);
    if(!isFaceDown) return false;
    bool setThisTurn = (zoneIndex < stSetThisTurn[playerIdx].size() ? stSetThisTurn[playerIdx][zoneIndex] : false);
    Attribute attr = spellTraps[playerIdx][zoneIndex].getAttribute();
