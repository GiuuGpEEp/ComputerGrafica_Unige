...
public:
    ...
    bool requestSelectOpponentSpellTrap(int ownerIdx);
    bool hasPendingOpponentSTChoice() const { return pendingOppST.has_value(); }
    std::optional<int> getPendingOpponentSTOwner() const;
    // indexInFlattened: prima tutte le M/T nella zona avversaria [0..n-1], poi eventualmente 1 voce per Field Spell (alla fine)
    bool resolvePendingSelectOpponentSpellTrap(size_t indexInFlattened);
    ...
    // Stato pending per scelta M/T dell'avversario (inclusa Magia Terreno)
    struct PendingOpponentST { int ownerIdx; };
