...
public:
    ...
    // Variante: richiesta di aggiungere un Drago di Livello 1 dal Deck alla mano (per Saggio on-summon)
    bool requestAddLevel1DragonToHand(int playerIdx);
    // Query: true se la pending add è in modalità "Level1Dragon" (altrimenti è Melodia)
    enum class AddChoiceMode { Melodia, Level1Dragon };
    std::optional<AddChoiceMode> getPendingAddMode() const;
    ...
    // Selezione di un proprio mostro sul campo (owner assoluto: 0 = P1, 1 = P2)
    // Flusso: request -> UI fa cliccare una carta nella zona mostri di owner; resolve riceve l'indice nella zona del relativo owner
    bool requestSelectOwnMonsterOnField(int ownerIdx);
    ...
    // Helper: aggiunge alla mano del giocatore indicato il primo mostro "Occhi di Blu" trovato nel suo Deck
    bool addFirstBlueEyesMonsterToHand(int ownerIdx);

    // Attivazione di effetti Mostro dalla mano (per carte come "Saggio con Occhi di Blu")
    bool canActivateMonsterFromHand(size_t handIndex) const;
    ...
    // Tracciamento ultima Normal Summon effettuata (nome e proprietario assoluto 0/1)
    struct LastNormalSummon { std::string name; int ownerIdx; };
    std::optional<LastNormalSummon> getLastNormalSummon() const { return lastNormalSummon; }
    void clearLastNormalSummon() { lastNormalSummon.reset(); }
...
public:
    ...
    // Tracciamento minimale per trigger "Normal Summon" della carta specifica
    std::optional<LastNormalSummon> lastNormalSummon;
    ...
    // Modalità per la pending add (di default Melodia)
    std::optional<AddChoiceMode> pendingAddMode;
    // Stato pending per scelta bersaglio dal Cimitero
    struct PendingGY { int ownerIdx; };
    std::optional<PendingGY> pendingGY;
    // Stato pending per scelta di un proprio mostro sul campo
    struct PendingOwnMonster { int ownerIdx; };
