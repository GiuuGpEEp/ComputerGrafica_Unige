...
- AppHandlers --> modfica della gestione degli eventi
  - Quando ricevo eventi di tipo `SpellActivated`/`TrapActivated`: aggiorno la UI, controllo quali carte sono attivabili e invoca l’apertura del prompt di risposta (`openResponsePromptIfAny`).
...
- UI (`main.cpp`)
  ...
  - `closeResponsePrompt`: interpretato come “pass” per P1; per ora esegue anche un auto-pass dell’avversario per far progredire/chiudere la chain (comportamento semplificato).
