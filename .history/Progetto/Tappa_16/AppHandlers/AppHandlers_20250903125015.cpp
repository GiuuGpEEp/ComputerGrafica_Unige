...
void AppHandlers::attachGameHandlers(Game &g, const Context &ctx){
    ...
    g.events().subscribe(GameEventType::DeckAddChoiceRequested, [&](){
        ...
        // Heuristica: se sul campo del current owner c'è Saggio con Occhi di Blu e non stiamo in Melodia multi-pick,
        // filtra per Draghi di Livello 1.
        auto level1DragonFilter = [](const Card& c){
        ...
        auto melodiaFilter = [](const Card& c){
    ...
    // Richiesta scelta di un proprio mostro (per Saggio)
    g.events().subscribe(GameEventType::OwnMonsterChoiceRequested, [&](){
        // Determina il proprietario della scelta pending (fallback al giocatore di turno)
        auto ownerOpt = g.getPendingOwnMonsterOwner();
        int owner = ownerOpt.has_value() ? *ownerOpt : g.getTurn().getCurrentPlayerIndex();
        // Se è il bot (P2), risolvi automaticamente scegliendo un mostro casuale senza aprire prompt
        if(owner == 1){
            const auto &zone = (owner == g.getTurn().getCurrentPlayerIndex()) ? g.getMonsterZone() : g.getOpponentMonsterZone();
            if(zone.empty()){ g.cancelPendingSelectOwnMonster(); return; }
            std::uniform_int_distribution<int> dist(0, static_cast<int>(zone.size())-1);
            size_t pick = static_cast<size_t>(dist(_apphandlers_rand));
            (void)g.resolvePendingSelectOwnMonster(pick);
            return;
        }
        // Human (P1): nessun overlay, clic diretto su un proprio mostro in campo (gestito da InputController/Main)
        if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Seleziona un tuo mostro da mandare al Cimitero");
        if(ctx.stopAllInputActions) ctx.stopAllInputActions();
    });

    // Aggiunta dal Deck: Drago di Livello 1 (Saggio on-summon)
    g.events().subscribe(GameEventType::DeckAddLevel1DragonChoiceRequested, [&](){
        if(ctx.closeResponsePrompt) ctx.closeResponsePrompt();
        auto ownerOpt = g.getPendingAddLevel1DragonOwner();
        int owner = ownerOpt.has_value() ? *ownerOpt : g.getTurn().getCurrentPlayerIndex();
        if(owner == 1){
            const Deck &d = g.getDeckOf(owner);
            auto candidates = d.collectWhere([](const Card& c){
                if(c.getType() != Type::Monster) return false; auto lv=c.getLevelOrRank(); if(!lv.has_value()||*lv!=1) return false; const auto &f=c.getFeatures(); return std::find(f.begin(), f.end(), Feature::Drago)!=f.end();
            });
            if(candidates.empty()){ g.cancelPendingAddLevel1Dragon(); return; }
            std::uniform_int_distribution<int> dist(0, static_cast<int>(candidates.size())-1);
            int pick = dist(_apphandlers_rand);
            (void)g.resolvePendingAddLevel1Dragon(static_cast<size_t>(pick));
            return;
        }
        if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(true);
        if(ctx.setDeckSendCandidates){
            ...
            ctx.setDeckSendCandidates(candidates);
            if(candidates.empty()){
                g.cancelPendingAddLevel1Dragon();
                if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(false);
                if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Nessun Drago di Livello 1");
                return;
            }
            if(ctx.stopAllInputActions) ctx.stopAllInputActions();
        }
    ...
    // Scelta BEWD/Spirit dopo il sacrificio del Saggio: Mano/Deck/Cimitero
    g.events().subscribe(GameEventType::BlueEyesSSChoiceRequested, [&](){
        if(ctx.closeResponsePrompt) ctx.closeResponsePrompt();
        ...
        if(owner == 1){
            std::vector<Card> all;
            // Hand
            const auto &hand = g.getHandOf(owner);
            for(const auto &c : hand) if(isBlueEyesOrSpirit(c)) all.push_back(c);
            // Deck
            const Deck &d = g.getDeckOf(owner);
            auto deckCands = d.collectWhere(isBlueEyesOrSpirit);
            all.insert(all.end(), deckCands.begin(), deckCands.end());
            // GY
            const auto &gy = g.getGraveyardOf(owner);
            for(const auto &c : gy) if(isBlueEyesOrSpirit(c)) all.push_back(c);
            if(all.empty()){ g.cancelPendingBlueEyesOrSpiritChoice(); if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Nessun bersaglio BEWD/Spirit"); return; }
            std::uniform_int_distribution<int> dist(0, static_cast<int>(all.size())-1);
            int pick = dist(_apphandlers_rand);
            (void)g.resolvePendingBlueEyesOrSpiritChoice(static_cast<size_t>(pick));
            return;
        }
        // Human: mostra overlay con lista concatenata (mano, deck, cimitero)
        if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(true);
        if(ctx.setDeckSendOwner) ctx.setDeckSendOwner(owner);
        if(ctx.setDeckSendCandidates){
            std::vector<Card> all;
            ...
            const auto &gy = g.getGraveyardOf(owner);
            for(const auto &c : gy) if(isBlueEyesOrSpirit(c)) all.push_back(c);
            ctx.setDeckSendCandidates(all);
            if(all.empty()){
                g.cancelPendingBlueEyesOrSpiritChoice();
                if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(false);
                if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Nessun bersaglio BEWD/Spirit");
                return;
            }
            if(ctx.stopAllInputActions) ctx.stopAllInputActions();
        }
    ...
    // Scelta M/T avversaria (inclusa Magia Terreno) per effetti tipo Drago Spirito del Bianco
    g.events().subscribe(GameEventType::OpponentSpellTrapChoiceRequested, [&](){
        // Chiudi response prompt durante la modale
        if(ctx.closeResponsePrompt) ctx.closeResponsePrompt();
        auto ownerOpt = g.getPendingOpponentSTOwner();
        int owner = ownerOpt.has_value() ? *ownerOpt : g.getTurn().getCurrentPlayerIndex();
        int opp = 1 - owner;
        const auto &oppST = g.getSpellTrapZone(); // questo è del current, quindi adeguiamo:
        const auto &stZone = (opp == g.getTurn().getCurrentPlayerIndex()) ? g.getSpellTrapZone() : g.getOpponentSpellTrapZone();
        bool hasField = g.getFieldSpellOf(opp).has_value();
        size_t total = stZone.size() + (hasField ? 1 : 0);
        if(owner == 1){
            // AI: scelta casuale
            if(total == 0){ g.cancelPendingSelectOpponentSpellTrap(); return; }
            std::uniform_int_distribution<int> dist(0, static_cast<int>(total)-1);
            size_t pick = static_cast<size_t>(dist(_apphandlers_rand));
            (void)g.resolvePendingSelectOpponentSpellTrap(pick);
            return;
        }
        // Human: in assenza di un overlay dedicato, fallback a scelta random per evitare blocchi
        if(total == 0){ g.cancelPendingSelectOpponentSpellTrap(); if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Nessuna M/T avversaria"); return; }
        // TODO: creare overlay riutilizzando pannello DeckSend con elenco M/T avversarie + voce Field Spell
        std::uniform_int_distribution<int> dist(0, static_cast<int>(total)-1);
        size_t pick = static_cast<size_t>(dist(_apphandlers_rand));
        (void)g.resolvePendingSelectOpponentSpellTrap(pick);
    });
    ...
    g.events().subscribe(GameEventType::OpponentSpellTrapChoiceRequested, [&](){
        if(ctx.closeResponsePrompt) ctx.closeResponsePrompt();
        auto ownerOpt = g.getPendingOpponentSpellTrapOwner();
        int owner = ownerOpt.has_value() ? *ownerOpt : g.getTurn().getCurrentPlayerIndex();
        int opp = 1 - owner;
        ...
        if(owner == 1){
            ...
            // Usiamo Game internals via public getters: serve la zona ST dell'avversario assoluto
            const auto &oppST = (opp == g.getTurn().getCurrentPlayerIndex()) ? g.getSpellTrapZone() : g.getOpponentSpellTrapZone();
            stCount = oppST.size();
            bool hasField = g.getFieldSpellOf(opp).has_value();
        ...
        // Human (P1): riusa overlay DeckSend per mostrare le carte M/T dell'avversario e la FieldSpell come ultima
        if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(true);
        if(ctx.setDeckSendOwner) ctx.setDeckSendOwner(owner);
        if(ctx.setDeckSendCandidates){
            std::vector<Card> cands;
            std::vector<size_t> indexMap;
            // Ottieni zona ST dell'avversario assoluto
            const auto &oppST = (opp == g.getTurn().getCurrentPlayerIndex()) ? g.getSpellTrapZone() : g.getOpponentSpellTrapZone();
            for(size_t i=0;i<oppST.size();++i){ cands.push_back(oppST[i]); indexMap.push_back(i); }
            // Field Spell come ultimo elemento (indexMap usa ST index per 0..N-1; per Field useremo sentinella SIZE_MAX)
            if(g.getFieldSpellOf(opp).has_value()){
                cands.push_back(*g.getFieldSpellOf(opp));
                indexMap.push_back(static_cast<size_t>(-1));
            }
            ctx.setDeckSendCandidates(cands);
            if(ctx.setDeckSendIndexMap) ctx.setDeckSendIndexMap(indexMap);
            if(cands.empty()){
                g.cancelPendingSelectOpponentSpellTrap();
                if(ctx.setDeckSendChoiceActive) ctx.setDeckSendChoiceActive(false);
                if(ctx.pushBattleFeedbackMsg) ctx.pushBattleFeedbackMsg("Nessuna M/T avversaria");
                return;
            }
            if(ctx.stopAllInputActions) ctx.stopAllInputActions();
        }
