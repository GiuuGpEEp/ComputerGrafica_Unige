...
void AppHandlers::attachGameHandlers(Game &g, const Context &ctx){
    ...
    g.events().subscribe(GameEventType::DeckAddChoiceRequested, [&](){
        ...
        bool useLevel1Dragon = false;
        {
            ...
            useLevel1Dragon = std::any_of(mz.begin(), mz.end(), [](const Card& c){ return c.getName() == std::string("Saggio con Occhi di Blu"); });
    ...
    g.events().subscribe(GameEventType::OwnMonsterChoiceRequested, [&](){
        // Nessun overlay: P1 clicca direttamente un suo mostro in campo (gestito in InputController/Main)
