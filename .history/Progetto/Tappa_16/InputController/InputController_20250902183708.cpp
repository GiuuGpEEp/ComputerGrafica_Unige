    ...
    if(const auto* kpEsc = e.getIf<sf::Event::KeyPressed>()){
        if(kpEsc->code == M_KEY_ESCAPE){
            ...
            if(state.deckSendChoiceActive){
                if(Game* g = state.getGame ? state.getGame() : nullptr){
                    // Se è attiva una richiesta di Add (Melodia), cancella quella; altrimenti Send (Reliquiario)
                    if(g->hasPendingAddFromDeck()) g->cancelPendingAddFromDeck();
                    else g->cancelPendingSendFromDeck();
                }
    ...
    if(!state.inputBlocked() && (e.is<sf::Event::MouseButtonPressed>() || e.is<sf::Event::MouseButtonReleased>())){
        ...
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
            if(mbp->button == M_MOUSE_LEFT){
                ...
                // If the overlay is already visible, use the overlay's currentOwner (set at startHold/open)
                if(state.graveyardOverlay.isOverlayVisible()){
                    int curOwner = state.graveyardOverlay.getCurrentOwner();
                    if(state.getGame){ if(Game* g = state.getGame()) gyCards = g->getGraveyardOf(curOwner); }
                    else { gyCards = state.getGraveyardCards ? state.getGraveyardCards() : std::vector<Card>{}; }
                } else {
                    if(state.getGame && gyOwner >= 0){ if(Game* g = state.getGame()) gyCards = g->getGraveyardOf(gyOwner); }
                    else { gyCards = state.getGraveyardCards ? state.getGraveyardCards() : std::vector<Card>{}; }
                }
                state.graveyardOverlay.handleMousePress(mousePosF, gyCards);
                // Se c'è una richiesta di scarto dalla mano pendente, intercetta il click sulla mano del proprietario pendente
                if(state.gamestate == GameState::Playing && state.getGame){
                    if(Game* g = state.getGame()){
                        if(g->hasPendingHandDiscard()){
                            // Determina l'owner della richiesta e controlla i click su quella mano (0=bottom, 1=top)
                            int owner = g->getPendingHandDiscardOwner().value_or(0);
                            auto &hand = g->getHandOf(owner);
                            for(size_t i=0;i<hand.size();++i){
                                if(hand[i].isClicked(sf::Mouse::getPosition(state.window))){
                                    (void)g->resolvePendingHandDiscard(i);
                                    return true; // consumato
                                }
                            }
                        }
                        // Se c'è una richiesta di selezione del proprio mostro, intercetta il click sulla rispettiva zona mostri
                        if(g->hasPendingOwnMonsterChoice()){
                            int owner = g->getPendingOwnMonsterOwner().value_or(g->getTurn().getCurrentPlayerIndex());
                            // Mappa il click sui 3 slot mostro del proprietario assoluto (0 = bottom, 1 = top)
                            for(size_t i=0;i<3;++i){
                                int slotIdx = g->getMonsterSlotIndexAt(owner, i);
                                if(slotIdx < 0) slotIdx = static_cast<int>(i);
                                sf::Vector2f slotPos = state.field.getSlotPosition(Type::Monster, owner==0?P1:P2, static_cast<size_t>(slotIdx));
                                // Creiamo un rettangolo attorno allo slot per hit-test
                                sf::Vector2f half = state.slotSize * 0.75f; // un po' più permissivo
                                sf::FloatRect rect(slotPos.x - half.x/2.f, slotPos.y - half.y/2.f, half.x, half.y);
                                if(rect.contains(mousePosF)){
                                    // i è un indice di iterazione su 0..2; ma dobbiamo tradurlo nell'indice nella zona
                                    // La zona logica è indicizzata per ordine di inserimento; getMonsterSlotIndexAt fornisce lo slot assegnato.
                                    // Dobbiamo trovare l'indice di zona che ha quello slot.
                                    size_t zoneSize = (owner==0 ? g->getMonsterZone().size() : g->getOpponentMonsterZone().size());
                                    size_t foundZoneIndex = zoneSize; // invalid
                                    for(size_t z=0; z<zoneSize; ++z){
                                        int s = g->getMonsterSlotIndexAt(owner, z);
                                        if(s == slotIdx){ foundZoneIndex = z; break; }
                                    }
                                    if(foundZoneIndex < zoneSize){
                                        if(g->resolvePendingSelectOwnMonster(foundZoneIndex)){
                                            return true; // consumato
                                        }
                                    }
                                }
