    ...
    sf::FloatRect gyRectTop(state.graveyardSlotPosTop, state.slotSize);
        if(const auto* mbp = e.getIf<sf::Event::MouseButtonPressed>()){
            if(mbp->button == M_MOUSE_LEFT){
                ...
                // Se c'è una richiesta di scarto dalla mano pendente, intercetta il click sulla mano P1
                if(state.gamestate == GameState::Playing && state.getGame){
                    if(Game* g = state.getGame()){
                        if(g->hasPendingHandDiscard()){
                            // hit-test sulle carte della mano del P1 (visibili)
                            // Nota: non abbiamo accesso diretto alla mano qui; il main gestisce il drag. Sfruttiamo un semplice hit delegato:
                            // percorriamo la mano visibile P1 tramite grave workaround: usiamo il fatto che il main disegna la mano P1 in basso
                            // Non abbiamo un vettore qui, quindi chiediamo a Game la mano corrente assoluta 0
                            auto &hand = g->getHandOf(0);
