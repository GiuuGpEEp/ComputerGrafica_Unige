    ...
    if(kp3->code == M_KEY_A && state.gamestate == GameState::Playing && !state.inputBlocked()){
        ...
        if(const auto* kp4 = e.getIf<sf::Event::KeyPressed>()){
            ...
            if(state.chooseSummonOrSet && game && state.pendingHandIndexForPlay.has_value() && !state.inputBlocked()){
                // NEW: Attivazione effetto mostro dalla mano (A) per Saggio con Occhi di Blu
                if(kp4->code == M_KEY_A){
                    size_t handIdx = state.pendingHandIndexForPlay.value();
                    // Per ora, abilita solo se la carta è Saggio con Occhi di Blu
                    auto &hand = game->getHandOf(game->getTurn().getCurrentPlayerIndex());
                    if(handIdx < hand.size() && hand[handIdx].getName() == std::string("Saggio con Occhi di Blu")){
                        // Flusso semplificato: scarta Saggio e richiedi selezione proprio mostro, poi apri scelta SS da deck/hand/GY (delegato a Game/effect futuri)
                        // Qui emettiamo solo un feedback e chiudiamo il prompt per non bloccare l'UI, in attesa dell'effetto registrato.
                        state.battleFeedbackMsg = "(WIP) Effetto di Saggio attivato"; state.battleFeedbackClock.restart();
                        // Nota: l'implementazione completa richiede Game::activateMonsterFromHand e un IActivationEffect.
                    } else {
                        state.battleFeedbackMsg = "Questa carta non ha un effetto attivabile dalla mano"; state.battleFeedbackClock.restart();
                    }
