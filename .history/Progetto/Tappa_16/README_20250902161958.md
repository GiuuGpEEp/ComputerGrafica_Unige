...
- La Melodia del Drago che si Risveglia
	...
	- Cosa è stato aggiunto/modificato nei file
		- GameLogic/Game/Game.h, Game.cpp
			- Flusso pendente per Melodia: requestHandDiscard(int), resolvePendingHandDiscard(size_t), setMelodiaAddsRemaining(int), setNextAddFromDeckAfterDiscard(int), hasPendingAddFromDeck(), resolvePendingAddFromDeck(size_t).
...
- Richiamo del Posseduto (semplificato)
	...
	- Cosa è stato aggiunto/modificato nei file
		- GameLogic/Game/Game.h, Game.cpp
			- Flusso pendente per la scelta dal Cimitero: requestSelectFromGraveyard(int), resolvePendingSelectFromGraveyard(size_t), cancelPendingSelectFromGraveyard(), getPendingGraveyardOwner(), hasPendingGraveyardChoice().
			- La resolve evoca specialmente in ATK (scoperta) il Mostro selezionato.
		...
		- Progetto/Tappa_16/main.cpp
			...
			- Conferma: se hasPendingGraveyardChoice(), usa deckSendIndexMap prima di resolvePendingSelectFromGraveyard().
