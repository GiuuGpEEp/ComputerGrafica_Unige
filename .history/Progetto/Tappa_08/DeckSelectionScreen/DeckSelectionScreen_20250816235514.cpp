    ...
    : window(window), textureManager(textureManager), resourceManager(resourceManager), backgroundTexture(backgroundTexture), windowSize(windowSize)
{
    ...
    for (size_t i = 0; i < deckNames.size(); ++i) {
        // Costruisci il percorso della texture in base al nome del deck
        std::string texPath = "../../resources/textures/textureDeck1/" + deckNames[i] + "Tex.png";
        sf::Texture& deckTex = textureManager.getTexture(texPath);
