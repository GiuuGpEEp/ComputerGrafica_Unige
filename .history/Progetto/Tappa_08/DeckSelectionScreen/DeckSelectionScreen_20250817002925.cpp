...
DeckSelectionScreen::DeckSelectionScreen(sf::RenderWindow& window, TextureManager& textureManager, ResourceManager& resourceManager, const sf::Texture& backgroundTexture, sf::Vector2u windowSize, sf)
...
void DeckSelectionScreen::draw() {
    ...
    if (!deckSprites.empty()) {
        ...
        float textX = windowSize.x / 2.f - deckNameText.getLocalBounds().width / 2.f;
