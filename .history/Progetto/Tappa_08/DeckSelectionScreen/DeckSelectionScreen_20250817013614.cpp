...
void DeckSelectionScreen::draw() {
    ...
    sf::Text deckNameText(deckLabels[i], font, 28);
        deckNameText.setFillColor(sf::Color::White);
        deckNameText.setStyle(sf::Text::Bold);
    float textX = boxX + (boxWidth - deckNameText.getLocalBounds().width) / 2.f;
        ...
        if (selectedDeckIndex == i) {
            sf::Text selectedText("Hai selezionato il deck: " + deckLabels[i], font, 32);
            selectedText.setFillColor(sf::Color::Yellow);
            selectedText.setStyle(sf::Text::Bold);
            float selTextX = boxX + (boxWidth - selectedText.getLocalBounds().width) / 2.f;
...
void DeckSelectionScreen::handleClick() {
    ...
    for (int i = 0; i < numDecks; ++i) {
        float boxX = startX + i * (boxWidth + spacing);
        sf::FloatRect boxRect(boxX, boxY, boxWidth, boxHeight);
        if (boxRect.contains(static_cast<sf::Vector2f>(mousePos))) {
            selectedDeckIndex = i;
            break;
        }
    }
