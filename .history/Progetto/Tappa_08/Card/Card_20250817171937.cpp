...
Attribute Card::stringToAttribute(const std::string& str) {
    // Normalizzazione: minuscolo e rimozione accenti basilare (solo 'à' per il nostro caso)
    std::string norm;
    norm.reserve(str.size());
    for(char c : str){
        unsigned char uc = static_cast<unsigned char>(c);
        // Sostituisco eventuale 'à' (UTF-8 gestito solo per singolo byte se già normalizzato; se multibyte lo ignoro qui)
        if(c == '\xC3') { // probabile inizio di carattere accentato in UTF-8, salta gestione semplice
            continue; // lascio che la versione senza accento sia già presente nel JSON se necessario
        }
        if(c == '\xA0') continue; // spazio non-break o byte residuo, ignoro
        if(c == 'à' || c == 'À') c = 'a';
        norm.push_back(static_cast<char>(::tolower(uc)));    
    }

    if (norm == "luce") return Attribute::Luce;
    if (norm == "oscurita" || norm == "oscurità") return Attribute::Oscurita; // entrambe convergono dopo normalizzazione
    if (norm == "vento") return Attribute::Vento;
    if (norm == "acqua") return Attribute::Acqua;
    if (norm == "fuoco") return Attribute::Fuoco;
    if (norm == "terra") return Attribute::Terra;
    if (norm == "magia") return Attribute::Magia;
    if (norm == "trappola") return Attribute::Trappola;
    if (norm == "nessuno") return Attribute::None;
    throw std::invalid_argument("Invalid attribute string: " + str);
}
