...
Card Card::cardFromJson(const nlohmann::json& jsonData, sf::Vector2f pos, sf::Vector2f size, TextureManager& textureManager) {
    ...
    // Usa il percorso completo per la texture del deck1
    std::string deck1TexturePath = "../../Progetto/resources/textures/textureDeck1/" + jsonData["texture"].get<std::string>();
    sf::Texture& texture = textureManager.getDeck1Texture(deck1TexturePath);
    ...
    card.originalTexturePath = deck1TexturePath;
    return card;
}
...
Attribute Card::stringToAttribute(const std::string& str) {
    ...
    if (str == "Oscurità") return Attribute::Oscurita;
