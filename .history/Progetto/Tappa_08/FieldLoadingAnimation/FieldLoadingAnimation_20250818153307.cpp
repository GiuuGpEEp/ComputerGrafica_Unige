...
FieldLoadingAnimation::FieldLoadingAnimation(const sf::Texture& homeTex, const sf::Texture& fieldTex, sf::Vector2u windowSize) 
    ...
    homeSprite.setScale((float)windowSize.x / homeSprite.getTexture()->getSize().x,
                        (float)windowSize.y / homeSprite.getTexture()->getSize().y);
    fieldSprite.setScale((float)windowSize.x / fieldSprite.getTexture()->getSize().x,
                         (float)windowSize.y / fieldSprite.getTexture()->getSize().y);
}

void FieldLoadingAnimation::start(const sf::Texture& homeTex, const sf::Texture& fieldTex, sf::Vector2u windowSize){
    homeSprite.setTexture(homeTex);
    fieldSprite.setTexture(fieldTex);
    auto sizeH = homeSprite.getTexture()->getSize();
    homeSprite.setScale((float)windowSize.x / sizeH.x, (float)windowSize.y / sizeH.y);
    auto sizeF = fieldSprite.getTexture()->getSize();
    fieldSprite.setScale((float)windowSize.x / sizeF.x, (float)windowSize.y / sizeF.y);
    elapsed = 0.f;
    started = true;
}
...
void FieldLoadingAnimation::draw(sf::RenderWindow& window){
    ...
    // Cross-fade
    sf::Color homeColor = sf::Color::White; homeColor.a = static_cast<sf::Uint8>(255 * (1.f - tt));
    sf::Color fieldColor = sf::Color::White; fieldColor.a = static_cast<sf::Uint8>(255 * tt);
    ...
    auto sizeH = homeSprite.getTexture()->getSize();
    auto sizeF = fieldSprite.getTexture()->getSize();
    homeSprite.setScale((float)window.getSize().x / sizeH.x * homeScaleFactor,
