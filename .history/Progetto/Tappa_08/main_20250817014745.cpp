                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if(gamestate == GameState::DeckSelection){
                        ...
                        if (hoveredIdx != -1) {
                            ...
                            deckSelected = true;
                            // Mostra la stringa di selezione deck
                            deckSelectionScreen.drawSelectedDeckText(detailFont);
                            // Cambia deck e torna alla HomePage
                            deck = resourceManager.getDeckByName(selectedDeckName);
                            homeScreen.setDeckName(selectedDeckName); // Assicurati che HomePage abbia questo metodo
                            gamestate = GameState::HomeScreen;
                        }
