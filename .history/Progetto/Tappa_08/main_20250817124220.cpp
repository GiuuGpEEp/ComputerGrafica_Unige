...
int main(){
    ...
    DeckSelectionScreen deckSelectionScreen(window, textureManager, resourceManager, homeScreenTexture, windowSize, detailFont);
            ...
            if (const auto *mouseButton = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if(gamestate == GameState::DeckSelection){
                        static sf::Clock selectionReturnClock; // avvia il conto alla rovescia quando selezionato
                        ...
                        // Se è stata fatta una selezione resta un attimo sulla schermata (1.2s) poi torna alla home
                        if(deckSelected && selectionReturnClock.getElapsedTime().asSeconds() > 0.5f){
                            gamestate = GameState::HomeScreen;
                        }
                    }
                }
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(showNoDeckWarning){
                ...
                if(elapsed < NO_DECK_WARNING_DURATION){
                    ...
                    sf::Color col = noDeckWarningText.getFillColor();
                    col.a = static_cast<uint8_t>(std::max(0.f, std::min(255.f, alpha)));
                    noDeckWarningText.setFillColor(col);
