...
int main(){
    ...
    if (!detailFont.openFromFile(fontPath)) {
        std::cerr << "ERRORE: Impossibile caricare il font: " << fontPath << std::endl;
    } else {
        std::cout << "Font caricato correttamente: " << fontPath << std::endl;
    }
    ...
    // ========== GESTIONE DEI DECK DISPONIBILI ==========
    std::vector<std::string> deckNames = resourceManager.getDeckNames();
    std::string currentDeckName = "Default";
    
    // Crea deck di default
    Deck deck(std::vector<Card>{}, deckSlotPos, deckCardSize, slotSize, textureFlipped, "Default");
    
    if (!deckNames.empty()) {
        // Seleziona il primo deck disponibile come default
        currentDeckName = deckNames[0];
        const Deck* selectedDeck = resourceManager.getDeckByName(currentDeckName);
        
        if (selectedDeck) {
            // Copia il deck selezionato (se supporta il copy constructor)
            // Altrimenti potresti dover usare un metodo clone o lavorare direttamente con il puntatore
            std::cout << "Deck caricato: " << selectedDeck->getDeckName() << std::endl;
            std::cout << "Numero di carte nel deck: " << selectedDeck->getCards().size() << std::endl;
            
            // Se il Deck supporta il copy constructor, puoi fare:
            // deck = *selectedDeck;
            
            // Altrimenti lavora direttamente con il puntatore:
            // const Deck* currentDeck = selectedDeck;
        }
    } else {
        std::cout << "Nessun deck disponibile nei file JSON." << std::endl;
        std::cout << "Usando deck vuoto di default." << std::endl;
    }

    // ========== MOSTRA INFORMAZIONI DEI DECK DISPONIBILI ==========
    std::cout << "\n========== DECK DISPONIBILI ==========" << std::endl;
    for (const auto& deckName : deckNames) {
        const Deck* deckInfo = resourceManager.getDeckByName(deckName);
        if (deckInfo) {
            std::cout << "- " << deckName << " (" << deckInfo->getCards().size() << " carte)" << std::endl;
        }
    }
    std::cout << "======================================\n" << std::endl;

    // ========== FUNZIONI HELPER PER CAMBIARE DECK ==========
    auto changeDeck = [&](const std::string& newDeckName) {
        const Deck* newDeck = resourceManager.getDeckByName(newDeckName);
        if (newDeck) {
            currentDeckName = newDeckName;
            // deck = *newDeck; // Se supporta copy constructor
            std::cout << "Deck cambiato a: " << newDeckName << std::endl;
            return true;
        } else {
            std::cout << "Deck non trovato: " << newDeckName << std::endl;
            return false;
        }
    };

    auto printCurrentDeckInfo = [&]() {
        const Deck* currentDeck = resourceManager.getDeckByName(currentDeckName);
        if (currentDeck) {
            std::cout << "Deck corrente: " << currentDeck->getDeckName() << std::endl;
            std::cout << "Carte nel deck: " << currentDeck->getCards().size() << std::endl;
            
            // Mostra le prime 5 carte del deck
            const auto& cards = currentDeck->getCards();
            std::cout << "Prime carte del deck:" << std::endl;
            for (size_t i = 0; i < std::min(cards.size(), size_t(5)); ++i) {
                std::cout << "  " << (i+1) << ". " << cards[i].getName() << std::endl;
            }
        }
    };

    // Crea HomePage (assumendo che esista)
    HomePage homeScreen(detailFont, windowSize, homeScreenTexture);
    ...
    while(window.isOpen()){
        while (const std::optional event = window.pollEvent()){
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>()) {
                ...
                // ========== COMANDI KEYBOARD PER TESTARE I DECK ==========
                if (key == sf::Keyboard::Num1 && deckNames.size() >= 1) {
                    changeDeck(deckNames[0]);
                }
                if (key == sf::Keyboard::Num2 && deckNames.size() >= 2) {
                    changeDeck(deckNames[1]);
                }
                if (key == sf::Keyboard::Num3 && deckNames.size() >= 3) {
                    changeDeck(deckNames[2]);
                }
                if (key == sf::Keyboard::I) {
                    printCurrentDeckInfo();
                }
                if (key == sf::Keyboard::L) {
                    std::cout << "Deck disponibili:" << std::endl;
