...
Deck::Deck()
    : cards(), cardsSize(0), animationFinished(true), deckName("")
{
...
Deck::Deck(const Deck& other)
    : cards(other.cards), cardsSize(other.cardsSize), animationFinished(other.animationFinished), deckName(other.deckName)
{
    // shuffleAnimation non viene copiata (puoi implementare la copia profonda se necessario)
                        ...
                        sf::Texture& textureFlipped)
{
    ...
    if(!deckObj.contains("name")) {
        throw std::runtime_error("Il JSON del deck non contiene la chiave 'name'.");
    }
    std::string deckName = deckObj["name"];
    return Deck(deckCards, pos, deckCardSize, slotSize, textureFlipped, deckName);
}



Deck::Deck(const std::vector<Card>& cards, sf::Vector2f pos, sf::Vector2f deckCardSize, sf::Vector2f slotSize, sf::Texture& textureFlipped, std::string deckName)
    : cards(cards), cardsSize(static_cast<int>(cards.size())), animationFinished(false), deckName(deckName)
{
    sf::Vector2f lastPos;
