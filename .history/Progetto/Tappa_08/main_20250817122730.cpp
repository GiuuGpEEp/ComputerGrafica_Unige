...
int main(){
    ...
    sf::Text noDeckWarningText(detailFont, "Non si può giocare se non è stato selezionato un deck", 34);
                ...
                if(but == sf::Mouse::Button::Left) {
                    ...
                    if(gamestate == GameState::DeckSelection){
                        if(deckSelectionScreen.getDeckBlueEyesBounds().contains(initialMousePos)) {
                            ...
                            deckSelected = true;
                            // Aggiorna subito il nome del deck nella HomePage
        ...
        if(gamestate == GameState::HomeScreen){
            ...
            if(showNoDeckWarning){
                ...
                if(elapsed < NO_DECK_WARNING_DURATION){
                    ...
                    col.a = static_cast<sf::Uint8_t>(std::max(0.f, std::min(255.f, alpha)));
