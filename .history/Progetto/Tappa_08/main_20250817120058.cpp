...
int main(){
    ...
    while(window.isOpen()){
        ...
        if(deckSelected) deck = resourceManager.getDeckByName(selectedDeckName);
                ...
                if(deck.isAnimationFinished()) {
                    ...
                    if (!shuffleStarted) {
                        deck.startShuffleAnimationAdvanced(deckSlotPos, deckCardSize);
                                   // Aggiorna la HomePage con il nome del deck selezionato
                                   homeScreen.setDeckName(selectedDeckName);
                                   // Svuota la mano e resetta la pescata
                                   cards.clear();
                                   fieldCards.clear();
                                   cardsToDraw = 5;
                                   animations.clear();
                    ...
                    // Quando l'animazione è finita, mischia effettivamente il deck e passa allo stato PlayinghomeScreen.setDeckName(selectedDeckName);cards.clear();fieldCards.clear();cardsToDraw = 5;animations.clear();
